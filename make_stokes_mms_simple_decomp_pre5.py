...
root = Path.cwd()
(root / "apps/stokes_mms_simple_decomp_gpu_mpi/src").mkdir(parents=True, exist_ok=True)

(root / "apps/stokes_mms_simple_decomp_gpu_mpi/src/main.cu").write_text(r'''#include <mpi.h>
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    momOpt.diffusionScheme = libscalar_decomp::DistDiffusionScheme::NonOrth;
    momOpt.gradScheme = gradScheme;
    momOpt.nNonOrthCorr = nNonOrthCorr;
    momOpt.solver.maxIter = maxit;
    momOpt.solver.absTol = 1e-10;
    momOpt.solver.relTol = 0.0;
    momOpt.solver.monitor = 0;

    DistEllipticOptions pOpt;
    pOpt.gradScheme = gradScheme;
    pOpt.laplacianScheme = "nonorth";
    pOpt.nNonOrthCorr = nNonOrthCorr;
    pOpt.useReferenceCell = true;
    pOpt.referenceGlobalCell = 0;
    pOpt.referenceValue = 0.0;
    pOpt.hypre.maxIter = maxit;
    pOpt.hypre.absTol = 1e-10;
    pOpt.hypre.relTol = 0.0;
    pOpt.hypre.tol = 1e-10;
    ...
    if (rank == 0) {
      std::printf("STOKES_SIMPLE_DECOMP_MMS setup: ranks=%d globalRows=%lld case=%s mu=%.6e tolMass=%.3e nsteps=%d pRelax=%.3f grad=%s nNonOrthCorr=%d\n",
                  size, (long long)dm.globalN, caseRoot.c_str(), mu, tolMass, nsteps, pRelax,
                  gradScheme.c_str(), nNonOrthCorr);
      std::fflush(stdout);
    }
    ...
    for (int step = 1; step <= nsteps; ++step) {
      ...
      if (rank == 0) {
        std::printf("iter %4d : massStar=%.12e massRes=%.12e "
                    "UL2=%.12e Uinf=%.12e pL2=%.12e pInf=%.12e "
                    "velIts=[%d %d %d] pIts=%d pRel=%.3e\n",
                    step, massStar, massAfter,
      ...
      if (massAfter < tolMass) {
        if (rank == 0) {
          std::printf("STOKES_SIMPLE_DECOMP_MMS Converged at iteration %d : massRes=%.12e\n",
                      step, massAfter);
        }
        break;
      }
...
(root / "apps/stokes_mms_simple_decomp_gpu_mpi/CMakeLists.txt").write_text(r'''add_executable(stokes_mms_simple_decomp_gpu_mpi
  src/main.cu
)

target_include_directories(stokes_mms_simple_decomp_gpu_mpi PRIVATE
  ${CMAKE_SOURCE_DIR}/libpoisson/include
  ${CMAKE_SOURCE_DIR}/libpoisson_decomp/include
  ${CMAKE_SOURCE_DIR}/libscalar_decomp/include
)

target_link_libraries(stokes_mms_simple_decomp_gpu_mpi PRIVATE
  libscalar_decomp
  libpoisson_decomp
  libpoisson
  MPI::MPI_CXX
...
set_target_properties(stokes_mms_simple_decomp_gpu_mpi PROPERTIES
...
if "add_subdirectory(apps/stokes_mms_simple_decomp_gpu_mpi)" not in txt:
    txt += "\nadd_subdirectory(apps/stokes_mms_simple_decomp_gpu_mpi)\n"

cmake.write_text(txt)

print("OK: created apps/stokes_mms_simple_decomp_gpu_mpi")
