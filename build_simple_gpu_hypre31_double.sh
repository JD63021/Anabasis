...
EXE=${EXE:-simple_gpu_dp}
LOG=${LOG:-build_simple_gpu_hypre31_double.log}
...
if [[ -x "$EXE" ]]; then
  ln -sf "$EXE" simple_gpu
  echo "Built double-precision HYPRE executable: ./$EXE"
  echo "Symlinked ./simple_gpu -> $EXE"
  ls -lh "$EXE" simple_gpu
else
