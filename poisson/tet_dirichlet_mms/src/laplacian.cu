...
void assemble_poisson_system(
  ...
  for (int f = 0; f < mesh.nInternalFaces; ++f) {
    ...
    double D = dot3(mesh.Sf[f], mesh.Sf[f]) / std::max(dDotS, 1e-30);
  ...
  for (int f = mesh.nInternalFaces; f < mesh.nFaces; ++f) {
    ...
    rhs[P] += static_cast<HYPRE_Complex>(D * boundaryFaceValue[f]);

    if (includeNonOrth) {
      auto T = sub3(mesh.Sf[f], mul3(D, d));
      double corr = dot3(T, grad[P]);
      rhs[P] += static_cast<HYPRE_Complex>(corr);
    }
  }
}
