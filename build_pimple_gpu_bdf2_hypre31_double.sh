...
EXE=${EXE:-pimple_gpu_bdf2_dp}
LOG=${LOG:-build_pimple_gpu_bdf2_hypre31_double.log}

SRC_APP=(
  apps/pimple_gpu/src/main.cu
  apps/pimple_gpu/src/bc_specs.cu
  apps/pimple_gpu/src/patch_geometry.cu
  apps/pimple_gpu/src/velocity_bc_eval.cu
  apps/pimple_gpu/src/bc_runtime_config.cu
)
SRC_LIBPOISSON=(
...
"$NVCC" -std=c++17 -O3 \
  -DANABASIS_EXPECT_HYPRE_COMPLEX_BYTES=8 \
  -arch="$SM_ARCH" \
  -ccbin "$(command -v mpicxx)" \
  -Iapps/pimple_gpu/src \
  -Ilibpoisson/include \
  -Ilibscalar/include \
...
if [[ -x "$EXE" ]]; then
  ln -sf "$EXE" pimple_gpu
  echo "Built double-precision HYPRE BDF2-momentum PIMPLE executable: ./$EXE"
  echo "Symlinked ./pimple_gpu -> $EXE"
  ls -lh "$EXE" pimple_gpu
else
