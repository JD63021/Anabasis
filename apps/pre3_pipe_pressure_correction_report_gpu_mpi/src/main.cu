    ...
    HYPRE_Complex *rhs)
{
  ...
  const double D = ss / dDotS;
  // PRE3E1 pressure-correction skeleton:
...
// -----------------------------------------------------------------------------
// PRE3E1 velocity bridge kernels.
...
// -----------------------------------------------------------------------------
// PRE3E1 pipe pressure correction report.
...
// -----------------------------------------------------------------------------
// PRE3E1 pipe pressure-correction kernels.
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    if(rank == 0) {
      ...
      std::printf("PRE3E1 pipe pressure correction report setup: ranks=%d globalRows=%lld globalNnz=%lld\n",
    ...
    if(!pFace.empty()) {
      ...
      if(procPatches.size() != 1) {
        throw std::runtime_error("PRE3E1 currently expects exactly one processor patch");
      }
      ...
      if(recvN != sendN) {
        throw std::runtime_error("PRE3E1 predictor velocity halo count mismatch");
      }
    ...
    // -----------------------------------------------------------------------
    // PRE3E1: pCorr/pressure halo exchange bridge.
    ...
    if(!pFace.empty()) {
      ...
      if(recvN != sendN) {
        throw std::runtime_error("PRE3E1 pCorr halo count mismatch");
      }
      ...
      // PRE3E1: vector U/V/W halo exchange, host staged first.
      ...
      if(rank == 0) {
        std::printf("PRE3E1 vector halo / velocity correction bridge: globalMaxVecJump=%.12e globalMaxVelCorr=%.12e globalSumVelCorr=%.12e\n",
      ...
      if(rank == 0) {
        std::printf("PRE3E1 pCorr halo/correction bridge: nProcFacesLocal=%zu globalMaxHaloJump=%.12e globalMaxFaceCorr=%.12e globalSumAbsFaceCorr=%.12e\n",
                    pFace.size(), globalMaxHaloJump, globalMaxFaceCorr, globalSumAbsFaceCorr);
      }

      // PRE3E1 pressure-correction residual: apply A*x-rhs on GPU.
    ...
    if(rank == 0) {
      std::printf("PRE3E1 pipe pressure correction report solve RESULT: solveErr=%d its=%d finalRel=%.12e xL2=%.12e xInf=%.12e wall=%.6e s\n",
                  (int)solveErr, (int)its, double(rel), globalL2, globalInf, maxWall);
      if(std::isfinite(globalL2) && std::isfinite(globalInf)) {
        std::printf("PRE3E1 RESULT: PASS_RAN\n");
      } else {
        std::printf("PRE3E1 RESULT: FAIL_NAN_INF\n");
      }
    }
