    ...
    HYPRE_Complex *rhs)
{
  ...
  const double D = ss / dDotS;
  // PRE3F1 momentum-solve skeleton:
...
// -----------------------------------------------------------------------------
// PRE3F1 velocity bridge kernels.
...
// -----------------------------------------------------------------------------
// PRE3F1 pipe momentum-solve kernels.
...
// -----------------------------------------------------------------------------
// PRE3F1 explicit flux correction report.
...
// -----------------------------------------------------------------------------
// PRE3F1 axial momentum boundary contribution.
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    if(rank == 0) {
      ...
      std::printf("PRE3F1 momentum flux audit setup: ranks=%d globalRows=%lld globalNnz=%lld\n",
    ...
    // PRE3F1 axial W momentum predictor:
    ...
    // -----------------------------------------------------------------------
    // PRE3F1: W/pressure halo exchange bridge.
    ...
    if(!pFace.empty()) {
      ...
      if(recvN != sendN) {
        throw std::runtime_error("PRE3F1 W halo count mismatch");
      }
      ...
      // PRE3F1: vector U/V/W halo exchange, host staged first.
      ...
      if(rank == 0) {
        std::printf("PRE3F1 vector halo / velocity correction bridge: globalMaxVecJump=%.12e globalMaxVelCorr=%.12e globalSumVelCorr=%.12e\n",
      ...
      if(rank == 0) {
        std::printf("PRE3F1 W halo/correction bridge: nProcFacesLocal=%zu globalMaxHaloJump=%.12e globalMaxFaceCorr=%.12e globalSumAbsFaceCorr=%.12e\n",
                    pFace.size(), globalMaxHaloJump, globalMaxFaceCorr, globalSumAbsFaceCorr);
      }
      // PRE3F1 explicit momentum-solve face-flux report.
      ...
      if(rank == 0) {
        std::printf("PRE3F1 explicit flux correction report: maxInternal=%.12e sumInternal=%.12e maxBoundary=%.12e sumBoundary=%.12e maxProcessor=%.12e sumProcessor=%.12e\n",
                    globalMaxIntCorr,
                    globalSumIntCorr,
                    globalMaxBndCorr,
                    globalSumBndCorr,
      ...
      // PRE3F1 momentum-solve residual: apply A*x-rhs on GPU.
      ...
      if(rank == 0) {
        ...
        std::printf("PRE3F1 GPU momentum residual: resInf=%.12e resL2=%.12e rhsInf=%.12e relInf=%.12e\n",
                    globalResInf,
                    globalResL2,
                    globalRhsInf,
                    globalResInf / std::max(globalRhsInf, 1e-300));

        std::printf("PRE3F1 momentum residual summary: massBeforeInf=%.12e massAfterInf=%.12e reduction=%.12e improvement=%.6e\n",
                    massBefore,
                    massAfter,
                    reduction,
                    improvement);
    ...
    if(rank == 0) {
      std::printf("PRE3F1 momentum flux audit W-solve RESULT: solveErr=%d its=%d finalRel=%.12e wL2=%.12e wInf=%.12e wall=%.6e s\n",
                  (int)solveErr, (int)its, double(rel), globalL2, globalInf, maxWall);
      if(std::isfinite(globalL2) && std::isfinite(globalInf)) {
        std::printf("PRE3F1 RESULT: PASS_RAN\n");
      } else {
        std::printf("PRE3F1 RESULT: FAIL_NAN_INF\n");
      }
    }
