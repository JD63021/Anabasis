...
int main(int argc, char **argv){
  ...
  if(rank==0){
    std::printf("============================================================\n");
    std::printf("OpenFOAM polyMesh SIMPLE-like pipe Poiseuille solver\n");
  ...
  if(!par.bcConfigPath.empty()){
    ...
    velocityPatchSpecs.push_back(pipebc::make_fixed_vector_function_bc(
        ...
        [&par](const std::array<double,3>& x, double){
          const double R = 0.5 * par.pipeDiameter;
          const double r2 = x[0]*x[0] + x[1]*x[1];
          const double shape = std::max(0.0, 1.0 - r2 / std::max(R*R, 1.0e-30));
          const double uz = 2.0 * par.Umean * shape;
          return std::array<double,3>{{0.0, 0.0, uz}};
        }));
