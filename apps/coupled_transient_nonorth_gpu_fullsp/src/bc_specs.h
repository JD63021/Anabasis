...
struct VelocityPatchBCSpec {
  ...
  // For ParabolicBoxSineInlet:
  //   U_mag(t) = U_mag_peak * sin(pi*time/parabolicTimeScale)
  // The Schafer-Turek 3D-3Q/3D-3Z benchmark uses parabolicTimeScale=8.0
  // and runs only on 0 <= t <= 8.
  double parabolicTimeScale = 8.0;
...
VelocityPatchBCSpec make_parabolic_box_sine_inlet_bc(
    const std::string& patchName,
    double H,
    double Umax,
    int coordA,
