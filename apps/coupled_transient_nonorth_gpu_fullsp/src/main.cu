...
struct Params {
  ...
  int pGradScheme=0;         // GLOBAL gradient scheme: 0=LSQ, 1=Gauss-linear for pressure, velocity, reconstruction gradients
...
__device__ __managed__ int g_coupledPruned10Pattern = 0;
// Coupled direct-HYPRE stage 1: keep coupled pattern indices in kernels,
// but map MatrixPattern positions to HYPRE ParCSR diag-data positions at use sites.
__device__ __managed__ int g_coupledDirectViaPerm = 0;
__device__ __managed__ int *g_coupledA_diag_perm_d = nullptr;
...
static std::vector<std::string> expand_case_config_args(int argc, char** argv){
  ...
  std::map<std::string,std::string> keyToFlag = {
    ...
    {"gradScheme", "-grad-scheme"},
    {"gradientScheme", "-grad-scheme"},
    {"grad-scheme", "-grad-scheme"},
    {"gradient-scheme", "-grad-scheme"},
    {"pGradScheme", "-grad-scheme"},
    {"p-grad-scheme", "-grad-scheme"},
...
static void parse_args(int argc, char** argv, Params &par){
  for(int i=1;i<argc;++i){
    ...
    if(!std::strcmp(argv[i],"-polyMeshDir")){need(argv[i]); par.polyMeshDir=argv[++i];}
    ...
    else if(!std::strcmp(argv[i],"-grad-scheme") || !std::strcmp(argv[i],"-gradient-scheme") ||
            !std::strcmp(argv[i],"-p-grad-scheme") || !std::strcmp(argv[i],"-pgrad-scheme")){
      need(argv[i]);
      std::string v=argv[++i];
      for(char &c:v) c=(char)std::tolower((unsigned char)c);
      ...
      else { std::fprintf(stderr,"Unknown -grad-scheme '%s'. Use lsq or gauss.\n", v.c_str()); MPI_Abort(MPI_COMM_WORLD,1);}
    }
...
struct GpuMemMarkState {
  bool init = false;
  double first = 0.0;
  double peak = 0.0;
};

static GpuMemMarkState g_gpu_mem_mark_state;

static void gpu_mem_mark(const char *label){
  if(!std::getenv("ANABASIS_GPU_MEM_MARKS")) return;
  CUDA_CALL(cudaDeviceSynchronize());
  const double used = get_gpu_used_mb();
  if(!g_gpu_mem_mark_state.init){
    g_gpu_mem_mark_state.init = true;
    g_gpu_mem_mark_state.first = used;
    g_gpu_mem_mark_state.peak = used;
  }
  if(used > g_gpu_mem_mark_state.peak) g_gpu_mem_mark_state.peak = used;
  std::printf("GPU_MEM_MARK label=%s usedMB=%.3f deltaFirstMB=%.3f peakDeltaMB=%.3f\n",
              label, used, used - g_gpu_mem_mark_state.first,
              g_gpu_mem_mark_state.peak - g_gpu_mem_mark_state.first);
  std::fflush(stdout);
}
...
struct DeviceMesh {
  ...
  HYPRE_Real *d_subx = nullptr, *d_suby = nullptr, *d_subz = nullptr;
  HYPRE_Real *d_subsx = nullptr, *d_subsy = nullptr, *d_subsz = nullptr;

  int nCells=0, nFaces=0, nInternalFaces=0;
  int *d_owner=nullptr, *d_neigh=nullptr, *d_bPatch=nullptr;
  HYPRE_Real *d_ccx=nullptr, *d_ccy=nullptr, *d_ccz=nullptr;
  HYPRE_Real *d_xfx=nullptr, *d_xfy=nullptr, *d_xfz=nullptr;
  HYPRE_Real *d_nfx=nullptr, *d_nfy=nullptr, *d_nfz=nullptr;
  HYPRE_Real *d_sfx=nullptr, *d_sfy=nullptr, *d_sfz=nullptr;
  HYPRE_Real *d_Af=nullptr, *d_vol=nullptr;
};
...
struct DeviceGradientOperator {
  ...
  int *d_src=nullptr;
  int *d_face=nullptr;
  HYPRE_Real *d_cx=nullptr, *d_cy=nullptr, *d_cz=nullptr;
};
...
struct DeviceBC {
  ...
  int *d_type=nullptr;
  HYPRE_Real *d_faceValue=nullptr;
};
...
struct GPULinearSystem {
  ...
  bool direct_values_need_perm=false;
  bool is_setup=false;
};
...
static void copy_vec_to_device_cast(const std::vector<From> &h, To *d){
  if(h.empty()) return;
  std::vector<To> tmp(h.size());
  for(std::size_t i=0; i<h.size(); ++i) tmp[i] = static_cast<To>(h[i]);
  CUDA_CALL(cudaMemcpy(d, tmp.data(), tmp.size()*sizeof(To), cudaMemcpyHostToDevice));
}
...
static void build_device_mesh(const Mesh &mesh, DeviceMesh &dm){
  ...
  std::vector<HYPRE_Real> ccx(mesh.nCells), ccy(mesh.nCells), ccz(mesh.nCells), vol(mesh.nCells);
  std::vector<HYPRE_Real> xfx(mesh.nFaces), xfy(mesh.nFaces), xfz(mesh.nFaces), nfx(mesh.nFaces), nfy(mesh.nFaces), nfz(mesh.nFaces), sfx(mesh.nFaces), sfy(mesh.nFaces), sfz(mesh.nFaces), Af(mesh.nFaces);
  ...
  if(dm.nSubTris > 0){
    ...
    device_alloc(dm.d_subx, h_subx.size()); copy_vec_to_device_cast(h_subx, dm.d_subx);
    device_alloc(dm.d_suby, h_suby.size()); copy_vec_to_device_cast(h_suby, dm.d_suby);
    device_alloc(dm.d_subz, h_subz.size()); copy_vec_to_device_cast(h_subz, dm.d_subz);
    device_alloc(dm.d_subsx, h_subsx.size()); copy_vec_to_device_cast(h_subsx, dm.d_subsx);
    device_alloc(dm.d_subsy, h_subsy.size()); copy_vec_to_device_cast(h_subsy, dm.d_subsy);
    device_alloc(dm.d_subsz, h_subsz.size()); copy_vec_to_device_cast(h_subsz, dm.d_subsz);
  }
...
static void copy_device_to_vec_cast(const From *d, std::vector<To> &h){
  if(h.empty()) return;
  if constexpr (std::is_same<From, To>::value){
    CUDA_CALL(cudaMemcpy(h.data(), d, h.size()*sizeof(To), cudaMemcpyDeviceToHost));
  } else {
    std::vector<From> tmp(h.size());
    CUDA_CALL(cudaMemcpy(tmp.data(), d, tmp.size()*sizeof(From), cudaMemcpyDeviceToHost));
    for(std::size_t i=0; i<h.size(); ++i){
      h[i] = static_cast<To>(tmp[i]);
    }
  }
}
...
static DeviceBC make_device_bc(int nFaces, const std::vector<std::string> &bcType, const std::vector<double> &bcFaceValue){
  ...
  device_alloc(bc.d_faceValue, bc.faceValue.size()); copy_vec_to_device_cast(bc.faceValue, bc.d_faceValue);
  return bc;
}
    ...
    DeviceBC* dbcP)
{
  ...
  if(dbcU){ dbcU->faceValue = uFaceBC; copy_vec_to_device_cast(dbcU->faceValue, dbcU->d_faceValue); }
  if(dbcV){ dbcV->faceValue = vFaceBC; copy_vec_to_device_cast(dbcV->faceValue, dbcV->d_faceValue); }
  if(dbcW){ dbcW->faceValue = wFaceBC; copy_vec_to_device_cast(dbcW->faceValue, dbcW->d_faceValue); }
  if(dbcP){ dbcP->faceValue = pFaceBC; copy_vec_to_device_cast(dbcP->faceValue, dbcP->d_faceValue); }
}

struct DeviceTimeSineVelocityBC {
  int nFaces = 0;
  int *d_face = nullptr;
  HYPRE_Real *d_uBase = nullptr;
  HYPRE_Real *d_vBase = nullptr;
  HYPRE_Real *d_wBase = nullptr;
  HYPRE_Real *d_invTimeScale = nullptr;
};

__global__ static void kernel_update_time_sine_velocity_bc(
    int n, const int *faces,
    const HYPRE_Real *uBase, const HYPRE_Real *vBase, const HYPRE_Real *wBase,
    const HYPRE_Real *invTimeScale, double time,
    HYPRE_Real *uFaceBC, HYPRE_Real *vFaceBC, HYPRE_Real *wFaceBC)
{
    ...
    DeviceTimeSineVelocityBC& tbc)
{
  ...
  device_alloc(tbc.d_uBase, uBase.size()); copy_vec_to_device_cast(uBase, tbc.d_uBase);
  device_alloc(tbc.d_vBase, vBase.size()); copy_vec_to_device_cast(vBase, tbc.d_vBase);
  device_alloc(tbc.d_wBase, wBase.size()); copy_vec_to_device_cast(wBase, tbc.d_wBase);
  device_alloc(tbc.d_invTimeScale, invTimeScale.size()); copy_vec_to_device_cast(invTimeScale, tbc.d_invTimeScale);
}
...
static void init_coupled_simple_scratch_only(GPUSimpleScratch &ss, const Mesh &mesh){
  // Coupled-only memory-probe/reference path:
  // keep only arrays used by the monolithic coupled transient loop.
  // Do NOT use this for potential initialization, nonlinear residual diagnostics,
  // post-coupled pressure corrections, segregated SIMPLE/PIMPLE code, force/scalar
  // extras, or VTU paths that need unavailable diagnostic arrays.
  ss.nCells = mesh.nCells;
  ss.nFaces = mesh.nFaces;

  device_alloc(ss.d_u, mesh.nCells);
  device_alloc(ss.d_v, mesh.nCells);
  device_alloc(ss.d_w, mesh.nCells);
  device_alloc(ss.d_p, mesh.nCells);

  device_alloc(ss.d_uOld, mesh.nCells);
  device_alloc(ss.d_vOld, mesh.nCells);
  device_alloc(ss.d_wOld, mesh.nCells);
  device_alloc(ss.d_pOld, mesh.nCells);

  device_alloc(ss.d_gradx, mesh.nCells);
  device_alloc(ss.d_grady, mesh.nCells);
  device_alloc(ss.d_gradz, mesh.nCells);

  device_alloc(ss.d_phi, mesh.nFaces);
  device_alloc(ss.d_divCorr, mesh.nCells);

  ss.reduceSize = std::max((mesh.nCells + 255)/256, 1);
  device_alloc(ss.d_reduce, ss.reduceSize);
  device_alloc(ss.d_reduce2, ss.reduceSize);
}
    ...
    double &loadedTime)
{
  ...
  copy_vec_to_device(p, ss.d_p);
    gpu_mem_mark("after initial field copies");
...
static void build_lsq_gradient_operator(const Mesh &mesh, DeviceGradientOperator &op, double weightPower){
  ...
  device_alloc(op.d_cx, cx.size()); copy_vec_to_device_cast(cx, op.d_cx);
  device_alloc(op.d_cy, cy.size()); copy_vec_to_device_cast(cy, op.d_cy);
  device_alloc(op.d_cz, cz.size()); copy_vec_to_device_cast(cz, op.d_cz);
}
...
__global__ static void kernel_apply_lsq_gradient(
    ...
    const HYPRE_Real *cx, const HYPRE_Real *cy, const HYPRE_Real *cz,
    const double *phi, const int *bPatch, const int *bcType, const HYPRE_Real *bcFaceValue,
    double *gx, double *gy, double *gz);

__global__ static void kernel_apply_gauss_linear_gradient(
    int nFaces, int nInternalFaces,
    const int *owner, const int *neigh, const int *bPatch,
    const HYPRE_Real *ccx, const HYPRE_Real *ccy, const HYPRE_Real *ccz,
    const HYPRE_Real *xfx, const HYPRE_Real *xfy, const HYPRE_Real *xfz,
    const HYPRE_Real *sfx, const HYPRE_Real *sfy, const HYPRE_Real *sfz,
    const HYPRE_Real *vol,
    const double *phi, const int *bcType, const HYPRE_Real *bcFaceValue,
    double *gx, double *gy, double *gz);
...
static void compute_selected_gradient_gpu(const Params &par, const DeviceGradientOperator &gop,
...
// Backward-compatible name: pGradScheme is now treated as the global gradient scheme.
static void compute_pressure_gradient_gpu(const Params &par, const DeviceGradientOperator &gop,
                                          const DeviceMesh &dm, const DeviceBC &bc,
                                          const double *d_phi, double *d_gx, double *d_gy, double *d_gz){
  compute_selected_gradient_gpu(par, gop, dm, bc, d_phi, d_gx, d_gy, d_gz);
}
...
static void upload_pattern(MatrixPattern &pat){
  ...
  gpu_mem_mark("upload_pattern after row metadata");
  ...
  gpu_mem_mark("upload_pattern after rows_and_cols");
...
__global__ static void kernel_fill_hypre_real(HYPRE_Real *a, int n, double val){
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if(i < n) a[i] = static_cast<HYPRE_Real>(val);
}
...
__global__ static void kernel_build_rhiechow_predicted_flux_stokes_3d(
    ...
    const HYPRE_Real *ccx, const HYPRE_Real *ccy, const HYPRE_Real *ccz,
    const HYPRE_Real *xfx, const HYPRE_Real *xfy, const HYPRE_Real *xfz,
    const HYPRE_Real *nfx, const HYPRE_Real *nfy, const HYPRE_Real *nfz,
    const HYPRE_Real *Af,
    const double *u, const double *v, const double *w,
    const double *p,
    const double *gradx, const double *grady, const double *gradz,
    const HYPRE_Real *rAU,
    const int *bcUType, const HYPRE_Real *uFaceBC,
    const int *bcVType, const HYPRE_Real *vFaceBC,
    const int *bcWType, const HYPRE_Real *wFaceBC,
    double rho,
...
__global__ static void kernel_pressure_nonorth_flux_and_divergence(
    ...
    const HYPRE_Real *nfx, const HYPRE_Real *nfy, const HYPRE_Real *nfz,
    const HYPRE_Real *sfx, const HYPRE_Real *sfy, const HYPRE_Real *sfz,
    const HYPRE_Real *Af, const HYPRE_Real *rAU, double rho,
    const int *bcPType,
    const double *gradx, const double *grady, const double *gradz,
...
__global__ static void kernel_correct_face_fluxes_simple_nonorth(
    ...
    const HYPRE_Real *nfx, const HYPRE_Real *nfy, const HYPRE_Real *nfz,
    const HYPRE_Real *Af, const HYPRE_Real *rAU, double rho,
    const int *bcPType, const HYPRE_Real *pFaceBC,
...
__global__ static void kernel_correct_velocity_from_pcorr_grad(
    ...
    const HYPRE_Real *rAU, const double *gradx, const double *grady, const double *gradz,
...
__global__ static void kernel_momentum_base_steady(int nCells, const int *diagPos, const HYPRE_Real *vol, const double *gradPcomp, HYPRE_Complex *vals, HYPRE_Complex *rhs){
  int c = blockIdx.x * blockDim.x + threadIdx.x;
  if(c < nCells){
    vals[diagPos[c]] = 0.0;
...
__global__ static void kernel_momentum_base_rhs_only(int nCells, const HYPRE_Real *vol, const double *gradPcomp, HYPRE_Complex *rhs){
  int c = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ static void kernel_momentum_boundary_faces_rhs_only(
    ...
    const int *bcQType, const HYPRE_Real *bcQFaceVal,
    const int *bcUType, const HYPRE_Real *bcUFaceVal,
    const int *bcVType, const HYPRE_Real *bcVFaceVal,
    const int *bcWType, const HYPRE_Real *bcWFaceVal,
...
__global__ static void kernel_extract_vol_over_diag(int nCells, const int *diagPos, const HYPRE_Complex *vals, const HYPRE_Real *vol, HYPRE_Real *rAU, double diagScale, double rAUScale){
  int c = blockIdx.x * blockDim.x + threadIdx.x;
  if(c < nCells){
...
__global__ static void kernel_pressure_internal_faces_rau(
    ...
    const HYPRE_Real *xfx, const HYPRE_Real *xfy, const HYPRE_Real *xfz,
    const HYPRE_Real *nfx, const HYPRE_Real *nfy, const HYPRE_Real *nfz,
    const HYPRE_Real *Af, const HYPRE_Real *rAU, double rho, double pCoeffScale,
    const int *facePP, const int *facePN, const int *faceNP, const int *faceNN,
    HYPRE_Complex *vals)
{
  int f = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ static void kernel_scatter_values_perm_chunk(
    int nChunk,
    int offset,
    const HYPRE_Complex *src,
    const int *permChunk,
    HYPRE_Complex *dst)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
static void build_diag_permutation_from_host_parcsr(GPULinearSystem &sys){
  ...
  const char *deferEnv = std::getenv("ANABASIS_DEFER_A_DIAG_PERM_UPLOAD");
  const bool deferPermUpload = deferEnv && std::atoi(deferEnv) != 0;

  if(!identity && !deferPermUpload){
  ...
  } else if(!identity && deferPermUpload){
    gpu_mem_mark("update_ij deferred A_diag_perm_d upload");
  }
}

static void update_ij_matrix_from_device(GPULinearSystem &sys){
  gpu_mem_mark("update_ij entry");
  std::vector<HYPRE_Complex> h_vals(sys.pat.nnz);
  copy_device_to_vec(sys.pat.d_vals, h_vals);
  gpu_mem_mark("update_ij after copy values to host");
  ...
  gpu_mem_mark("update_ij after IJ assemble host");

  const char *freeValsEnv = std::getenv("ANABASIS_FREE_APP_DVALS_BEFORE_IJ_MIGRATE");
  const bool freeValsBeforeMigrate = freeValsEnv && std::atoi(freeValsEnv) != 0;
  if(freeValsBeforeMigrate && !sys.direct_matrix_values && sys.pat.d_vals){
    CUDA_CALL(cudaDeviceSynchronize());
    device_free(sys.pat.d_vals);
    gpu_mem_mark("update_ij after early free app pat.d_vals before IJ migrate");
  }
  ...
  gpu_mem_mark("update_ij after IJ migrate device");
  HYPRE_CALL(HYPRE_IJMatrixGetObject(sys.Aij, (void**)&sys.Apar));
}
...
static void ensure_A_diag_perm_allocated_if_needed(GPULinearSystem &sys, const char *tag);
static void free_A_diag_perm_after_hypre_copy_if_enabled(GPULinearSystem &sys);


static inline bool anabasis_chunked_perm_scatter_enabled(){
  const char *e = std::getenv("ANABASIS_CHUNKED_A_DIAG_PERM_SCATTER");
  return e && std::atoi(e) != 0;
}

static int anabasis_perm_chunk_n(){
  const char *e = std::getenv("ANABASIS_A_DIAG_PERM_CHUNK_N");
  if(e){
    const long v = std::atol(e);
    if(v > 0 && v < 2147483647L) return (int)v;
  }
  return 1048576; // 1M ints = 4 MiB temporary device chunk
}

static void scatter_matrix_values_perm_chunked(GPULinearSystem &sys){
  if(sys.A_diag_identity_perm) return;

  if(!sys.pat.d_vals){
    throw std::runtime_error("chunked perm scatter called with null app pat.d_vals");
  }
  if(!sys.A_diag_data_dev){
    throw std::runtime_error("chunked perm scatter called with null HYPRE diag data pointer");
  }
  if(sys.A_diag_perm_h.empty() || (int)sys.A_diag_perm_h.size() != sys.pat.nnz){
    throw std::runtime_error("chunked perm scatter needs host A_diag_perm_h with size == nnz");
  }

  const int nnz = sys.pat.nnz;
  int chunkN = anabasis_perm_chunk_n();
  if(chunkN <= 0) chunkN = 1048576;
  if(chunkN > nnz) chunkN = nnz;

  int *d_perm_chunk = nullptr;
  device_alloc(d_perm_chunk, chunkN);

  const int block = 256;
  for(int off = 0; off < nnz; off += chunkN){
    const int nThis = std::min(chunkN, nnz - off);
    CUDA_CALL(cudaMemcpy(d_perm_chunk,
                         sys.A_diag_perm_h.data() + off,
                         nThis * sizeof(int),
                         cudaMemcpyHostToDevice));

    const int grid = (nThis + block - 1) / block;
    kernel_scatter_values_perm_chunk<<<grid, block>>>(
        nThis, off, sys.pat.d_vals, d_perm_chunk, sys.A_diag_data_dev);
    CUDA_CHECK_LAST();
  }

  CUDA_CALL(cudaDeviceSynchronize());
  device_free(d_perm_chunk);
  gpu_mem_mark("after chunked A_diag_perm scatter into hypre");
}
...
static void copy_matrix_values_into_hypre(GPULinearSystem &sys){
  ...
  if(!sys.pat.d_vals){
    throw std::runtime_error("copy_matrix_values_into_hypre called with null app pat.d_vals");
  }
  ...
  if(sys.A_diag_identity_perm){
    ...
    if(anabasis_chunked_perm_scatter_enabled()){
      scatter_matrix_values_perm_chunked(sys);
    } else {
      ensure_A_diag_perm_allocated_if_needed(sys, "after realloc A_diag_perm_d before hypre copy");
...
static inline bool anabasis_env_flag_enabled(const char *name){
  const char *e = std::getenv(name);
  return e && std::atoi(e) != 0;
}

static void ensure_app_matrix_values_allocated(GPULinearSystem &sys, const char *tag){
  if(sys.direct_matrix_values) return;
  if(sys.pat.d_vals) return;
  if(sys.pat.nnz <= 0){
    throw std::runtime_error("Cannot allocate app matrix values: pattern nnz <= 0");
  }
  device_alloc(sys.pat.d_vals, sys.pat.nnz);
  CUDA_CALL(cudaMemset(sys.pat.d_vals, 0, sys.pat.nnz * sizeof(HYPRE_Complex)));
  if(tag) gpu_mem_mark(tag);
}

static void free_app_matrix_values_after_hypre_copy_if_enabled(GPULinearSystem &sys){
  if(!anabasis_env_flag_enabled("ANABASIS_FREE_APP_DVALS_AFTER_HYPRE_COPY")) return;
  if(sys.direct_matrix_values) return;
  if(!sys.pat.d_vals) return;

  CUDA_CALL(cudaDeviceSynchronize());
  device_free(sys.pat.d_vals);
  gpu_mem_mark("after free app pat.d_vals after hypre copy");
}

static void ensure_A_diag_perm_allocated_if_needed(GPULinearSystem &sys, const char *tag){
  if(sys.A_diag_identity_perm) return;
  if(sys.A_diag_perm_d) return;
  if(sys.A_diag_perm_h.empty() || (int)sys.A_diag_perm_h.size() != sys.pat.nnz){
    throw std::runtime_error("Cannot re-upload A_diag_perm_d: host permutation is missing or wrong size");
  }

  device_alloc(sys.A_diag_perm_d, sys.pat.nnz);
  CUDA_CALL(cudaMemcpy(sys.A_diag_perm_d,
                       sys.A_diag_perm_h.data(),
                       sys.pat.nnz * sizeof(int),
                       cudaMemcpyHostToDevice));
  if(tag) gpu_mem_mark(tag);
}

static void free_A_diag_perm_after_hypre_copy_if_enabled(GPULinearSystem &sys){
  if(!anabasis_env_flag_enabled("ANABASIS_FREE_A_DIAG_PERM_AFTER_HYPRE_COPY")) return;
  if(sys.A_diag_identity_perm) return;
  if(sys.direct_values_need_perm) return;
  if(!sys.A_diag_perm_d) return;

  CUDA_CALL(cudaDeviceSynchronize());
  device_free(sys.A_diag_perm_d);
  gpu_mem_mark("after free A_diag_perm_d after hypre copy");
}
...
static inline bool anabasis_coupled_direct_hypre_via_perm_enabled(){
  const char *e = std::getenv("ANABASIS_COUPLED_DIRECT_HYPRE_VIA_PERM");
  return e && std::atoi(e) != 0;
}

static void set_coupled_direct_perm_device_state(GPULinearSystem &sys, bool on){
  if(on && !sys.A_diag_identity_perm){
    if(!sys.A_diag_perm_d){
      throw std::runtime_error("Coupled direct-via-perm requested but A_diag_perm_d is null");
    }
    g_coupledDirectViaPerm = 1;
    g_coupledA_diag_perm_d = sys.A_diag_perm_d;
  } else {
    g_coupledDirectViaPerm = 0;
    g_coupledA_diag_perm_d = nullptr;
  }
  CUDA_CALL(cudaDeviceSynchronize());
}

static void enable_coupled_direct_hypre_matrix_updates_via_perm(GPULinearSystem &sys){
  if(sys.direct_matrix_values) return;
  ...
  if(!sys.A_diag_identity_perm){
    ensure_A_diag_perm_allocated_if_needed(sys, "after alloc A_diag_perm_d for coupled direct-via-perm");

    // Diagonal-only coupled kernels access pat.d_diagPos directly instead of
    // going through coupled_pos(). Remap those positions once to HYPRE order.
    remap_device_index_array_inplace(sys.pat.d_diagPos, sys.pat.diagPos.size(), sys.A_diag_perm_d);

    // General coupled row/face kernels still compute MatrixPattern positions;
    // coupled_pos() / coupled_find_col_pos() map those positions through this
    // device permutation at the final indexing step.
    set_coupled_direct_perm_device_state(sys, true);
    sys.direct_values_need_perm = true;
  } else {
    set_coupled_direct_perm_device_state(sys, false);
    sys.direct_values_need_perm = false;
  }
  ...
  if(sys.pat.d_vals){
    CUDA_CALL(cudaDeviceSynchronize());
    device_free(sys.pat.d_vals);
    gpu_mem_mark("after free app pat.d_vals for coupled direct-via-perm");
  }
}

static inline bool anabasis_coupled_direct_free_perm_after_assembly_enabled(){
  const char *e1 = std::getenv("ANABASIS_COUPLED_DIRECT_FREE_PERM_AFTER_ASSEMBLY");
  const char *e2 = std::getenv("ANABASIS_COUPLED_DIRECT_HYPRE_STAGE2");
  return (e1 && std::atoi(e1) != 0) || (e2 && std::atoi(e2) != 0);
}

static void prepare_coupled_direct_perm_for_assembly(GPULinearSystem &sys){
  if(!(sys.direct_matrix_values && sys.direct_values_need_perm)) return;
  ...
  ensure_A_diag_perm_allocated_if_needed(
      sys, "after realloc A_diag_perm_d for coupled direct assembly");

  set_coupled_direct_perm_device_state(sys, true);
}

static void release_coupled_direct_perm_if_requested(GPULinearSystem &sys, const char *tag){
  if(!(sys.direct_matrix_values && sys.direct_values_need_perm)) return;
  if(!anabasis_coupled_direct_free_perm_after_assembly_enabled()) return;

  CUDA_CALL(cudaDeviceSynchronize());

  // Disable the managed device pointer before freeing the backing allocation.
  set_coupled_direct_perm_device_state(sys, false);

  if(sys.A_diag_perm_d){
    device_free(sys.A_diag_perm_d);
    sys.A_diag_perm_d = nullptr;
    gpu_mem_mark(tag ? tag : "after free A_diag_perm_d after coupled direct assembly");
  }
}
...
copy_device_rhs_and_device_x0_into_hypre(GPULinearSystem &sys, const T *d_x0){
  static_assert(sizeof(T) == sizeof(HYPRE_Complex), "device x0 type must match HYPRE_Complex storage");
  CUDA_CALL(cudaMemcpy(sys.b_data_dev, sys.d_rhs, sys.n * sizeof(HYPRE_Complex), cudaMemcpyDeviceToDevice));
...
template<typename T>
static typename std::enable_if<!std::is_same<T,double>::value, void>::type
copy_solution_from_hypre_to_device(GPULinearSystem &sys, T *d_xout){
  static_assert(sizeof(T) == sizeof(HYPRE_Complex), "device xout type must match HYPRE_Complex storage");
...
static void init_common_linear_storage(GPULinearSystem &sys, MatrixPattern pat){
  ...
  gpu_mem_mark("linear before upload_pattern");
  upload_pattern(sys.pat);
  gpu_mem_mark("linear after upload_pattern");
...
static void init_coupled_momentum_scratch_only(GPUMomentumAssembler &mom, const Mesh &mesh){
  // Coupled-only path does not need the segregated momentum HYPRE system.
  // It only needs these gradient work arrays for:
  //   - coupled momentum assembly face-skew terms
  //   - subtriangle velocity flux reconstruction
  //   - nonlinear residual monitor
  device_alloc(mom.d_gradQx, mesh.nCells);
  device_alloc(mom.d_gradQy, mesh.nCells);
  device_alloc(mom.d_gradQz, mesh.nCells);
}
...
static void update_pressure_matrix_from_rAU(const Mesh &mesh, const DeviceMesh &dm, GPULinearSystem &ps, const DeviceBC &bcP, const HYPRE_Real *d_rAU, double rho, double pCoeffScale, int refCell, bool usePressureAnchor, bool doSetup, double &tsetup){
  int block=256;
  int gridVals=(ps.pat.nnz + block - 1)/block;
  int gridFaces=(mesh.nInternalFaces + block - 1)/block;
  int nBoundaryFaces = mesh.nFaces - mesh.nInternalFaces;
    ...
    bool doMatrixSetup)
{
  ...
  its=-1; relres=0.0;
  HYPRE_Int itsTmp=-1; HYPRE_Real relTmp=HYPRE_Real(0);
...
static void solve_pressure_gpu(GPULinearSystem &ps, const std::vector<double> &rhs, const std::vector<double> &x0, double relTol, double absTol, int maxIter, std::vector<double> &xout, HYPRE_Int &its, double &relres, double &tsolve){
  ...
  tsolve += MPI_Wtime()-t0;
  its=0; relres=0.0;
  HYPRE_Real relTmp = HYPRE_Real(0);
  HYPRE_CALL(HYPRE_ParCSRPCGGetNumIterations(ps.solver,&its));
  HYPRE_CALL(HYPRE_ParCSRPCGGetFinalRelativeResidualNorm(ps.solver,&relTmp));
  relres = static_cast<double>(relTmp);
...
static void solve_pressure_gpu_device_rhs(GPULinearSystem &ps, const std::vector<double> &x0, double relTol, double absTol, int maxIter, std::vector<double> &xout, HYPRE_Int &its, double &relres, double &tsolve){
  ...
  gpu_mem_mark("after coupled Krylov solve");
  }
  HYPRE_CALL(HYPRE_ParCSRPCGSetTol(ps.solver, relTol));
  HYPRE_CALL(HYPRE_ParCSRPCGSetAbsoluteTol(ps.solver, absTol));
  HYPRE_CALL(HYPRE_ParCSRPCGSetMaxIter(ps.solver, maxIter));
...
struct GPUCoupledAssembler {
  ...
  CoupledFaceSlots slots;
  HYPRE_Real *d_rAU=nullptr;
  HYPRE_Real *d_xDouble=nullptr;
  double *d_resNonlinear=nullptr;
};

static inline double mib_from_bytes(unsigned long long b){
  return double(b) / (1024.0 * 1024.0);
}


static void audit_coupled_scalar_block_sparsity(const GPULinearSystem &lin,
                                                const char *tag,
                                                double zeroTol)
{
  if(!std::getenv("ANABASIS_COUPLED_BLOCK_AUDIT")) return;

  const MatrixPattern &pat = lin.pat;
  if(pat.nRows <= 0 || pat.nnz <= 0 || pat.rowOffsets.empty() || pat.cols.empty() || !pat.d_vals){
    std::printf("COUPLED_BLOCK_AUDIT tag=%s skipped=1 reason=missing_pattern_or_values\n", tag);
    std::fflush(stdout);
    return;
  }

  std::vector<HYPRE_Complex> h_vals(pat.nnz);
  copy_device_to_vec(pat.d_vals, h_vals);

  unsigned long long alloc[4][4] = {};
  unsigned long long nz[4][4] = {};
  unsigned long long diagAlloc[4][4] = {};
  unsigned long long diagNz[4][4] = {};
  unsigned long long offAlloc[4][4] = {};
  unsigned long long offNz[4][4] = {};
  double sumAbs[4][4] = {};
  double maxAbs[4][4] = {};

  const int nRows = pat.nRows;
  const int nCells = nRows / 4;

  unsigned long long totalAlloc = 0;
  unsigned long long totalNz = 0;
  unsigned long long totalDiagAlloc = 0;
  unsigned long long totalDiagNz = 0;
  unsigned long long totalOffAlloc = 0;
  unsigned long long totalOffNz = 0;

  for(int r=0; r<nRows; ++r){
    const int rc = r & 3;
    const int rowCell = r >> 2;
    const int a = pat.rowOffsets[r];
    const int b = pat.rowOffsets[r+1];

    for(int kk=a; kk<b; ++kk){
      const long long cglob = (long long)pat.cols[kk];
      if(cglob < 0 || cglob >= nRows) continue;

      const int cc = int(cglob & 3LL);
      const int colCell = int(cglob >> 2LL);
      const double av = std::fabs((double)h_vals[kk]);
      ...
      if(av > zeroTol){
        ...
        totalNz++;
        sumAbs[rc][cc] += av;
        if(av > maxAbs[rc][cc]) maxAbs[rc][cc] = av;
  ...
  const char *name[4] = {"Ux","Uy","Wz","p"};

  std::printf("COUPLED_BLOCK_AUDIT tag=%s rows=%d cells=%d nnzAlloc=%llu nnzNonzero=%llu nzPct=%.3f zeroTol=%.3e\n",
              tag, nRows, nCells, totalAlloc, totalNz, pct(totalNz,totalAlloc), zeroTol);

  std::printf("COUPLED_BLOCK_AUDIT_SPLIT tag=%s diagAlloc=%llu diagNz=%llu diagNzPct=%.3f offAlloc=%llu offNz=%llu offNzPct=%.3f\n",
              tag,
              totalDiagAlloc, totalDiagNz, pct(totalDiagNz,totalDiagAlloc),
              totalOffAlloc, totalOffNz, pct(totalOffNz,totalOffAlloc));

  for(int r=0; r<4; ++r){
    for(int c=0; c<4; ++c){
      std::printf("COUPLED_BLOCK_AUDIT_ITEM tag=%s row=%s col=%s alloc=%llu nz=%llu nzPct=%.3f diagAlloc=%llu diagNz=%llu offAlloc=%llu offNz=%llu sumAbs=%.6e maxAbs=%.6e\n",
                  tag, name[r], name[c],
                  alloc[r][c], nz[r][c], pct(nz[r][c], alloc[r][c]),
                  diagAlloc[r][c], diagNz[r][c],
                  offAlloc[r][c], offNz[r][c],
                  sumAbs[r][c], maxAbs[r][c]);
    }
  }

  const unsigned long long velCrossAlloc =
      alloc[0][1]+alloc[0][2]+alloc[1][0]+alloc[1][2]+alloc[2][0]+alloc[2][1];
  const unsigned long long velCrossNz =
      nz[0][1]+nz[0][2]+nz[1][0]+nz[1][2]+nz[2][0]+nz[2][1];

  const unsigned long long velDiagAlloc =
      alloc[0][0]+alloc[1][1]+alloc[2][2];
  const unsigned long long velDiagNz =
      nz[0][0]+nz[1][1]+nz[2][2];

  const unsigned long long gradAlloc = alloc[0][3]+alloc[1][3]+alloc[2][3];
  const unsigned long long gradNz    = nz[0][3]+nz[1][3]+nz[2][3];

  const unsigned long long divAlloc = alloc[3][0]+alloc[3][1]+alloc[3][2];
  const unsigned long long divNz    = nz[3][0]+nz[3][1]+nz[3][2];

  const unsigned long long ppAlloc = alloc[3][3];
  const unsigned long long ppNz    = nz[3][3];

  std::printf("COUPLED_BLOCK_AUDIT_GROUP tag=%s group=velDiag alloc=%llu nz=%llu nzPct=%.3f\n",
              tag, velDiagAlloc, velDiagNz, pct(velDiagNz, velDiagAlloc));
  std::printf("COUPLED_BLOCK_AUDIT_GROUP tag=%s group=velCross alloc=%llu nz=%llu nzPct=%.3f\n",
              tag, velCrossAlloc, velCrossNz, pct(velCrossNz, velCrossAlloc));
  std::printf("COUPLED_BLOCK_AUDIT_GROUP tag=%s group=grad_p_to_mom alloc=%llu nz=%llu nzPct=%.3f\n",
              tag, gradAlloc, gradNz, pct(gradNz, gradAlloc));
  std::printf("COUPLED_BLOCK_AUDIT_GROUP tag=%s group=div_vel_to_cont alloc=%llu nz=%llu nzPct=%.3f\n",
              tag, divAlloc, divNz, pct(divNz, divAlloc));
  std::printf("COUPLED_BLOCK_AUDIT_GROUP tag=%s group=pressure_pressure alloc=%llu nz=%llu nzPct=%.3f\n",
              tag, ppAlloc, ppNz, pct(ppNz, ppAlloc));

  std::fflush(stdout);
}

static void print_coupled_matrix_ledger(const GPUCoupledAssembler &cpl,
                                        const Mesh &mesh,
                                        const Params &par,
                                        const char *tag)
{
  if(!std::getenv("ANABASIS_COUPLED_MATRIX_LEDGER")) return;

  const MatrixPattern &pat = cpl.lin.pat;
  const unsigned long long nRows = (unsigned long long)pat.nRows;
  const unsigned long long nnz   = (unsigned long long)pat.nnz;
  const unsigned long long nCells = (unsigned long long)mesh.nCells;
  const unsigned long long nIF    = (unsigned long long)mesh.nInternalFaces;

  auto live_bytes = [](const void *ptr, unsigned long long n, unsigned long long sz)->unsigned long long {
    return ptr ? n * sz : 0ULL;
  };

  const unsigned long long b_ncols     = live_bytes(pat.d_ncols,      nRows,   sizeof(int));
  const unsigned long long b_rowOffsets= live_bytes(pat.d_rowOffsets, nRows+1, sizeof(int));
  const unsigned long long b_diagPos   = live_bytes(pat.d_diagPos,    nRows,   sizeof(int));
  const unsigned long long b_rows      = live_bytes(pat.d_rows,       nRows,   sizeof(HYPRE_BigInt));
  const unsigned long long b_cols      = live_bytes(pat.d_cols,       nnz,     sizeof(HYPRE_BigInt));
  const unsigned long long b_vals      = live_bytes(pat.d_vals,       nnz,     sizeof(HYPRE_Complex));

  const unsigned long long b_facePP    = live_bytes(pat.d_facePP, nIF, sizeof(int));
  const unsigned long long b_facePN    = live_bytes(pat.d_facePN, nIF, sizeof(int));
  const unsigned long long b_faceNP    = live_bytes(pat.d_faceNP, nIF, sizeof(int));
  const unsigned long long b_faceNN    = live_bytes(pat.d_faceNN, nIF, sizeof(int));

  const unsigned long long b_rhs       = live_bytes(cpl.lin.d_rhs, nRows, sizeof(HYPRE_Complex));
  const unsigned long long b_x         = live_bytes(cpl.lin.d_x,   nRows, sizeof(HYPRE_Complex));
  const unsigned long long b_perm      = live_bytes(cpl.lin.A_diag_perm_d, nnz, sizeof(int));

  const unsigned long long b_slotPP    = live_bytes(cpl.slots.d_slotPP, nIF, sizeof(int));
  const unsigned long long b_slotPN    = live_bytes(cpl.slots.d_slotPN, nIF, sizeof(int));
  const unsigned long long b_slotNP    = live_bytes(cpl.slots.d_slotNP, nIF, sizeof(int));
  const unsigned long long b_slotNN    = live_bytes(cpl.slots.d_slotNN, nIF, sizeof(int));
  const unsigned long long b_cellSelf  = live_bytes(cpl.slots.d_cellSelfSlot, nCells, sizeof(int));

  const unsigned long long b_rAU       = live_bytes(cpl.d_rAU, nCells, sizeof(HYPRE_Real));
  const unsigned long long b_xDouble   = live_bytes(cpl.d_xDouble, 4ULL*nCells, sizeof(HYPRE_Real));
  const unsigned long long b_resNL     = live_bytes(cpl.d_resNonlinear, 4ULL*nCells, sizeof(double));

  const unsigned long long b_pattern_live =
      b_ncols + b_rowOffsets + b_diagPos + b_rows + b_cols + b_vals +
      b_facePP + b_facePN + b_faceNP + b_faceNN;

  const unsigned long long b_vec_live =
      b_rhs + b_x + b_perm;

  const unsigned long long b_slots_live =
      b_slotPP + b_slotPN + b_slotNP + b_slotNN + b_cellSelf;

  const unsigned long long b_cpl_scratch_live =
      b_rAU + b_xDouble + b_resNL;

  const unsigned long long b_total_live =
      b_pattern_live + b_vec_live + b_slots_live + b_cpl_scratch_live;

  CUDA_CALL(cudaDeviceSynchronize());
  const double usedMB = get_gpu_used_mb();

  std::printf("COUPLED_LEDGER tag=%s rows=%llu cells=%llu intFaces=%llu nnz=%llu nnzPerCell=%.3f sizeofComplex=%zu sizeofBigInt=%zu sizeofInt=%zu gpuUsedMB=%.3f\\n",
              tag, nRows, nCells, nIF, nnz, double(nnz)/double(std::max(1ULL,nCells)),
              sizeof(HYPRE_Complex), sizeof(HYPRE_BigInt), sizeof(int), usedMB);

  std::printf("COUPLED_LEDGER_ITEM tag=%s name=pat.d_ncols MiB=%.3f\\n", tag, mib_from_bytes(b_ncols));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=pat.d_rowOffsets MiB=%.3f\\n", tag, mib_from_bytes(b_rowOffsets));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=pat.d_diagPos MiB=%.3f\\n", tag, mib_from_bytes(b_diagPos));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=pat.d_rows MiB=%.3f\\n", tag, mib_from_bytes(b_rows));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=pat.d_cols MiB=%.3f\\n", tag, mib_from_bytes(b_cols));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=pat.d_vals MiB=%.3f\\n", tag, mib_from_bytes(b_vals));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=pat.faceMaps MiB=%.3f\\n", tag, mib_from_bytes(b_facePP+b_facePN+b_faceNP+b_faceNN));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=lin.d_rhs MiB=%.3f\\n", tag, mib_from_bytes(b_rhs));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=lin.d_x MiB=%.3f\\n", tag, mib_from_bytes(b_x));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=lin.A_diag_perm_d MiB=%.3f\\n", tag, mib_from_bytes(b_perm));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=coupled.slots MiB=%.3f\\n", tag, mib_from_bytes(b_slots_live));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=coupled.d_rAU MiB=%.3f\\n", tag, mib_from_bytes(b_rAU));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=coupled.d_xDouble MiB=%.3f\\n", tag, mib_from_bytes(b_xDouble));
  std::printf("COUPLED_LEDGER_ITEM tag=%s name=coupled.d_resNonlinear MiB=%.3f\\n", tag, mib_from_bytes(b_resNL));
  std::printf("COUPLED_LEDGER_TOTAL tag=%s livePatternMiB=%.3f liveVecMiB=%.3f liveSlotsMiB=%.3f liveScratchMiB=%.3f liveAppTotalMiB=%.3f gpuUsedMB=%.3f\\n",
              tag,
              mib_from_bytes(b_pattern_live),
              mib_from_bytes(b_vec_live),
              mib_from_bytes(b_slots_live),
              mib_from_bytes(b_cpl_scratch_live),
              mib_from_bytes(b_total_live),
              usedMB);
  std::fflush(stdout);
}
...
static inline int coupled_pruned10_row_stride_host(int rowVar){
  return (rowVar == 3) ? 4 : 2;
}

static inline int coupled_pruned10_col_offset_host(int rowVar, int colVar){
  if(rowVar == 3){
    return (colVar >= 0 && colVar < 4) ? colVar : -1;
  }
  if(colVar == rowVar) return 0;
  if(colVar == 3) return 1;
  return -1;
}

static CoupledPatternBuild build_coupled_pattern_pruned10(const Mesh &mesh){
  CoupledPatternBuild out;
  MatrixPattern &pat = out.pat;
  CoupledFaceSlots &slots = out.slots;
  ...
  for(int c=0; c<nCells; ++c){
    ...
    for(int rv=0; rv<4; ++rv){
      const int row = 4*c + rv;
      pat.rows[row] = (HYPRE_BigInt)row;
      pat.ncols[row] = coupled_pruned10_row_stride_host(rv) * (int)cols.size();
      pat.rowOffsets[row+1] = pat.rowOffsets[row] + pat.ncols[row];
    }
  }
  ...
  for(int c=0; c<nCells; ++c){
    ...
    for(int rv=0; rv<4; ++rv){
      const int row = 4*c + rv;
      const int base = pat.rowOffsets[row];
      const int stride = coupled_pruned10_row_stride_host(rv);

      for(int sidx=0; sidx<(int)cols.size(); ++sidx){
        const int cc = cols[sidx];

        if(rv == 3){
          for(int cv=0; cv<4; ++cv){
            const int pos = base + stride*sidx + cv;
            pat.cols[pos] = (HYPRE_BigInt)(4*cc + cv);
            if(cc == c && cv == rv) pat.diagPos[row] = pos;
          }
        } else {
          {
            const int cv = rv;
            const int off = coupled_pruned10_col_offset_host(rv, cv);
            const int pos = base + stride*sidx + off;
            pat.cols[pos] = (HYPRE_BigInt)(4*cc + cv);
            if(cc == c) pat.diagPos[row] = pos;
          }
          {
            const int cv = 3;
            const int off = coupled_pruned10_col_offset_host(rv, cv);
  ...
  for(int f=0; f<mesh.nInternalFaces; ++f){
    ...
    // Keep these meaningful for old debug/remap paths: U-row/U-col positions.
    pat.facePP[f] = pat.rowOffsets[4*P+0] + coupled_pruned10_row_stride_host(0)*slots.slotPP[f] + coupled_pruned10_col_offset_host(0,0);
    pat.facePN[f] = pat.rowOffsets[4*P+0] + coupled_pruned10_row_stride_host(0)*slots.slotPN[f] + coupled_pruned10_col_offset_host(0,0);
    pat.faceNP[f] = pat.rowOffsets[4*N+0] + coupled_pruned10_row_stride_host(0)*slots.slotNP[f] + coupled_pruned10_col_offset_host(0,0);
    pat.faceNN[f] = pat.rowOffsets[4*N+0] + coupled_pruned10_row_stride_host(0)*slots.slotNN[f] + coupled_pruned10_col_offset_host(0,0);
  }

  return out;
}

__device__ __forceinline__ int coupled_pruned10_row_stride_device(int rowVar){
  return (rowVar == 3) ? 4 : 2;
}

__device__ __forceinline__ int coupled_pruned10_col_offset_device(int rowVar, int colVar){
  if(rowVar == 3){
    return (colVar >= 0 && colVar < 4) ? colVar : -1;
  }
  if(colVar == rowVar) return 0;
...
__device__ __forceinline__ int coupled_direct_map_pos(int p){
  if(g_coupledDirectViaPerm && g_coupledA_diag_perm_d && p >= 0){
    return g_coupledA_diag_perm_d[p];
  }
  return p;
}
...
__device__ __forceinline__ int coupled_pos(const int *rowOffsets, int cell, int rowVar, int slot, int colVar){
  int p;
  if(g_coupledPruned10Pattern){
    const int off = coupled_pruned10_col_offset_device(rowVar, colVar);
    const int stride = coupled_pruned10_row_stride_device(rowVar);
    p = rowOffsets[4*cell + rowVar] + stride*slot + off;
  } else {
    p = rowOffsets[4*cell + rowVar] + 4*slot + colVar;
  }
  return coupled_direct_map_pos(p);
}
...
__global__ static void kernel_pack_coupled_x(int nCells, const double *u, const double *v, const double *w, const double *p, HYPRE_Real *x){
  int c = blockIdx.x*blockDim.x + threadIdx.x;
  if(c < nCells){
    x[4*c+0] = u[c];
    x[4*c+1] = v[c];
...
__global__ static void kernel_unpack_coupled_x(int nCells, const HYPRE_Real *x, double *u, double *v, double *w, double *p){
  int c = blockIdx.x*blockDim.x + threadIdx.x;
  if(c < nCells){
    u[c] = x[4*c+0];
    v[c] = x[4*c+1];
...
__global__ static void kernel_coupled_add_bdf_momentum_mass(
    int nCells, const HYPRE_Real *vol,
...
__global__ static void kernel_extract_rAU_from_coupled(
    ...
    const HYPRE_Real *vol, HYPRE_Real *rAU, double diagScale, double rAUScale)
{
  int c = blockIdx.x*blockDim.x + threadIdx.x;
  if(c < nCells){
    const int rowU = 4*c + 0;
    ...
    HYPRE_BigInt targetCol)
{
  for(int p = rowOffsets[row]; p < rowOffsets[row+1]; ++p){
    if(cols[p] == targetCol) return coupled_direct_map_pos(p);
  }
  return -1;
}
...
__global__ static void kernel_coupled_pressure_gradient_boundary_subtris_ethier(
    ...
    const int *subFace,
    const HYPRE_Real *subx, const HYPRE_Real *suby, const HYPRE_Real *subz,
    const HYPRE_Real *subsx, const HYPRE_Real *subsy, const HYPRE_Real *subsz,
    const int *owner, const int *bPatch,
    const int *bcPType,
...
__global__ static void kernel_coupled_continuity_velocity_internal_faces(
    ...
    const HYPRE_Real *xfx, const HYPRE_Real *xfy, const HYPRE_Real *xfz,
    const HYPRE_Real *nfx, const HYPRE_Real *nfy, const HYPRE_Real *nfz,
    const HYPRE_Real *Af, double rho,
    const int *rowOffsets,
    const int *slotPP, const int *slotPN, const int *slotNP, const int *slotNN,
    HYPRE_Complex *vals)
{
    ...
    HYPRE_Complex *vals, HYPRE_Complex *rhs)
{
  ...
  const HYPRE_Real *bcVv[3] = {uFaceBC, vFaceBC, wFaceBC};
  for(int cv=0; cv<3; ++cv){
...
__global__ static void kernel_coupled_rc_gradcorr_rhs_internal_faces(
    ...
    const HYPRE_Real *xfx, const HYPRE_Real *xfy, const HYPRE_Real *xfz,
    const HYPRE_Real *nfx, const HYPRE_Real *nfy, const HYPRE_Real *nfz,
    const HYPRE_Real *Af, const HYPRE_Real *rAU,
    const double *gradx, const double *grady, const double *gradz,
...
__global__ static void kernel_coupled_rc_gradcorr_implicit_internal_faces(
    ...
    const HYPRE_Real *cx, const HYPRE_Real *cy, const HYPRE_Real *cz,
    const int *bPatch, const int *bcPType, const HYPRE_Real *pFaceBC,
    const double *pOld,
    ...
    HYPRE_Complex *vals, HYPRE_Complex *rhs)
{
  ...
  for(int p=rowOffsets[row]; p<rowOffsets[row+1]; ++p) vals[coupled_direct_map_pos(p)] = (HYPRE_Complex)0.0;
...
static void init_coupled_system(GPUCoupledAssembler &cpl, const Mesh &mesh, const Params &par){
  gpu_mem_mark("init_coupled entry");

  const bool usePruned10 =
      (std::getenv("ANABASIS_COUPLED_PRUNED10") &&
       std::atoi(std::getenv("ANABASIS_COUPLED_PRUNED10")) != 0);

  g_coupledPruned10Pattern = usePruned10 ? 1 : 0;
  CUDA_CALL(cudaDeviceSynchronize());

  auto build = usePruned10
      ? build_coupled_pattern_pruned10(mesh)
      : build_coupled_pattern_full4(mesh);

  if([](){ int r=0; MPI_Comm_rank(MPI_COMM_WORLD,&r); return r; }() == 0){
    std::printf("Coupled pattern mode: %s\n", usePruned10 ? "pruned10" : "full4");
  }

  gpu_mem_mark(usePruned10 ? "init_coupled after host pruned10 pattern build"
                           : "init_coupled after host full4 pattern build");
  cpl.slots = std::move(build.slots);
  init_common_linear_storage(cpl.lin, std::move(build.pat));
  gpu_mem_mark("init_coupled after init_common_linear_storage");
  print_coupled_matrix_ledger(cpl, mesh, par, "after_init_common_linear_storage");
  upload_coupled_slots(cpl.slots);
  gpu_mem_mark("init_coupled after upload_coupled_slots");
  device_alloc(cpl.d_rAU, mesh.nCells);
  gpu_mem_mark("init_coupled after d_rAU");
  device_alloc(cpl.d_xDouble, 4*mesh.nCells);
  gpu_mem_mark("init_coupled after d_xDouble");
  if(par.nonlinearResidualEnable){
    device_alloc(cpl.d_resNonlinear, 4*mesh.nCells);
  }
  gpu_mem_mark("init_coupled after optional d_resNonlinear");
  print_coupled_matrix_ledger(cpl, mesh, par, "after_coupled_scratch_allocs");
  ...
  gpu_mem_mark("init_coupled before update_ij_matrix_from_device");
  update_ij_matrix_from_device(cpl.lin);
  gpu_mem_mark("init_coupled after update_ij_matrix_from_device");
  print_coupled_matrix_ledger(cpl, mesh, par, "after_update_ij_matrix");

  // Coupled memory optimization step 2:
  // In the restricted memory-probe/reference path below, the coupled assembly
  // no longer needs the app-owned device column array after HYPRE has consumed
  // the matrix structure. HYPRE owns its own ParCSR column arrays.
  //
  // Keep this guarded. d_cols is still required by nonlinear residual diagnostics,
  // LSQ pressure-gradient coupled terms, and implicit RC-gradient paths.
  const bool canFreeCoupledDeviceCols =
      (!par.nonlinearResidualEnable) &&
      (par.pGradScheme == 1) &&
      (!par.coupledRcGradImplicit);

  if(canFreeCoupledDeviceCols){
    device_free(cpl.lin.pat.d_cols);

    // Additional metadata not used by the coupled assembly path after HYPRE
    // has consumed the structure. Keep rowOffsets and diagPos; kernels need them.
    device_free(cpl.lin.pat.d_rows);
    device_free(cpl.lin.pat.d_ncols);
    device_free(cpl.lin.pat.d_facePP);
    device_free(cpl.lin.pat.d_facePN);
    device_free(cpl.lin.pat.d_faceNP);
    device_free(cpl.lin.pat.d_faceNN);

    gpu_mem_mark("after free coupled d_cols_rows_ncols_faceMaps guarded");
    print_coupled_matrix_ledger(cpl, mesh, par, "after_guarded_metadata_frees");
  }
  ...
  if(anabasis_coupled_direct_hypre_via_perm_enabled()){
    enable_coupled_direct_hypre_matrix_updates_via_perm(cpl.lin);
    if([](){ int r=0; MPI_Comm_rank(MPI_COMM_WORLD,&r); return r; }() == 0){
      std::printf("Coupled matrix direct HYPRE updates: enabled via pattern->ParCSR permutation.\n");
    }
    gpu_mem_mark("after enable coupled direct-via-perm");
    print_coupled_matrix_ledger(cpl, mesh, par, "after_enable_coupled_direct_via_perm");
    release_coupled_direct_perm_if_requested(
        cpl.lin, "after free A_diag_perm_d after coupled direct init");
  }
...
static void assemble_coupled_darwish_system(
    ...
    double assemblyTime)
{
  prepare_coupled_direct_perm_for_assembly(cpl.lin);
  ensure_app_matrix_values_allocated(cpl.lin, "after realloc app pat.d_vals before coupled assembly");
  ...
  auto assemble_component = [&](int rowVar, const double *d_q, const DeviceBC &dbcQ){
    compute_selected_gradient_gpu(par, gop, dm, dbcQ, d_q,
  ...
  if(g_coupledSubtriContinuity && dm.nSubTris > 0){
    ...
    if(faceSkewScale != 0.0){
      compute_selected_gradient_gpu(par, gop, dm, dbcU, d_uOld,
                               momScratch.d_gradQx, momScratch.d_gradQy, momScratch.d_gradQz);
      ...
      compute_selected_gradient_gpu(par, gop, dm, dbcV, d_vOld,
                               momScratch.d_gradQx, momScratch.d_gradQy, momScratch.d_gradQz);
      kernel_coupled_continuity_velocity_internal_subtri_skew_rhs_component<<<gridSubTris, block>>>(
      ...
      compute_selected_gradient_gpu(par, gop, dm, dbcW, d_wOld,
                               momScratch.d_gradQx, momScratch.d_gradQy, momScratch.d_gradQz);
      kernel_coupled_continuity_velocity_internal_subtri_skew_rhs_component<<<gridSubTris, block>>>(
  ...
  if(const char* auditEnv = std::getenv("ANABASIS_COUPLED_RC_AUDIT")){
    ...
    if(auditCount < auditLimit){
      ...
      copy_device_to_vec_cast(cpl.d_rAU, h_rAU);
  ...
  release_coupled_direct_perm_if_requested(
      cpl.lin, "after free A_diag_perm_d after coupled direct assembly");
}
...
__global__ static void kernel_coupled_subtri_velocity_flux_correction_component(
    ...
    const double *q,
    const double *gradx, const double *grady, const double *gradz,
    const int *bcType, const HYPRE_Real *qFaceBC,
    double rho, double faceSkewScale,
    double *phi)
{
  ...
  atomicAdd(&phi[f], rho * (qSub - qParent) * Sq);
}
    ...
    double *d_phi)
{
  ...
  compute_selected_gradient_gpu(par, gop, dmesh, dbcU, d_u,
  ...
  compute_selected_gradient_gpu(par, gop, dmesh, dbcV, d_v,
                           mom.d_gradQx, mom.d_gradQy, mom.d_gradQz);
  kernel_coupled_subtri_velocity_flux_correction_component<<<gridSub, block>>>(
  ...
  compute_selected_gradient_gpu(par, gop, dmesh, dbcW, d_w,
                           mom.d_gradQx, mom.d_gradQy, mom.d_gradQz);
  kernel_coupled_subtri_velocity_flux_correction_component<<<gridSub, block>>>(
    ...
    double *d_phi)
{
  ...
  add_coupled_subtri_velocity_flux_correction_if_enabled(
      par, dmesh, mesh, mom, gop, dbcU, dbcV, dbcW,
      d_u, d_v, d_w,
      rho, faceSkewScale, d_phi);
}
    ...
    bool doSetup)
{
  ...
  gpu_mem_mark("before coupled matrix copy");
  {
    double auditTol = 0.0;
    if(const char *e = std::getenv("ANABASIS_COUPLED_BLOCK_AUDIT_TOL")){
      auditTol = std::atof(e);
    }
    audit_coupled_scalar_block_sparsity(cpl.lin, "before_coupled_matrix_copy", auditTol);
  }
  copy_matrix_values_into_hypre(cpl.lin);
  gpu_mem_mark("after coupled matrix copy");
  free_app_matrix_values_after_hypre_copy_if_enabled(cpl.lin);
  free_A_diag_perm_after_hypre_copy_if_enabled(cpl.lin);
  copy_device_rhs_and_device_x0_into_hypre(cpl.lin, d_x0);
  gpu_mem_mark("after coupled rhs/x0 copy");
  ...
  if(doSetup || !cpl.lin.is_setup){
    gpu_mem_mark("before coupled solver setup");
    const double t0 = MPI_Wtime();
    if(cpl.lin.solverKind == 2){
    ...
    gpu_mem_mark("after coupled solver setup");
  }
  ...
  if(cpl.lin.solverKind == 2){
    ...
    relErr = HYPRE_ParCSRFlexGMRESGetFinalRelativeResidualNorm(cpl.lin.solver, &relTmp);
  } else if(cpl.lin.solverKind == 3){
    itsErr = HYPRE_ParCSRGMRESGetNumIterations(cpl.lin.solver, &its);
    relErr = HYPRE_ParCSRGMRESGetFinalRelativeResidualNorm(cpl.lin.solver, &relTmp);
  } else {
    itsErr = HYPRE_ParCSRBiCGSTABGetNumIterations(cpl.lin.solver, &its);
    relErr = HYPRE_ParCSRBiCGSTABGetFinalRelativeResidualNorm(cpl.lin.solver, &relTmp);
  }
  relres = static_cast<double>(relTmp);
  ...
  gpu_mem_mark("after copy solution from hypre");
}
...
CUDA_CALL(cudaFree(0));
  ...
  if(rank==0){
    ...
    std::printf("gradScheme     : %s  (global pressure/velocity/reconstruction gradients)\n", par.pGradScheme == 1 ? "Gauss linear" : "LSQ");
    std::printf("LSQ stencil    : %s%s\n", par.lsqStencilMode == 0 ? "compact" : "extended-accepted-as-compact", par.pGradScheme == 1 ? " (inactive unless LSQ-only options are enabled)" : "");
    std::printf("LSQ weight     : 1/|d|^%.6g%s\n", par.lsqWeightPower, par.pGradScheme == 1 ? " (inactive unless LSQ-only options are enabled)" : "");
    ...
    std::printf("pGradScheme    : %s  (legacy alias for gradScheme)\n", par.pGradScheme == 1 ? "Gauss linear" : "LSQ");
  ...
  gpu_mem_mark("after build_device_mesh");
  ...
  gpu_mem_mark("after DeviceBC uploads");
  ...
  // Coupled-only memory optimization:
  // The app returns from the coupled branch before the old segregated SIMPLE loop.
  // For MMS/coupled reference cases with no potential init and no post-coupled
  // pressure-correction loop, the full segregated momentum and pressure systems
  // are dead resident memory. Keep only the momentum gradient scratch arrays
  // needed by coupled assembly/reconstruction.
  const bool needLegacySegregatedSystems =
      (par.initMode == 1) || (par.coupledPressureNonOrthCorr > 0);

  if(needLegacySegregatedSystems){
    init_momentum_system(mom, mesh);
    gpu_mem_mark("after init_momentum_system full");
    init_pressure_system(pressureSys, mesh, dmesh, par, refCell, usePressureAnchor, pressureSetup);
    gpu_mem_mark("after init_pressure_system full");
  } else {
    init_coupled_momentum_scratch_only(mom, mesh);
    gpu_mem_mark("after init_coupled_momentum_scratch_only");
    gpu_mem_mark("skipped init_pressure_system");
  }

  GPUSimpleScratch ss;

  const bool canUseCoupledSimpleScratchOnly =
      (!par.nonlinearResidualEnable) &&
      (par.initMode != 1) &&
      (par.coupledPressureNonOrthCorr == 0) &&
      (!par.coupledPressureCorrectVelocity) &&
      (!par.forceEnable) &&
      (!par.scalarEnable) &&
      (par.writeEvery == 0);

  if(canUseCoupledSimpleScratchOnly){
    init_coupled_simple_scratch_only(ss, mesh);
    gpu_mem_mark("after init_coupled_simple_scratch_only");
  } else {
    init_simple_scratch(ss, mesh);
    gpu_mem_mark("after init_simple_scratch full");
  ...
  const bool needLsqGradientOperator = (par.pGradScheme != 1) || (par.coupledRcGradImplicit != 0);
  if(needLsqGradientOperator){
    build_lsq_gradient_operator(mesh, gop, par.lsqWeightPower);
  } else if(rank == 0){
    std::printf("LSQ gradient operator: skipped because global gradScheme=gauss and no LSQ-only coupled option is active.\n");
  }
  ...
  if(par.initMode == 1){
    ...
    kernel_fill_hypre_real<<<gridCells, block>>>(mom.d_rAU, mesh.nCells, par.potentialInitRAU);
    ...
    compute_selected_gradient_gpu(par, gop, dmesh, dbcP, ss.d_pCorr,
                             ss.d_gradx, ss.d_grady, ss.d_gradz);
  ...
  // -------------------------------------------------------------------------
  {
    ...
    gpu_mem_mark("after init_coupled_system");
      ...
      PhaseStats &solveStats)
  {
    ...
    for(int it=1; it<=nVelSolves; ++it){
      ...
      compute_selected_gradient_gpu(par, gop, dmesh, dbcQ, d_qOut,
                               mom.d_gradQx, mom.d_gradQy, mom.d_gradQz);
      ...
      PhaseStats &asmStats)
  {
    ...
    PhaseMark pm_grad = profile_begin();
    compute_selected_gradient_gpu(par, gop, dmesh, dbcQ, d_qInitial,
                             mom.d_gradQx, mom.d_gradQy, mom.d_gradQz);
    profile_record(gradStats, pm_grad);
  ...
  for(int step=1; step<=maxSteps; ++step){
    ...
    if(par.pMode == 1 && par.pSolveMode == 1){
      ...
      compute_selected_gradient_gpu(par, gop, dmesh, dbcP, ss.d_p, ss.d_gradx, ss.d_grady, ss.d_gradz);
    }
    if(par.pMode == 1){
      const int block = 256;
    ...
    for(int pcorr=1; pcorr<=nPressureSolves; ++pcorr){
      ...
      compute_selected_gradient_gpu(par, gop, dmesh, dbcP, ss.d_pCorr, ss.d_gradx, ss.d_grady, ss.d_gradz);
    ...
    if(par.pMode == 1){
      ...
      else compute_selected_gradient_gpu(par, gop, dmesh, dbcP, ss.d_p, ss.d_gradx, ss.d_grady, ss.d_gradz);
    } else {
      compute_selected_gradient_gpu(par, gop, dmesh, dbcP, ss.d_pCorr, ss.d_gradx, ss.d_grady, ss.d_gradz);
    }
    ...
    if(rank == 0){
      ...
      if(doBadCellAudit){
        ...
        copy_device_to_vec_cast(mom.d_rAU, rAUH);
