...
static int parse_coord_token(
    const std::string& tok,
    const std::string& path,
    int lineNo) {
  if (tok == "x" || tok == "X" || tok == "0") return 0;
  if (tok == "y" || tok == "Y" || tok == "1") return 1;
  if (tok == "z" || tok == "Z" || tok == "2") return 2;

  parse_fail(path, lineNo,
             "coordinate must be x/y/z or 0/1/2, got '" + tok + "'");
  return 0;
}
...
RuntimeBCConfig load_runtime_bc_config(const std::string& path) {
  ...
  while (std::getline(in, raw)) {
    ...
    if (tok[0] == "velocity") {
      ...
      if (type == "wall_noslip") {
      ...
      } else if (type == "parabolic_box_inlet") {
        if (tok.size() != 7 && tok.size() != 8) {
          parse_fail(path, lineNo,
                     "parabolic_box_inlet syntax: velocity <patch> parabolic_box_inlet <H> <Umax> <coord1> <coord2> [average_patch_normal|local_face_normal]");
        }

        const double H = parse_double_token(tok[3], path, lineNo, "parabolic H");
        const double Umax = parse_double_token(tok[4], path, lineNo, "parabolic Umax");
        const int coordA = parse_coord_token(tok[5], path, lineNo);
        const int coordB = parse_coord_token(tok[6], path, lineNo);

        if (coordA == coordB) {
          parse_fail(path, lineNo, "parabolic_box_inlet requires two different transverse coordinates");
        }

        const NormalVectorMode mode =
            (tok.size() == 8)
                ? parse_normal_mode(tok[7], path, lineNo)
                : NormalVectorMode::AveragePatchNormal;

        cfg.velocityPatchSpecs.push_back(
            make_parabolic_box_inlet_bc(patch, H, Umax, coordA, coordB, mode));
      } else if (type == "parabolic_box_sine_inlet") {
        if (tok.size() != 8 && tok.size() != 9) {
          parse_fail(path, lineNo,
                     "parabolic_box_sine_inlet syntax: velocity <patch> parabolic_box_sine_inlet <H> <Umax> <coord1> <coord2> <timeScale> [average_patch_normal|local_face_normal]");
        }
        ...
        const int coordA = parse_coord_token(tok[5], path, lineNo);
        const int coordB = parse_coord_token(tok[6], path, lineNo);
        const double timeScale = parse_double_token(tok[7], path, lineNo, "sine time scale");

        if (coordA == coordB) {
          parse_fail(path, lineNo, "parabolic_box_sine_inlet requires two different transverse coordinates");
        }
        if (timeScale <= 0.0) {
          parse_fail(path, lineNo, "parabolic_box_sine_inlet requires positive timeScale");
        }

        const NormalVectorMode mode =
            (tok.size() == 9)
                ? parse_normal_mode(tok[8], path, lineNo)
                : NormalVectorMode::AveragePatchNormal;

        cfg.velocityPatchSpecs.push_back(
            make_parabolic_box_sine_inlet_bc(patch, H, Umax, coordA, coordB, timeScale, mode));
      } else {
