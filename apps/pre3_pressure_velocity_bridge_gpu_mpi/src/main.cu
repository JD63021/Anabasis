...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    if(rank == 0) {
      ...
      std::printf("PRE3D1 pressure/velocity bridge setup: ranks=%d globalRows=%lld globalNnz=%lld\n",
    ...
    // -----------------------------------------------------------------------
    // PRE3D1: pCorr/pressure halo exchange bridge.
    ...
    if(!pFace.empty()) {
      ...
      if(procPatches.size() != 1) {
        throw std::runtime_error("PRE3D1 currently expects exactly one processor patch");
      }
      ...
      if(recvN != sendN) {
        throw std::runtime_error("PRE3D1 pCorr halo count mismatch");
      }
      ...
      if(rank == 0) {
        std::printf("PRE3D1 pCorr halo/correction bridge: nProcFacesLocal=%zu globalMaxHaloJump=%.12e globalMaxFaceCorr=%.12e globalSumAbsFaceCorr=%.12e\n",
    ...
    if(rank == 0) {
      std::printf("PRE3D1 pressure/velocity bridge pressure-solve RESULT: solveErr=%d its=%d finalRel=%.12e L2=%.12e Linf=%.12e wall=%.6e s\n",
                  (int)solveErr, (int)its, double(rel), globalL2, globalInf, maxWall);
      if(std::isfinite(globalL2) && std::isfinite(globalInf)) {
        std::printf("PRE3D1 pressure/velocity bridge RESULT: PASS_RAN\n");
      } else {
        std::printf("PRE3D1 pressure/velocity bridge RESULT: FAIL_NAN_INF\n");
      }
    }
