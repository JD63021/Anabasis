...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    if(rank == 0) {
      ...
      std::printf("PRE3D0 pressure bridge setup: ranks=%d globalRows=%lld globalNnz=%lld\n",
    ...
    if(rank == 0) {
      std::printf("PRE3D0 pressure bridge pressure-solve RESULT: solveErr=%d its=%d finalRel=%.12e L2=%.12e Linf=%.12e wall=%.6e s\n",
                  (int)solveErr, (int)its, double(rel), globalL2, globalInf, maxWall);
      if(std::isfinite(globalL2) && std::isfinite(globalInf)) {
        std::printf("PRE3D0 pressure solve RESULT: PASS_RAN\n");
      } else {
        std::printf("PRE3D0 pressure solve RESULT: FAIL_NAN_INF\n");
      }
    }
