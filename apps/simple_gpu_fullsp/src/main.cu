    ...
    HYPRE_Complex *rhs)
{
  const int c = blockIdx.x * blockDim.x + threadIdx.x;
  if(c >= nCells) return;
  const auto q = simple_mms_exact_value((double)ccx[c], (double)ccy[c], (double)ccz[c], rho, mu, U0, P0);
    ...
    int comp)
{
  ...
  if(par.simpleMmsSourceQuad8){
    kernel_add_simple_mms_momentum_source_quad8<<<grid, block>>>(
        ...
        rho, mu, par.simpleMmsVelocityScale, par.simpleMmsPressureScale,
        comp, mom.lin.d_rhs);
  } else {
...
static void update_pressure_matrix_from_rAU(const Mesh &mesh, const DeviceMesh &dm, GPULinearSystem &ps, const DeviceBC &bcP, const cfdScalar *d_rAU, double rho, double pCoeffScale, int refCell, bool usePressureAnchor, bool skipPressureBoundaryCoeffs, bool doSetup, double &tsetup){
  int block=256;
  int gridVals=(ps.pat.nnz + block - 1)/block;
  int gridFaces=(mesh.nInternalFaces + block - 1)/block;
  int nBoundaryFaces = mesh.nFaces - mesh.nInternalFaces;
...
CUDA_CALL(cudaFree(0));
  ...
  if(rank==0){
    ...
    std::printf("SIMPLE MMS     : enable=%d initExact=%d pressureDirichlet=%d velScale=%.8g pScale=%.8g fixedVelBoundaryPEqn=%d sourceQuad8=%d\n",
        ...
        par.simpleMmsFixedVelocityBoundaryPressureEqn, par.simpleMmsSourceQuad8);
  ...
  const bool usePressureAnchor = pressureReferenceNeeded || pressureEqnFixedVelocityBoundaryFlux || (par.sineMmsEnable != 0);
  ...
  for(int step=1; step<=maxSteps; ++step){
    ...
    update_pressure_matrix_from_rAU(mesh, dmesh, pressureSys, dbcP, mom.d_rAU, par.rho, par.pCoeffScale, refCell, usePressureAnchor, pressureEqnFixedVelocityBoundaryFlux, doPressureSetup, pressureSetup);
