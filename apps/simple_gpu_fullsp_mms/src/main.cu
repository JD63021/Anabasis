...
struct Params {
  ...
  double simpleMmsPressurePenaltyCoeff = 0.0; // MMS diagnostic: distributed pressure penalty assembled before AMG setup
    ...
    HYPRE_Complex *rhs)
{
  ...
  const auto q = simple_mms_exact_value(
      (double)ccx[c], (double)ccy[c], (double)ccz[c],
      rho, mu, U0, P0, stokesOnly);
  rhs[c] = (HYPRE_Complex)((double)rhs[c] + beta * q.p);
}


static inline void add_simple_mms_pressure_penalty_rhs_if_needed(
    const Mesh &mesh,
    const DeviceMesh &dm,
    const Params &par,
    double mu,
    HYPRE_Complex *rhs)
{
  if(!(par.simpleMmsEnable && par.simpleMmsPressurePenaltyCoeff != 0.0)) return;
  const int block = 256;
...
static void update_pressure_matrix_from_rAU(const Mesh &mesh, const DeviceMesh &dm, GPULinearSystem &ps, const DeviceBC &bcP, const cfdScalar *d_rAU, double rho, double pCoeffScale, double pressurePenaltyCoeff, int refCell, bool usePressureAnchor, bool skipPressureBoundaryCoeffs, bool doSetup, double &tsetup){
  int block=256;
  int gridVals=(ps.pat.nnz + block - 1)/block;
  int gridFaces=(mesh.nInternalFaces + block - 1)/block;
  int nBoundaryFaces = mesh.nFaces - mesh.nInternalFaces;
...
CUDA_CALL(cudaFree(0));
  ...
  const bool usePressureAnchor = originalUsePressureAnchor &&
      !(par.simpleMmsEnable && par.simpleMmsPressurePenaltyCoeff != 0.0);

  if(rank==0){
    std::printf("Pressure anchor enabled : %s\n", usePressureAnchor ? "yes" : "no");
    std::printf("NOANCHOR_PREPENALTY_SP originalUsePressureAnchor=%s beta=%.12e\n",
  ...
  if(par.initMode == 1){
    ...
    update_pressure_matrix_from_rAU(mesh, dmesh, pressureSys, dbcP, mom.d_rAU,
                                    par.rho, par.pCoeffScale, par.simpleMmsPressurePenaltyCoeff, refCell, usePressureAnchor,
    ...
    CUDA_CHECK_LAST();
    add_simple_mms_pressure_penalty_rhs_if_needed(mesh, dmesh, par, mu, pressureSys.d_rhs);
  ...
  for(int step=1; step<=maxSteps; ++step){
    ...
    update_pressure_matrix_from_rAU(mesh, dmesh, pressureSys, dbcP, mom.d_rAU, par.rho, par.pCoeffScale, par.simpleMmsPressurePenaltyCoeff, refCell, usePressureAnchor, pressureEqnFixedVelocityBoundaryFlux, doPressureSetup, pressureSetup);
