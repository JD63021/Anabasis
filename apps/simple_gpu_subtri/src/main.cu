...
struct Params {
  ...
  // These are OFF by default so the original simple_gpu behavior is preserved.
  // They can also be overridden by environment variables with the same meaning:
  //   ANABASIS_SIMPLE_FACE_SKEW_CORR=1
  //   ANABASIS_SIMPLE_SUBTRI_CONTINUITY=1
  //   ANABASIS_SIMPLE_SUBTRI_FLUX_RECON=1
  int simpleFaceSkewCorr = 0;      // add deferred parent-face velocity skew to phiStar
  int simpleSubtriContinuity = 0;  // use subtriangle-corrected phiStar for pressure RHS/divergence
  int simpleSubtriFluxRecon = 0;
  int simplePressureSubtri = 0;   // optional experimental pressure subtri nonorth correction   // use subtriangle-corrected velocity flux in final phi if continuity path is off

  // Steady SIMPLE MMS verification mode for unit-cube tests.
  // Manufactured field is a 2D Taylor-Green-like divergence-free velocity
  // extruded in z, with a 3D pressure.  The body force is added to the
  // momentum RHS so the steady incompressible Navier-Stokes equations are
  // satisfied at the cell centres:
  //   div(rho U q) - div(mu grad q) = -grad(p)_q + f_q.
  ...
  // MMS debug instrumentation. 0 disables staged diagnostics.
  int simpleMmsDebugEvery = 0;
  int simpleMmsDebugMaxStep = 0;   // 0 = no cap
  int simpleMmsAuditPressureEqn = 0; // final audit: exact p in current pressure equation
  // For MMS with exact velocity imposed on every boundary, the pressure
  // equation must not alter boundary fluxes. Exact pressure Dirichlet is still
  // used for pressure-gradient evaluation, but pressure-equation boundary
  // coefficients/flux corrections are skipped and an anchor is used.
  int simpleMmsFixedVelocityBoundaryPressureEqn = 1;
  // Diagnostic only: replace SIMPLE predictor flux with analytical MMS
  // phiStar = rho*(U_exact + rAU grad(p_exact)) dot Sf on internal faces,
  // and rho*U_exact dot Sf on fixed-velocity boundary faces.
  int simpleMmsForceExactPhiStar = 0;
  // Diagnostic only: build phiStar using the exact same discrete pressure
  // operator that the pressure equation uses. This removes continuum-vs-
  // discrete pressure-gradient mismatch from the test.
  int simpleMmsForceDiscreteExactPhiStar = 0;
  // Diagnostic only for unit-cube uniform blockMesh MMS:
  // use exact analytical face integrals of U instead of face-centre U dot Sf.
  int simpleMmsForceExactIntegratedUFlux = 0;
  // Production-relevant SIMPLE fix:
  // build the pressure-lift part of phiStar using the same pressure-flux
  // correction operator used by the pressure equation.
  int simpleConsistentPhiStar = 0;
  // MMS-only diagnostic/fix:
  // build pressure-RHS phiStar using same face-jump pressure operator
  // used later for pressure correction:
  //   phiStar = phiPred - D_f * jump(pOld)
  int simpleMmsUseJumpLiftPhiStar = 0;
  // BlockMesh MMS diagnostic: integrate manufactured momentum source
  // over each Cartesian cell using 2x2x2 Gauss quadrature instead of midpoint.
  int simpleMmsSourceQuad8 = 0;
  // Diagnostic only: trace actual pressure-equation state at pressure solve.
  int simpleMmsTracePressureEqn = 0;
...
static std::vector<std::string> expand_case_config_args(int argc, char** argv){
  ...
  std::map<std::string,std::string> keyToFlag = {
    ...
    {"simplePressureSubtri", "-simple-pressure-subtri"},
    ...
    {"simpleMmsDebugEvery", "-simple-mms-debug-every"},
    {"simple-mms-debug-every", "-simple-mms-debug-every"},
    {"simpleMmsDebugMaxStep", "-simple-mms-debug-max-step"},
    {"simple-mms-debug-max-step", "-simple-mms-debug-max-step"},
    {"simpleMmsAuditPressureEqn", "-simple-mms-audit-pressure-eqn"},
    {"simple-mms-audit-pressure-eqn", "-simple-mms-audit-pressure-eqn"},
    ...
    {"simpleMmsForceExactPhiStar", "-simple-mms-force-exact-phistar"},
    {"simple-mms-force-exact-phistar", "-simple-mms-force-exact-phistar"},
    {"simpleMmsForceDiscreteExactPhiStar", "-simple-mms-force-discrete-exact-phistar"},
    {"simple-mms-force-discrete-exact-phistar", "-simple-mms-force-discrete-exact-phistar"},
    {"simpleMmsForceExactIntegratedUFlux", "-simple-mms-force-exact-integrated-u-flux"},
    {"simple-mms-force-exact-integrated-u-flux", "-simple-mms-force-exact-integrated-u-flux"},
    {"simpleConsistentPhiStar", "-simple-consistent-phistar"},
    {"simple-consistent-phistar", "-simple-consistent-phistar"},
    {"simpleMmsUseJumpLiftPhiStar", "-simple-mms-use-jump-lift-phistar"},
    {"simple-mms-use-jump-lift-phistar", "-simple-mms-use-jump-lift-phistar"},
    {"simpleMmsSourceQuad8", "-simple-mms-source-quad8"},
    {"simple-mms-source-quad8", "-simple-mms-source-quad8"},
    {"simpleMmsTracePressureEqn", "-simple-mms-trace-pressure-eqn"},
    {"simple-mms-trace-pressure-eqn", "-simple-mms-trace-pressure-eqn"},
...
static void parse_args(int argc, char** argv, Params &par){
  for(int i=1;i<argc;++i){
    ...
    if(!std::strcmp(argv[i],"-polyMeshDir")){need(argv[i]); par.polyMeshDir=argv[++i];}
      ...
      else if(!std::strcmp(argv[i],"-simple-pressure-subtri")){need(argv[i]); par.simplePressureSubtri=std::atoi(argv[++i]);}
    ...
    else if(!std::strcmp(argv[i],"-simple-mms-debug-every")){need(argv[i]); par.simpleMmsDebugEvery=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-mms-debug-max-step")){need(argv[i]); par.simpleMmsDebugMaxStep=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-mms-audit-pressure-eqn")){need(argv[i]); par.simpleMmsAuditPressureEqn=std::atoi(argv[++i]);}
    ...
    else if(!std::strcmp(argv[i],"-simple-mms-force-exact-phistar")){need(argv[i]); par.simpleMmsForceExactPhiStar=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-mms-force-discrete-exact-phistar")){need(argv[i]); par.simpleMmsForceDiscreteExactPhiStar=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-mms-force-exact-integrated-u-flux")){need(argv[i]); par.simpleMmsForceExactIntegratedUFlux=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-consistent-phistar")){need(argv[i]); par.simpleConsistentPhiStar=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-mms-use-jump-lift-phistar")){need(argv[i]); par.simpleMmsUseJumpLiftPhiStar=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-mms-source-quad8")){need(argv[i]); par.simpleMmsSourceQuad8=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-simple-mms-trace-pressure-eqn")){need(argv[i]); par.simpleMmsTracePressureEqn=std::atoi(argv[++i]);}
    ...
    double U0, double P0)
{
  ...
  const double convx = rho * (q.u*ux + q.v*uy);
  const double convy = rho * (q.u*vx + q.v*vy);
    ...
    int& nUsed)
{
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    const double V = mesh.vol[c];
    const auto q = simple_mms_exact_value(x, y, z, rho, mu, U0, P0);
    const double dp = (p[c] - q.p) - pMeanOffset;
    const double pe = q.p;
    pErr2 += V*dp*dp;
    pEx2  += V*pe*pe;
  }
...
static void simple_mms_pressure_shape_diagnostic(
    const Mesh& mesh,
    const std::vector<double>& p,
    double rho, double mu, double U0, double P0,
    double margin,
    const char* label)
{
  ...
  const double mx = sx / std::max(vol, 1.0e-300);
  const double my = sy / std::max(vol, 1.0e-300);
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    const double V = mesh.vol[c];
    const auto q = simple_mms_exact_value(x, y, z, rho, mu, U0, P0);
    const double ex = q.p - mx;
    const double ey = p[c] - my;
  ...
  const double alpha = cov / std::max(varx, 1.0e-300);
  const double beta = my - alpha * mx;

  double err2 = 0.0, errFit2 = 0.0, ex2 = 0.0;
  double errInf = 0.0, errFitInf = 0.0;
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    const double e = (p[c] - q.p) - (my - mx);
    const double ef = p[c] - (alpha * q.p + beta);

    err2 += V * e * e;
    errFit2 += V * ef * ef;
    ex2 += V * q.p * q.p;

    errInf = std::max(errInf, std::fabs(e));
    errFitInf = std::max(errFitInf, std::fabs(ef));
  }

  const double rel = std::sqrt(std::max(err2, 0.0)) / std::sqrt(std::max(ex2, 1.0e-300));
  const double relFit = std::sqrt(std::max(errFit2, 0.0)) / std::sqrt(std::max(ex2, 1.0e-300));

  std::printf(
      "MMSP_SHAPE scope=%s alpha=%.12e beta=%.12e relMeanFree=%.12e relAfterScaleOffset=%.12e "
      "infMeanFree=%.12e infAfterScaleOffset=%.12e corr=%.12e\n",
      label, alpha, beta, rel, relFit, errInf, errFitInf,
      cov / std::sqrt(std::max(varx*vary, 1.0e-300)));
}



static void simple_mms_pressure_layer_mode_diagnostic(
    const Mesh& mesh,
    const std::vector<double>& p,
    double rho, double mu, double U0, double P0)
{
  const double pi = 3.141592653589793238462643383279502884;
  const int Nguess = std::max(1, (int)std::llround(std::cbrt((double)mesh.nCells)));
  const double h = 1.0 / (double)Nguess;
  const int maxLayer = std::max(0, Nguess/2);

  double vol = 0.0;
  double sx = 0.0, sy = 0.0;

  for(int c=0; c<mesh.nCells; ++c){
    const auto q = simple_mms_exact_value(mesh.cc[c][0], mesh.cc[c][1], mesh.cc[c][2],
                                          rho, mu, U0, P0);
    const double V = mesh.vol[c];
    vol += V;
    sx += V * q.p;
  ...
  double varx = 0.0, cov = 0.0, ex2 = 0.0;
  for(int c=0; c<mesh.nCells; ++c){
    const auto q = simple_mms_exact_value(mesh.cc[c][0], mesh.cc[c][1], mesh.cc[c][2],
                                          rho, mu, U0, P0);
    const double V = mesh.vol[c];
    const double pe = q.p;
    const double dx = pe - mx;
    const double dy = p[c] - my;
    varx += V * dx * dx;
    cov  += V * dx * dy;
    ex2  += V * pe * pe;
  }
  ...
  double totalE2 = 0.0;
  double totalFitE2 = 0.0;
  std::vector<int> count(maxLayer + 1, 0);
  std::vector<double> layerVol(maxLayer + 1, 0.0);
  std::vector<double> layerE2(maxLayer + 1, 0.0);
  std::vector<double> layerFitE2(maxLayer + 1, 0.0);
  std::vector<double> layerMean(maxLayer + 1, 0.0);
  std::vector<double> layerFitMean(maxLayer + 1, 0.0);
  std::vector<double> layerMax(maxLayer + 1, 0.0);
  std::vector<double> layerFitMax(maxLayer + 1, 0.0);
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    const double d = std::min(std::min(x, 1.0-x), std::min(std::min(y, 1.0-y), std::min(z, 1.0-z)));
    int layer = (int)std::floor(d / h);
    if(layer < 0) layer = 0;
    ...
    layerVol[layer] += V;
    layerE2[layer] += V * e * e;
    layerFitE2[layer] += V * ef * ef;
    layerMean[layer] += V * e;
    layerFitMean[layer] += V * ef;
    layerMax[layer] = std::max(layerMax[layer], std::fabs(e));
    layerFitMax[layer] = std::max(layerFitMax[layer], std::fabs(ef));

    totalE2 += V * e * e;
    totalFitE2 += V * ef * ef;
  }

  std::printf("MMSP_LAYER_HEADER Nguess=%d alpha=%.12e beta=%.12e totalRel=%.12e totalRelFit=%.12e\n",
      Nguess, alpha, beta,
      std::sqrt(std::max(totalE2,0.0)) / std::sqrt(std::max(ex2,1.0e-300)),
      std::sqrt(std::max(totalFitE2,0.0)) / std::sqrt(std::max(ex2,1.0e-300)));

  const int printLayers = std::min(maxLayer, 8);
  for(int k=0; k<=printLayers; ++k){
    if(count[k] == 0) continue;
    const double rel = std::sqrt(std::max(layerE2[k],0.0)) / std::sqrt(std::max(ex2,1.0e-300));
    const double relFit = std::sqrt(std::max(layerFitE2[k],0.0)) / std::sqrt(std::max(ex2,1.0e-300));
    std::printf(
        "MMSP_LAYER k=%d cells=%d volFrac=%.12e relContrib=%.12e relFitContrib=%.12e "
        "fracE2=%.12e fracFitE2=%.12e meanE=%.12e meanFitE=%.12e maxE=%.12e maxFitE=%.12e\n",
        k, count[k], layerVol[k] / std::max(vol,1.0e-300),
        rel, relFit,
        layerE2[k] / std::max(totalE2,1.0e-300),
        layerFitE2[k] / std::max(totalFitE2,1.0e-300),
        layerMean[k] / std::max(layerVol[k],1.0e-300),
        layerFitMean[k] / std::max(layerVol[k],1.0e-300),
        layerMax[k], layerFitMax[k]);
  }
    ...
    for(int b=1; b<=Mmax; ++b){
      for(int c=1; c<=Mmax; ++c){
        ...
        for(int cell=0; cell<mesh.nCells; ++cell){
          const double x = mesh.cc[cell][0];
          const double y = mesh.cc[cell][1];
          const double z = mesh.cc[cell][2];
          const auto q = simple_mms_exact_value(x, y, z, rho, mu, U0, P0);
          const double ef = p[cell] - (alpha*q.p + beta);
          const double basis = std::sin(a*pi*x) * std::sin(b*pi*y) * std::sin(c*pi*z);
          const double V = mesh.vol[cell];
          num += V * ef * basis;
          den += V * basis * basis;
        }
        const double coeff = num / std::max(den, 1.0e-300);
        const double energy = coeff * coeff * den;
        modes.push_back({a,b,c,coeff, energy / std::max(totalFitE2, 1.0e-300)});
      }
    }
  }

  std::sort(modes.begin(), modes.end(), [](const ModeRec& x, const ModeRec& y){
    return x.frac > y.frac;
  });

  const int nPrint = std::min((int)modes.size(), 12);
  for(int i=0; i<nPrint; ++i){
    std::printf("MMSP_MODE rank=%d mode=%d,%d,%d coeff=%.12e fracFitE2=%.12e\n",
        i+1, modes[i].a, modes[i].b, modes[i].c, modes[i].coeff, modes[i].frac);
  }
}
...
struct SimpleMmsNormPack {
  int n = 0;
  double vol = 0.0;
  double pMeanOffset = 0.0;

  double uL1Abs = 0.0, uL2Abs = 0.0, uLinfAbs = 0.0;
  double uL1Rel = 0.0, uL2Rel = 0.0, uLinfRel = 0.0;

  double pL1Abs = 0.0, pL2Abs = 0.0, pLinfAbs = 0.0;
  double pL1Rel = 0.0, pL2Rel = 0.0, pLinfRel = 0.0;
};

static SimpleMmsNormPack simple_mms_compute_norm_pack(
    const Mesh& mesh,
    const std::vector<double>& u,
    const std::vector<double>& v,
    const std::vector<double>& w,
    ...
    double margin)
{
  ...
  out.pMeanOffset = (out.vol > 0.0) ? pDiffVol / out.vol : 0.0;

  double uL1Ex = 0.0, uL2Ex2 = 0.0, uLinfEx = 0.0;
  double pL1Ex = 0.0, pL2Ex2 = 0.0, pLinfEx = 0.0;
  double uL2Err2 = 0.0, pL2Err2 = 0.0;
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    const double dv = v[c] - q.v;
    const double dw = w[c] - q.w;
    const double ue = std::sqrt(du*du + dv*dv + dw*dw);
    const double ux = std::sqrt(q.u*q.u + q.v*q.v + q.w*q.w);

    const double dp = (p[c] - q.p) - out.pMeanOffset;
    const double pe = std::fabs(q.p);

    out.uL1Abs += V * ue;
    uL2Err2    += V * ue * ue;
    out.uLinfAbs = std::max(out.uLinfAbs, ue);

    uL1Ex   += V * ux;
    uL2Ex2  += V * ux * ux;
    uLinfEx = std::max(uLinfEx, ux);

    out.pL1Abs += V * std::fabs(dp);
    pL2Err2    += V * dp * dp;
    out.pLinfAbs = std::max(out.pLinfAbs, std::fabs(dp));

    pL1Ex   += V * pe;
    pL2Ex2  += V * pe * pe;
    pLinfEx = std::max(pLinfEx, pe);
  }

  out.uL2Abs = std::sqrt(std::max(uL2Err2, 0.0));
  out.pL2Abs = std::sqrt(std::max(pL2Err2, 0.0));

  out.uL1Rel   = out.uL1Abs   / std::max(uL1Ex, 1.0e-300);
  out.uL2Rel   = out.uL2Abs   / std::sqrt(std::max(uL2Ex2, 1.0e-300));
  out.uLinfRel = out.uLinfAbs / std::max(uLinfEx, 1.0e-300);

  out.pL1Rel   = out.pL1Abs   / std::max(pL1Ex, 1.0e-300);
  out.pL2Rel   = out.pL2Abs   / std::sqrt(std::max(pL2Ex2, 1.0e-300));
  out.pLinfRel = out.pLinfAbs / std::max(pLinfEx, 1.0e-300);
...
static void simple_mms_trace_k0k1_pressure_correction_sidebins_host(
    const char* label,
    int step,
    const Mesh& mesh,
    const std::vector<double>& rawFlux,
    const std::vector<double>& afterFlux,
    const std::vector<double>& exactFlux)
{
  const int Nguess = std::max(1, (int)std::llround(std::cbrt((double)mesh.nCells)));
  const double h = 1.0 / (double)Nguess;

  auto layer_of_cell = [&](int c)->int {
    const double x = mesh.cc[c][0];
    const double y = mesh.cc[c][1];
    const double z = mesh.cc[c][2];
    const double d = std::min(std::min(x, 1.0-x), std::min(std::min(y, 1.0-y), std::min(z, 1.0-z)));
    int k = (int)std::floor(d / h);
    if(k < 0) k = 0;
    if(k > Nguess/2) k = Nguess/2;
    return k;
  };

  struct Acc {
    int n = 0;
    double tt = 0.0, aa = 0.0, rr = 0.0, at = 0.0;
    double l1r = 0.0, maxr = 0.0;
    double meanr = 0.0;
  };
  ...
  const char* sideName[6] = {"xmin", "xmax", "ymin", "ymax", "zmin", "zmax"};

  for(int f=0; f<mesh.nInternalFaces; ++f){
    const int P = mesh.owner[f];
    const int N = mesh.neigh[f];
    const int kP = layer_of_cell(P);
    const int kN = layer_of_cell(N);

    if(!((kP == 0 && kN == 1) || (kP == 1 && kN == 0))) continue;

    const double ax = std::fabs(mesh.nf[f][0]);
    const double ay = std::fabs(mesh.nf[f][1]);
    const double az = std::fabs(mesh.nf[f][2]);

    int si = -1;

    if(ax >= ay && ax >= az){
      si = (mesh.xf[f][0] < 0.5 ? 0 : 1);
    } else if(ay >= ax && ay >= az){
      si = (mesh.xf[f][1] < 0.5 ? 2 : 3);
    } else {
      si = (mesh.xf[f][2] < 0.5 ? 4 : 5);
    }

    const double target  = exactFlux[f] - rawFlux[f];
    const double applied = afterFlux[f] - rawFlux[f];
    const double resid   = applied - target;

    auto& a = side[si];
    a.n += 1;
    a.tt += target * target;
    a.aa += applied * applied;
    a.rr += resid * resid;
    a.at += applied * target;
    a.l1r += std::fabs(resid);
    a.maxr = std::max(a.maxr, std::fabs(resid));
    a.meanr += resid;
  }

  for(int si=0; si<6; ++si){
    const auto& a = side[si];
    if(a.n == 0) continue;

    const double alpha = a.at / std::max(a.tt, 1.0e-300);
    const double relErr = std::sqrt(std::max(a.rr, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));
    const double appliedOverTarget = std::sqrt(std::max(a.aa, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));

    std::printf(
        "MMSCORRFIT_SIDE step=%04d label=%s side=%s nFaces=%d "
        "alpha=%.12e relErr=%.12e appliedNormOverTarget=%.12e "
        "resFluxL1=%.12e resFluxL2=%.12e resFluxMax=%.12e resFluxMean=%.12e\n",
        step, label, sideName[si], a.n,
        alpha, relErr, appliedOverTarget,
        a.l1r, std::sqrt(std::max(a.rr, 0.0)), a.maxr, a.meanr / std::max(1, a.n));
  }
}



static void simple_mms_write_k0k1_pressure_correction_faces_csv(
    ...
    const std::vector<double>& exactFlux)
{
  ...
  std::string path = std::string(outPrefix) + "_k0k1_corrfit_faces_step" +
      (step < 10 ? "000" : (step < 100 ? "00" : (step < 1000 ? "0" : ""))) +
      std::to_string(step) + ".csv";

  FILE* fp = std::fopen(path.c_str(), "w");
  if(!fp){
    std::printf("MMSCORRFIT_FACECSV failed_to_open path=%s\n", path.c_str());
    return;
  }

  std::fprintf(fp,
      "face,owner,neigh,orient,xf,yf,zf,Af,nx,ny,nz,"
      "rawFlux,afterFlux,exactFlux,targetFlux,appliedFlux,residualFlux,"
      "absResidualFlux,ratioAppliedOverTarget,residualOverArea,"
      "pOwnerExact,pNeighExact,pJumpExact,"
      "targetOverPJump,appliedOverPJump,residualOverPJump,"
      "ownerLayer,neighLayer,ownerX,ownerY,ownerZ,neighX,neighY,neighZ\n");
  ...
  for(int f=0; f<mesh.nInternalFaces; ++f){
    ...
    const char* orient = "xnormal";
    if(ay >= ax && ay >= az) orient = "ynormal";
    if(az >= ax && az >= ay) orient = "znormal";

    const double raw = rawFlux[f];
    const double aft = afterFlux[f];
    const double exa = exactFlux[f];

    const double target = exa - raw;
    const double applied = aft - raw;
    const double residual = aft - exa;
    const double ratio = std::fabs(target) > 1.0e-300 ? applied / target : 0.0;
    const double A = std::max(mesh.Af[f], 1.0e-300);

    const auto qP = simple_mms_exact_value(mesh.cc[P][0], mesh.cc[P][1], mesh.cc[P][2],
                                           1.0, 1.0, 1.0, 1.0);
    const auto qN = simple_mms_exact_value(mesh.cc[N][0], mesh.cc[N][1], mesh.cc[N][2],
                                           1.0, 1.0, 1.0, 1.0);

    const double pPExact = qP.p;
    const double pNExact = qN.p;
    const double pJumpExact = pNExact - pPExact;
    const double invPJump = std::fabs(pJumpExact) > 1.0e-300 ? 1.0 / pJumpExact : 0.0;

    std::fprintf(fp,
        "%d,%d,%d,%s,%.17e,%.17e,%.17e,%.17e,%.17e,%.17e,%.17e,"
        "%.17e,%.17e,%.17e,%.17e,%.17e,%.17e,"
        "%.17e,%.17e,%.17e,"
        "%.17e,%.17e,%.17e,%.17e,%.17e,%.17e,"
        "%d,%d,%.17e,%.17e,%.17e,%.17e,%.17e,%.17e\n",
        f, P, N, orient,
        mesh.xf[f][0], mesh.xf[f][1], mesh.xf[f][2],
        mesh.Af[f], mesh.nf[f][0], mesh.nf[f][1], mesh.nf[f][2],
        raw, aft, exa, target, applied, residual,
        std::fabs(residual), ratio, residual / A,
        pPExact, pNExact, pJumpExact,
        target * invPJump, applied * invPJump, residual * invPJump,
        kP, kN,
        mesh.cc[P][0], mesh.cc[P][1], mesh.cc[P][2],
        mesh.cc[N][0], mesh.cc[N][1], mesh.cc[N][2]);
  ...
  std::printf("MMSCORRFIT_FACECSV label=%s step=%04d wrote=%d path=%s\n",
      label, step, nWrite, path.c_str());
}



// PATCH_MMS_CORRFIT_TOPO_GROUPS

// PATCH_MMS_TOPO_DIVERGENCE_TRACE
static void simple_mms_trace_flux_divergence_topo_host(
    const char* label,
    int step,
    const Mesh& mesh,
    const std::vector<double>& flux)
{
  std::vector<int> layer(mesh.nCells, -1);

  // topo0 = cells touching any boundary face
  for(int f=mesh.nInternalFaces; f<mesh.nFaces; ++f){
    const int c = mesh.owner[f];
    if(c >= 0 && c < mesh.nCells) layer[c] = 0;
  }

  const int maxTopoLayer = 5;
  for(int k=0; k<maxTopoLayer; ++k){
    bool changed = false;
    for(int f=0; f<mesh.nInternalFaces; ++f){
      const int P = mesh.owner[f];
      const int N = mesh.neigh[f];
      if(P < 0 || P >= mesh.nCells || N < 0 || N >= mesh.nCells) continue;
      if(layer[P] == k && layer[N] < 0){ layer[N] = k + 1; changed = true; }
      if(layer[N] == k && layer[P] < 0){ layer[P] = k + 1; changed = true; }
    }
    if(!changed) break;
  }
  ...
  for(int f=0; f<mesh.nInternalFaces; ++f){
    const int P = mesh.owner[f];
    const int N = mesh.neigh[f];
    const double ph = flux[f];
    if(P >= 0 && P < mesh.nCells) div[P] += ph;
    if(N >= 0 && N < mesh.nCells) div[N] -= ph;
  }

  for(int f=mesh.nInternalFaces; f<mesh.nFaces; ++f){
    const int P = mesh.owner[f];
    if(P >= 0 && P < mesh.nCells) div[P] += flux[f];
  }
  ...
  struct Acc {
    ...
    double l1OverVol = 0.0;
    double l2OverVol = 0.0;
    double linfOverVol = 0.0;
  };

  Acc acc[8];
  const char* names[8] = {
    "topo0", "topo1", "topo2", "topo3", "topo4", "topo5",
    "far_or_unassigned", "all"
  };
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    for(int pass=0; pass<2; ++pass){
      const int gg = (pass == 0) ? g : 7;
      const double v = div[c];

      acc[gg].n++;
      const double vc = std::max(mesh.vol[c], 1.0e-300);
      const double vOverVol = v / vc;

      acc[gg].l1 += std::fabs(v);
      acc[gg].l2 += v*v;
      acc[gg].linf = std::max(acc[gg].linf, std::fabs(v));
      acc[gg].signedSum += v;
      acc[gg].vol += mesh.vol[c];

      acc[gg].l1OverVol += std::fabs(vOverVol);
      acc[gg].l2OverVol += vOverVol*vOverVol;
      acc[gg].linfOverVol = std::max(acc[gg].linfOverVol, std::fabs(vOverVol));
    }
  }

  for(int g=0; g<8; ++g){
    if(acc[g].n == 0) continue;

    std::printf(
        "MMSDIV_TOPO step=%04d label=%s group=%s nCells=%d "
        "L1mean=%.12e L2rms=%.12e Linf=%.12e signedSum=%.12e vol=%.12e\n",
        step, label, names[g], acc[g].n,
        acc[g].l1 / std::max(1, acc[g].n),
        std::sqrt(acc[g].l2 / std::max(1, acc[g].n)),
        acc[g].linf,
        acc[g].signedSum,
        acc[g].vol);

    std::printf(
        "MMSDIVVOL_TOPO step=%04d label=%s group=%s nCells=%d "
        "L1mean=%.12e L2rms=%.12e Linf=%.12e\n",
        step, label, names[g], acc[g].n,
        acc[g].l1OverVol / std::max(1, acc[g].n),
        std::sqrt(acc[g].l2OverVol / std::max(1, acc[g].n)),
        acc[g].linfOverVol);
  }
}


static void simple_mms_trace_pressure_correction_fit_topo_groups_host(
    const char* label,
    int step,
    const Mesh& mesh,
    const std::vector<double>& rawFlux,
    ...
    const std::vector<double>& exactFlux)
{
  ...
  // topo0 = cells directly owning boundary faces
  for(int f=mesh.nInternalFaces; f<mesh.nFaces; ++f){
    const int c = mesh.owner[f];
    if(c >= 0 && c < mesh.nCells) layer[c] = 0;
  }

  // Small BFS by repeated internal-face sweeps. No need for a separate adjacency.
  const int maxTopoLayer = 4;
  for(int k=0; k<maxTopoLayer; ++k){
    bool changed = false;
    for(int f=0; f<mesh.nInternalFaces; ++f){
      const int P = mesh.owner[f];
  ...
  struct Acc {
    int n = 0;
    double tt = 0.0, aa = 0.0, rr = 0.0, at = 0.0;
    double sumAbsTv = 0.0, sumAbsAv = 0.0, sumAbsRv = 0.0;
    double maxAbsTv = 0.0, maxAbsAv = 0.0, maxAbsRv = 0.0;
    int nP = 0;
    double sumTP = 0.0, sumAP = 0.0, sumRP = 0.0;
    double sumTP2 = 0.0, sumAP2 = 0.0, sumRP2 = 0.0;
  };

  // group: all_internal, topo0_topo0, topo0_topo1, topo0_far, non_topo0
  Acc acc[5][3];
  const char* gname[5] = {"all_internal", "topo0_topo0", "topo0_topo1", "topo0_far", "non_topo0"};
  const char* oname[3] = {"xnormal", "ynormal", "znormal"};

  int topoCounts[6] = {0,0,0,0,0,0};
  for(int c=0; c<mesh.nCells; ++c){
    int k = layer[c];
    if(k < 0) topoCounts[5]++;
    else if(k < 5) topoCounts[k]++;
    else topoCounts[5]++;
  }
  ...
  for(int f=0; f<mesh.nInternalFaces; ++f){
    ...
    const int kP = layer[P];
    const int kN = layer[N];

    int g = 4;
    if(kP == 0 || kN == 0){
      if(kP == 0 && kN == 0) g = 1;
      else if((kP == 0 && kN == 1) || (kP == 1 && kN == 0)) g = 2;
      else g = 3;
    }
    ...
    const double applied = afterFlux[f] - rawFlux[f];
    const double resid   = applied - target;
    const double Aface = std::max(mesh.Af[f], 1.0e-300);

    const auto qP = simple_mms_exact_value(mesh.cc[P][0], mesh.cc[P][1], mesh.cc[P][2],
                                           1.0, 1.0, 1.0, 1.0);
    const auto qN = simple_mms_exact_value(mesh.cc[N][0], mesh.cc[N][1], mesh.cc[N][2],
                                           1.0, 1.0, 1.0, 1.0);
    const double pJump = qN.p - qP.p;

    for(int pass=0; pass<2; ++pass){
      const int gg = (pass == 0) ? 0 : g;
      Acc& a = acc[gg][o];
      ...
      const double tv = std::fabs(target) / Aface;
      const double av = std::fabs(applied) / Aface;
      const double rv = std::fabs(resid) / Aface;

      a.sumAbsTv += tv;
      a.sumAbsAv += av;
      a.sumAbsRv += rv;

      a.maxAbsTv = std::max(a.maxAbsTv, tv);
      a.maxAbsAv = std::max(a.maxAbsAv, av);
      a.maxAbsRv = std::max(a.maxAbsRv, rv);

      if(std::fabs(pJump) > 1.0e-14){
        const double tp = target / pJump;
        const double ap = applied / pJump;
        const double rp = resid / pJump;
  ...
  std::printf(
      "MMSCORRFIT_TOPO_HEADER step=%04d label=%s cells_topo0=%d topo1=%d topo2=%d topo3=%d topo4=%d unassigned_or_far=%d\n",
      step, label,
      topoCounts[0], topoCounts[1], topoCounts[2],
      topoCounts[3], topoCounts[4], topoCounts[5]);
  ...
  for(int g=0; g<5; ++g){
    for(int o=0; o<3; ++o){
      ...
      const double invn = 1.0 / (double)a.n;
      const double alpha = a.at / std::max(a.tt, 1.0e-300);
      const double relErr =
          std::sqrt(std::max(a.rr, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));
      const double appliedOverTarget =
          std::sqrt(std::max(a.aa, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));

      double tpm=0.0, apm=0.0, rpm=0.0;
      double tps=0.0, aps=0.0, rps=0.0;

      if(a.nP > 0){
        const double invp = 1.0 / (double)a.nP;
        tpm = a.sumTP * invp;
        apm = a.sumAP * invp;
        rpm = a.sumRP * invp;

        tps = std::sqrt(std::max(0.0, a.sumTP2 * invp - tpm*tpm));
        aps = std::sqrt(std::max(0.0, a.sumAP2 * invp - apm*apm));
        rps = std::sqrt(std::max(0.0, a.sumRP2 * invp - rpm*rpm));
      }

      std::printf(
          "MMSCORRFIT_TOPO step=%04d label=%s group=%s orient=%s nFaces=%d nP=%d "
          "alpha=%.12e relErr=%.12e appliedNormOverTarget=%.12e "
          "targetFluxL2=%.12e appliedFluxL2=%.12e residualFluxL2=%.12e "
          "meanAbsTargetVel=%.12e meanAbsAppliedVel=%.12e meanAbsResidualVel=%.12e "
          "maxAbsTargetVel=%.12e maxAbsAppliedVel=%.12e maxAbsResidualVel=%.12e "
          "targetOverPJumpMean=%.12e targetOverPJumpStd=%.12e "
          "appliedOverPJumpMean=%.12e appliedOverPJumpStd=%.12e "
          "residOverPJumpMean=%.12e residOverPJumpStd=%.12e\n",
          step, label, gname[g], oname[o], a.n, a.nP,
          alpha, relErr, appliedOverTarget,
          std::sqrt(std::max(a.tt, 0.0)),
          std::sqrt(std::max(a.aa, 0.0)),
          std::sqrt(std::max(a.rr, 0.0)),
          a.sumAbsTv * invn,
          a.sumAbsAv * invn,
          a.sumAbsRv * invn,
          a.maxAbsTv,
...
static void simple_mms_trace_k0k1_pressure_correction_fit_host(
    const char* label,
    int step,
    const Mesh& mesh,
    const std::vector<double>& rawFlux,
    ...
    const std::vector<double>& exactFlux)
{
  ...
  struct Acc {
    int n = 0;
    double tt = 0.0;   // target dot target
    double aa = 0.0;   // applied dot applied
    double rr = 0.0;   // residual dot residual
    double at = 0.0;   // applied dot target
    double l1t = 0.0;
    double l1a = 0.0;
    double l1r = 0.0;
    double maxT = 0.0;
    double maxA = 0.0;
    double maxR = 0.0;
    double meanAbsTargetVel = 0.0;
    double meanAbsAppliedVel = 0.0;
    double meanAbsResidualVel = 0.0;
    double maxAbsTargetVel = 0.0;
    double maxAbsAppliedVel = 0.0;
    double maxAbsResidualVel = 0.0;
  };
  ...
  for(int f=0; f<mesh.nInternalFaces; ++f){
    ...
    const double target  = exactFlux[f] - rawFlux[f];
    const double applied = afterFlux[f] - rawFlux[f];
    const double resid   = applied - target; // == afterFlux - exactFlux
    ...
    a.l1t += std::fabs(target);
    a.l1a += std::fabs(applied);
    a.l1r += std::fabs(resid);

    a.maxT = std::max(a.maxT, std::fabs(target));
    a.maxA = std::max(a.maxA, std::fabs(applied));
    a.maxR = std::max(a.maxR, std::fabs(resid));

    a.meanAbsTargetVel += std::fabs(target) / Aface;
    a.meanAbsAppliedVel += std::fabs(applied) / Aface;
    a.meanAbsResidualVel += std::fabs(resid) / Aface;

    a.maxAbsTargetVel = std::max(a.maxAbsTargetVel, std::fabs(target) / Aface);
    a.maxAbsAppliedVel = std::max(a.maxAbsAppliedVel, std::fabs(applied) / Aface);
    a.maxAbsResidualVel = std::max(a.maxAbsResidualVel, std::fabs(resid) / Aface);
  }

  const char* name[3] = {"xnormal", "ynormal", "znormal"};

  for(int o=0; o<3; ++o){
    const auto& a = acc[o];
    const double invn = a.n > 0 ? 1.0 / (double)a.n : 0.0;
    const double alpha = a.at / std::max(a.tt, 1.0e-300);
    const double relErr = std::sqrt(std::max(a.rr, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));
    const double appliedOverTarget = std::sqrt(std::max(a.aa, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));

    std::printf(
        "MMSCORRFIT_ORIENT step=%04d label=%s group=k0k1 orient=%s nFaces=%d "
        "alpha=%.12e relErr=%.12e appliedNormOverTarget=%.12e "
        "targetFluxL2=%.12e appliedFluxL2=%.12e residualFluxL2=%.12e "
        "meanAbsTargetVel=%.12e meanAbsAppliedVel=%.12e meanAbsResidualVel=%.12e "
        "maxAbsTargetVel=%.12e maxAbsAppliedVel=%.12e maxAbsResidualVel=%.12e\n",
        step, label, name[o], a.n,
        alpha, relErr, appliedOverTarget,
        std::sqrt(std::max(a.tt, 0.0)),
        std::sqrt(std::max(a.aa, 0.0)),
        std::sqrt(std::max(a.rr, 0.0)),
        a.meanAbsTargetVel * invn,
        a.meanAbsAppliedVel * invn,
        a.meanAbsResidualVel * invn,
        a.maxAbsTargetVel,
        a.maxAbsAppliedVel,
        a.maxAbsResidualVel);
  }
...
static void simple_mms_trace_k0k1_face_error_orient_host(
    const char* label,
    int step,
    const Mesh& mesh,
    const std::vector<double>& diffFlux)
{
  const int Nguess = std::max(1, (int)std::llround(std::cbrt((double)mesh.nCells)));
  const double h = 1.0 / (double)Nguess;
  ...
  struct Acc {
    ...
    double maxv = 0.0;
    double mean = 0.0;
    double meanAbsOverArea = 0.0;
    double maxAbsOverArea = 0.0;
  };
  ...
  for(int f=0; f<mesh.nInternalFaces; ++f){
    ...
    const double e = diffFlux[f];
    const double A = std::max(mesh.Af[f], 1.0e-300);
    acc[o].n += 1;
    acc[o].l1 += std::fabs(e);
    acc[o].l2 += e*e;
    acc[o].maxv = std::max(acc[o].maxv, std::fabs(e));
    acc[o].mean += e;
    acc[o].meanAbsOverArea += std::fabs(e) / A;
    acc[o].maxAbsOverArea = std::max(acc[o].maxAbsOverArea, std::fabs(e) / A);
  }

  const char* name[3] = {"xnormal", "ynormal", "znormal"};

  for(int o=0; o<3; ++o){
    const double invn = acc[o].n > 0 ? 1.0 / (double)acc[o].n : 0.0;
    std::printf(
        "MMSFACEERR_ORIENT step=%04d label=%s group=k0k1 orient=%s nFaces=%d "
        "fluxL1=%.12e fluxL2=%.12e fluxMax=%.12e fluxMean=%.12e "
        "meanAbsVelErr=%.12e maxAbsVelErr=%.12e\n",
        step, label, name[o], acc[o].n,
        acc[o].l1, std::sqrt(std::max(acc[o].l2, 0.0)), acc[o].maxv, acc[o].mean * invn,
        acc[o].meanAbsOverArea * invn, acc[o].maxAbsOverArea);
  }
}



static std::vector<double> simple_mms_host_div_over_vol_from_flux(
    const Mesh& mesh,
    const std::vector<double>& phi)
{
  std::vector<double> div(mesh.nCells, 0.0);
  ...
  for(int c=0; c<mesh.nCells; ++c){
    div[c] /= std::max(mesh.vol[c], 1.0e-300);
  }
...
static void simple_mms_trace_facegroup_residual_host(
    const char* label,
    int step,
    const Mesh& mesh,
    const std::vector<double>& phi)
{
  ...
  std::vector<int> layer(mesh.nCells, 0);
  for(int c=0; c<mesh.nCells; ++c) layer[c] = layer_of_cell(c);
  ...
  auto add_to_cell = [&](std::vector<double>& r, int c, double val){
    r[c] += val / std::max(mesh.vol[c], 1.0e-300);
  };

  std::vector<double> r_all(mesh.nCells, 0.0);
  std::vector<double> r_bface(mesh.nCells, 0.0);
  std::vector<double> r_i00(mesh.nCells, 0.0);
  std::vector<double> r_i01(mesh.nCells, 0.0);
  std::vector<double> r_i0far(mesh.nCells, 0.0);
  std::vector<double> r_interior(mesh.nCells, 0.0);
  ...
  for(int f=0; f<mesh.nFaces; ++f){
    ...
    if(f < mesh.nInternalFaces){
      ...
      add_to_cell(r_all, P,  ph);
      add_to_cell(r_all, N, -ph);

      const int kP = layer[P];
      const int kN = layer[N];

      if(kP == 0 && kN == 0){
        add_to_cell(r_i00, P,  ph);
        add_to_cell(r_i00, N, -ph);
      } else if((kP == 0 && kN == 1) || (kP == 1 && kN == 0)){
        add_to_cell(r_i01, P,  ph);
        add_to_cell(r_i01, N, -ph);
      } else if(kP == 0 || kN == 0){
        add_to_cell(r_i0far, P,  ph);
        add_to_cell(r_i0far, N, -ph);
      } else {
        add_to_cell(r_interior, P,  ph);
        add_to_cell(r_interior, N, -ph);
      }
    } else {
      add_to_cell(r_all, P, ph);
      add_to_cell(r_bface, P, ph);
    }
  }

  auto summarize = [&](const std::vector<double>& r, int onlyLayer)->Acc {
    Acc a;
    double vol = 0.0;
    for(int c=0; c<mesh.nCells; ++c){
      if(onlyLayer >= 0 && layer[c] != onlyLayer) continue;
      const double V = mesh.vol[c];
      const double v = r[c];
      a.l1 += V * std::fabs(v);
      a.l2 += V * v * v;
      a.mean += V * v;
      a.maxv = std::max(a.maxv, std::fabs(v));
      vol += V;
      a.ncell += 1;
    }
    a.l2 = std::sqrt(std::max(a.l2, 0.0));
    a.mean = a.mean / std::max(vol, 1.0e-300);
    return a;
  };

  const Acc all0 = summarize(r_all, 0);
  const double denom = std::max(all0.l2 * all0.l2, 1.0e-300);

  auto print_group = [&](const char* name, const std::vector<double>& r){
    const Acc a0 = summarize(r, 0);
    std::printf(
        "MMSPEQN_FACEGROUP step=%04d label=%s layer=0 group=%s cells=%d "
        "L1=%.12e L2=%.12e max=%.12e mean=%.12e fracVsAll0L2=%.12e\n",
        step, label, name, a0.ncell, a0.l1, a0.l2, a0.maxv, a0.mean,
        (a0.l2*a0.l2)/denom);
  };

  std::printf(
      "MMSPEQN_FACEGROUP_HEADER step=%04d label=%s Nguess=%d layer0_all_L2=%.12e layer0_all_max=%.12e\n",
      step, label, Nguess, all0.l2, all0.maxv);

  print_group("boundary_faces", r_bface);
  print_group("internal_k0_k0", r_i00);
  print_group("internal_k0_k1", r_i01);
  print_group("internal_k0_far", r_i0far);
  print_group("non_k0_internal", r_interior);
}


static void simple_mms_trace_divergence_layers_host(
    const char* label,
    int step,
    const Mesh& mesh,
    const std::vector<double>& divOverVol)
{
  const int Nguess = std::max(1, (int)std::llround(std::cbrt((double)mesh.nCells)));
  const double h = 1.0 / (double)Nguess;
  const int maxLayer = std::max(0, Nguess/2);
  ...
  std::vector<int> count(maxLayer+1, 0);
  std::vector<double> vol(maxLayer+1, 0.0);
  std::vector<double> l1(maxLayer+1, 0.0);
  std::vector<double> l2(maxLayer+1, 0.0);
  std::vector<double> mx(maxLayer+1, 0.0);
  std::vector<double> mean(maxLayer+1, 0.0);
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    int k = (int)std::floor(d / h);
    if(k < 0) k = 0;
    if(k > maxLayer) k = maxLayer;

    const double V = mesh.vol[c];
    const double r = divOverVol[c];
    ...
    l2[k] += V * r * r;
    mean[k] += V * r;
    mx[k] = std::max(mx[k], std::fabs(r));

    totalVol += V;
    totalL1 += V * std::fabs(r);
    totalL2 += V * r * r;
    totalMax = std::max(totalMax, std::fabs(r));
  }

  std::printf("MMSPEQN_LAYER_HEADER step=%04d label=%s Nguess=%d totalL1=%.12e totalL2=%.12e totalMax=%.12e\n",
      step, label, Nguess, totalL1, std::sqrt(std::max(totalL2,0.0)), totalMax);

  const int printLayers = std::min(maxLayer, 8);
  for(int k=0; k<=printLayers; ++k){
    if(count[k] == 0) continue;
    std::printf(
        "MMSPEQN_LAYER step=%04d label=%s k=%d cells=%d volFrac=%.12e "
        "L1=%.12e L2=%.12e max=%.12e mean=%.12e fracL2=%.12e\n",
        step, label, k, count[k], vol[k]/std::max(totalVol,1.0e-300),
        l1[k], std::sqrt(std::max(l2[k],0.0)), mx[k],
        mean[k]/std::max(vol[k],1.0e-300),
        l2[k]/std::max(totalL2,1.0e-300));
  }
}


static void simple_mms_trace_pressure_norms_host(
    const char* label,
    int step,
    const Mesh& mesh,
    ...
    double rho, double mu, double U0, double P0)
{
  ...
  for(int c=0; c<mesh.nCells; ++c){
    const auto q = simple_mms_exact_value(mesh.cc[c][0], mesh.cc[c][1], mesh.cc[c][2], rho, mu, U0, P0);
    const double dp = pField[c] - q.p;
    mean += mesh.vol[c] * dp;
    vol += mesh.vol[c];
  }

  mean = (vol > 0.0) ? mean / vol : 0.0;

  double l1 = 0.0, l2 = 0.0, linf = 0.0;
  double e1 = 0.0, e2 = 0.0, einf = 0.0;

  for(int c=0; c<mesh.nCells; ++c){
    const auto q = simple_mms_exact_value(mesh.cc[c][0], mesh.cc[c][1], mesh.cc[c][2], rho, mu, U0, P0);
    const double dp = (pField[c] - q.p) - mean;
    const double pe = fabs(q.p);
    const double V = mesh.vol[c];

    l1 += V * fabs(dp);
    l2 += V * dp * dp;
    linf = std::max(linf, fabs(dp));

    e1 += V * pe;
    e2 += V * q.p * q.p;
    einf = std::max(einf, pe);
  }

  const double rel1 = l1 / std::max(e1, 1.0e-300);
  const double rel2 = sqrt(std::max(l2, 0.0)) / sqrt(std::max(e2, 1.0e-300));
  const double reli = linf / std::max(einf, 1.0e-300);

  std::printf(
      "MMSPEQN_PERR step=%04d label=%s PrelMF[L1,L2,Linf]=%.12e %.12e %.12e "
      "PabsMF[L1,L2,Linf]=%.12e %.12e %.12e pMeanOffset=%.12e\n",
      step, label, rel1, rel2, reli,
      l1, sqrt(std::max(l2, 0.0)), linf, mean);
}


static void simple_mms_print_hbya_pressure_lift_fit(
    const char* stage,
    int step,
    const Mesh& mesh,
    const std::vector<double>& hbyaU,
    const std::vector<double>& hbyaV,
    const std::vector<double>& hbyaW,
    const std::vector<double>& rAU,
    double rho, double mu, double U0, double P0)
{
  const double pi = 3.141592653589793238462643383279502884;
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    const double sz = std::sin(pi*z), cz = std::cos(pi*z);

    const double px = P0 * pi * cx * sy * sz;
    const double py = P0 * pi * sx * cy * sz;
    const double pz = P0 * pi * sx * sy * cz;

    const double gx = rAU[c] * px;
    const double gy = rAU[c] * py;
    const double gz = rAU[c] * pz;

    const double dx = hbyaU[c] - q.u;
    const double dy = hbyaV[c] - q.v;
    const double dz = hbyaW[c] - q.w;

    dot += V * (dx*gx + dy*gy + dz*gz);
    gg  += V * (gx*gx + gy*gy + gz*gz);
    dd  += V * (dx*dx + dy*dy + dz*dz);

    const double ep_x = dx - gx;
    const double ep_y = dy - gy;
    const double ep_z = dz - gz;

    const double em_x = dx + gx;
    const double em_y = dy + gy;
    const double em_z = dz + gz;

    errPlus  += V * (ep_x*ep_x + ep_y*ep_y + ep_z*ep_z);
    errMinus += V * (em_x*em_x + em_y*em_y + em_z*em_z);
  }

  const double alpha = dot / std::max(gg, 1.0e-300);
  ...
  for(int c=0; c<mesh.nCells; ++c){
    ...
    const double ex = dx - alpha*gx;
    const double ey = dy - alpha*gy;
    const double ez = dz - alpha*gz;

    errFit += V * (ex*ex + ey*ey + ez*ez);
  }

  const double deltaL2 = std::sqrt(std::max(dd, 0.0));
  const double gL2 = std::sqrt(std::max(gg, 0.0));

  std::printf(
      "MMSHBYA step=%04d stage=%s alphaFit=%.12e "
      "deltaL2=%.12e rAUgradP_L2=%.12e "
      "relErrFit=%.12e relErrAlphaPlus1=%.12e relErrAlphaMinus1=%.12e\n",
      step, stage, alpha,
      deltaL2, gL2,
      std::sqrt(std::max(errFit, 0.0)) / std::max(deltaL2, 1.0e-300),
      std::sqrt(std::max(errPlus, 0.0)) / std::max(deltaL2, 1.0e-300),
      std::sqrt(std::max(errMinus, 0.0)) / std::max(deltaL2, 1.0e-300));
}


static void simple_mms_print_stage_norms(
    const char* stage,
    int step,
    const Mesh& mesh,
    const std::vector<double>& u,
    ...
    double rho, double mu, double U0, double P0)
{
  ...
  const auto all   = simple_mms_compute_norm_pack(mesh, u, v, w, p, rho, mu, U0, P0, -1.0);
  const auto core1 = simple_mms_compute_norm_pack(mesh, u, v, w, p, rho, mu, U0, P0, 1.01*h);
  const auto core2 = simple_mms_compute_norm_pack(mesh, u, v, w, p, rho, mu, U0, P0, 2.01*h);

  auto pr = [&](const char* scope, const SimpleMmsNormPack& a){
    std::printf(
      "MMSDBG step=%04d stage=%s scope=%s n=%d vol=%.12e "
      "Urel[L1,L2,Linf]=%.12e %.12e %.12e "
      "PrelMF[L1,L2,Linf]=%.12e %.12e %.12e "
      "Uabs[L1,L2,Linf]=%.12e %.12e %.12e "
      "PabsMF[L1,L2,Linf]=%.12e %.12e %.12e "
      "pMeanOffset=%.12e\n",
      step, stage, scope, a.n, a.vol,
      a.uL1Rel, a.uL2Rel, a.uLinfRel,
      a.pL1Rel, a.pL2Rel, a.pLinfRel,
      a.uL1Abs, a.uL2Abs, a.uLinfAbs,
      a.pL1Abs, a.pL2Abs, a.pLinfAbs,
      a.pMeanOffset);
  };

  pr("all", core1.n > 0 ? all : all);
  if(core1.n > 0) pr("core1", core1);
  if(core2.n > 0) pr("core2", core2);
}




__global__ static void kernel_fill_simple_mms_pressure_exact_device(
    int nCells,
    ...
    double *pExact)
{
  ...
  if(c >= nCells) return;
  const auto q = simple_mms_exact_value(ccx[c], ccy[c], ccz[c], rho, mu, U0, P0);
  pExact[c] = q.p;
}

__global__ static void kernel_divide_by_volume_inplace(
    int nCells,
    double *x,
    const double *vol)
{
    ...
    HYPRE_Complex *rhs)
{
  ...
  // Intended diagnostic for uniform Cartesian blockMesh unit-cube cells.
  // h is inferred from cell volume.
  const double h = cbrt(fmax(vol[c], 1.0e-300));
  ...
  rhs[c] += (HYPRE_Complex)(favg * vol[c]);
}
    ...
    double *phi)
{
  ...
  // Exact-velocity MMS boundaries prescribe physical boundary flux directly.
  // The pressure equation must not pressure-correct those boundary faces.
  phi[f] = phiStar[f];
}



__global__ static void kernel_simple_mms_force_exact_phistar(
    int nFaces,
    int nInternalFaces,
    const int *owner,
    const int *neigh,
    ...
    double *phiStar)
{
  ...
  const double x = xfx[f];
  const double y = xfy[f];
  const double z = xfz[f];
  ...
  if(f < nInternalFaces){
    ...
    // Linear rAU interpolation is enough on uniform blockMesh and matches the
    // standard face-coefficient spirit. This is diagnostic only.
    const double rauf = 0.5 * (rAU[P] + rAU[N]);
  ...
  const double Sx = nfx[f] * Af[f];
  const double Sy = nfy[f] * Af[f];
  const double Sz = nfz[f] * Af[f];

  phiStar[f] = rho * (Uf*Sx + Vf*Sy + Wf*Sz);
}



__global__ static void kernel_simple_mms_fill_exact_u_face_flux(
    int nFaces,
    const double *xfx,
    const double *xfy,
    const double *xfz,
    ...
    double *phiU)
{
  ...
  const auto q = simple_mms_exact_value(xfx[f], xfy[f], xfz[f], rho, mu, U0, P0);

  const double Sx = nfx[f] * Af[f];
  const double Sy = nfy[f] * Af[f];
  const double Sz = nfz[f] * Af[f];

  phiU[f] = rho * (q.u*Sx + q.v*Sy + q.w*Sz);
}

__global__ static void kernel_simple_mms_build_discrete_exact_phistar(
    int nFaces,
    const double *phiU,
    const double *phiCorrectedByExactP,
    double *phiStar)
{
  const int f = blockIdx.x * blockDim.x + threadIdx.x;
  if(f >= nFaces) return;

  // If pressure correction gives:
  //   phiCorrectedByExactP = phiU + C(p_exact)
  // then choose:
  //   phiStar = phiU - C(p_exact) = 2*phiU - phiCorrectedByExactP
  // so that applying p_exact to phiStar returns phiU.
  phiStar[f] = 2.0*phiU[f] - phiCorrectedByExactP[f];
}



__global__ static void kernel_simple_mms_fill_exact_integrated_u_face_flux(
    int nFaces,
    int nCells,
    const double *xfx,
    const double *xfy,
    ...
    double *phiU)
{
  ...
  const int N = max(1, (int)llround(cbrt((double)nCells)));
  const double h = 1.0 / (double)N;

  const double x = xfx[f];
  const double y = xfy[f];

  const double ax = fabs(nfx[f]);
  const double ay = fabs(nfy[f]);
  const double az = fabs(nfz[f]);

  double flux = 0.0;

  if(ax >= ay && ax >= az && ax > 0.5){
    // x-normal face:
    // u = U0*pi*sin^2(pi*x)*sin(2*pi*y), independent of z.
    // Integral over face = sign(nx) * int_y int_z u dy dz.
    const double y0 = fmax(0.0, y - 0.5*h);
    const double y1 = fmin(1.0, y + 0.5*h);

    const double intSin2piY = (cos(2.0*pi*y0) - cos(2.0*pi*y1)) / (2.0*pi);
    const double val = U0 * pi * sin(pi*x)*sin(pi*x) * intSin2piY * h;

    flux = rho * copysign(1.0, nfx[f]) * val;
  }
  else if(ay >= ax && ay >= az && ay > 0.5){
    // y-normal face:
    // v = -U0*pi*sin(2*pi*x)*sin^2(pi*y), independent of z.
    // Integral over face = sign(ny) * int_x int_z v dx dz.
    const double x0 = fmax(0.0, x - 0.5*h);
    const double x1 = fmin(1.0, x + 0.5*h);

    const double intSin2piX = (cos(2.0*pi*x0) - cos(2.0*pi*x1)) / (2.0*pi);
    const double val = -U0 * pi * sin(pi*y)*sin(pi*y) * intSin2piX * h;

    flux = rho * copysign(1.0, nfy[f]) * val;
  }
  else {
    // z-normal face; this MMS has w = 0.
    flux = 0.0;
  }
...
__global__ static void kernel_simple_build_physical_velocity_flux(
    int nFaces,
    int nInternalFaces,
    const int *owner,
    const int *neigh,
    ...
    const int *bcVType,
    const int *bcWType,
    const double *uFaceBC,
    const double *vFaceBC,
    const double *wFaceBC,
    double rho,
    double *phiU)
{
  const int f = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if(f < nInternalFaces){
    ...
    // type 1 is fixed-value in the runtime BC machinery.
    if(patch >= 0){
      if(bcUType[patch] == 1) uf = uFaceBC[f];
      if(bcVType[patch] == 1) vf = vFaceBC[f];
      if(bcWType[patch] == 1) wf = wFaceBC[f];
    }
  }
  ...
  phiU[f] = rho * (uf*Sx + vf*Sy + wf*Sz);
}



__global__ static void kernel_subtract_flux_vectors_mmsdiag(
    int n,
    const double *a,
    const double *b,
    double *out)
...
__global__ static void kernel_simple_mms_fill_exact_midpoint_u_face_flux(
    int nFaces,
    ...
    double *phi)
{
  ...
  const auto q = simple_mms_exact_value(xfx[f], xfy[f], xfz[f], rho, 1.0, U0, 1.0);

  const double Sx = nfx[f] * Af[f];
  const double Sy = nfy[f] * Af[f];
  const double Sz = nfz[f] * Af[f];

  phi[f] = rho * (q.u*Sx + q.v*Sy + q.w*Sz);
}


__global__ static void kernel_simple_mms_accumulate_exact_subtri_u_flux(
    int nSubTris,
    const int *subFace,
    ...
    double *phi)
{
  ...
  const auto q = simple_mms_exact_value(subx[it], suby[it], subz[it], rho, mu, U0, P0);

  // One-point centroid quadrature over each fan triangle.  This is generic for
  // arbitrary polyMesh faces and uses the same oriented subtriangle area vectors
  // as the SIMPLE subtri continuity correction.  Do not infer h from nCells.
  const double flx = rho * (q.u*subsx[it] + q.v*subsy[it] + q.w*subsz[it]);
  hypreAtomicAdd(&phi[f], flx);
}
...
struct GPUSimpleScratch {
  ...
  double *d_phiPhys=nullptr; // preserved physical predictor flux for consistent phiStar
...
static void build_device_mesh(const Mesh &mesh, DeviceMesh &dm){
  ...
  // For each face polygon, use fan triangles around the vertex-average centre:
  //   triangle i = (faceVertexAverage, vertex_i, vertex_{i+1}).
  // The triangle area-vector orientation follows the parent OpenFOAM face orientation.
  ...
  for(int f=0; f<mesh.nFaces; ++f){
    ...
    for(int ii=0; ii<nv; ++ii){
      ...
      // Robustness: OpenFOAM face orientation should already make the fan
      // triangle area vectors agree with the parent Sf direction, but enforce
      // it explicitly for arbitrary polyMesh faces.  This protects both the
      // production subtriangle flux correction and MMS exact-flux diagnostics.
      ...
      h_subx.push_back((cx + pa[0] + pb[0]) / 3.0);
      h_suby.push_back((cy + pa[1] + pb[1]) / 3.0);
      h_subz.push_back((cz + pa[2] + pb[2]) / 3.0);
...
static void init_simple_scratch(GPUSimpleScratch &ss, const Mesh &mesh){
  ...
  device_alloc(ss.d_phiStar, mesh.nFaces); device_alloc(ss.d_phi, mesh.nFaces); device_alloc(ss.d_phiNonOrth, mesh.nFaces); device_alloc(ss.d_phiPhys, mesh.nFaces);
...
static void destroy_simple_scratch(GPUSimpleScratch &ss){
  ...
  device_free(ss.d_phiStar); device_free(ss.d_phi); device_free(ss.d_phiNonOrth); device_free(ss.d_phiPhys);
    ...
    double *phi)
{
  ...
  const double ncomp = (comp == 0 ? nfx[f] : (comp == 1 ? nfy[f] : nfz[f]));
  hypreAtomicAdd(&phi[f], rho * Af[f] * qSkew * ncomp);
}
    ...
    double *d_phi)
{
  ...
  kernel_simple_add_phi_velocity_skew_component<<<gridFaces, block>>>(
    ...
    double *d_phi)
{
  ...
  compute_lsq_gradient_gpu(gop, dmesh, dbcU, d_u, mom.d_gradQx, mom.d_gradQy, mom.d_gradQz);
  kernel_simple_subtri_velocity_flux_correction_component<<<gridSub, block>>>(
      ...
      dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
      dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
      d_u, mom.d_gradQx, mom.d_gradQy, mom.d_gradQz,
  ...
  kernel_simple_subtri_velocity_flux_correction_component<<<gridSub, block>>>(
      ...
      dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
      dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
      d_v, mom.d_gradQx, mom.d_gradQy, mom.d_gradQz,
  ...
  kernel_simple_subtri_velocity_flux_correction_component<<<gridSub, block>>>(
      ...
      dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
      dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
      d_w, mom.d_gradQx, mom.d_gradQy, mom.d_gradQz,
    ...
    double *d_phi)
{
  ...
  if(faceSkew){
    add_simple_parent_velocity_skew_to_phi(dmesh, mesh, mom, gop, dbcU, dbcV, dbcW,
                                           d_u, d_v, d_w, par.rho, d_phi);
  }
  if(subtriContinuity){
    add_simple_subtri_velocity_flux_correction(dmesh, mesh, mom, gop, dbcU, dbcV, dbcW,
                                               d_u, d_v, d_w, par.rho,
                                               faceSkew ? 1.0 : 0.0, d_phi);
  }
}
    ...
    double *d_phi)
{
  ...
  // If subtriContinuity is enabled, phiStar was already upgraded before the
  // pressure RHS, and final phi inherits that correction. Do not double-add it.
...
// PATCH_SIMPLE_PRESSURE_SUBTRI_NONORTH
//
// Optional pressure subtriangle quadrature correction for the explicit
// nonorthogonal pressure flux.
//
// Existing pressure correction is OF-like:
//
//   phi_p = coeff*(pN - pP) + phiNonOrth_face_center
//
// This patch keeps the implicit orthogonal matrix unchanged and only corrects
// the explicit nonorth term:
//
//   phiNonOrth += integral_subtri(rAU grad(p).dS) - face_center(rAU grad(p).Sf)
//
// That makes the explicit pressure-flux part use the same subtriangle face
// geometry idea already used by the velocity/continuity flux path.
//
// Enable with:
//   ANABASIS_SIMPLE_PRESSURE_SUBTRI=1
//
// Default is OFF, so production behavior is unchanged unless explicitly enabled.
__global__ static void kernel_simple_pressure_subtri_nonorth_correction(
    ...
    double *divNonOrth)
{
  ...
  const int f = subFace[it];
  if(f >= nInternalFaces) return;   // keep boundary behavior unchanged/safe
  ...
  // Parent face interpolation.
  double lamF = ((xfx[f]-ccx[P])*dx + (xfy[f]-ccy[P])*dy + (xfz[f]-ccz[P])*dz) / denom;
  lamF = fmin(1.0, fmax(0.0, lamF));

  const double rAUF = (1.0 - lamF)*rAU[P] + lamF*rAU[N];
  const double gFx = (1.0 - lamF)*gradx[P] + lamF*gradx[N];
  const double gFy = (1.0 - lamF)*grady[P] + lamF*grady[N];
  const double gFz = (1.0 - lamF)*gradz[P] + lamF*gradz[N];

  // Subtriangle quadrature-point interpolation.
  double lamQ = ((subx[it]-ccx[P])*dx + (suby[it]-ccy[P])*dy + (subz[it]-ccz[P])*dz) / denom;
  lamQ = fmin(1.0, fmax(0.0, lamQ));

  const double rAUQ = (1.0 - lamQ)*rAU[P] + lamQ*rAU[N];
  const double gQx = (1.0 - lamQ)*gradx[P] + lamQ*gradx[N];
  const double gQy = (1.0 - lamQ)*grady[P] + lamQ*grady[N];
  const double gQz = (1.0 - lamQ)*gradz[P] + lamQ*gradz[N];

  // Add only the subtri-minus-parent quadrature correction.
  // Summing over subtriangles gives:
  //   integral(rAU grad p . dS) - rAU_F grad_F p . Sf
  const double corr =
      rho * ((rAUQ*gQx - rAUF*gFx)*subsx[it]
           + (rAUQ*gQy - rAUF*gFy)*subsy[it]
           + (rAUQ*gQz - rAUF*gFz)*subsz[it]);

  atomicAdd(&phiNonOrth[f], corr);
  atomicAdd(&divNonOrth[P], corr);
  atomicAdd(&divNonOrth[N], -corr);
}

static void apply_simple_pressure_subtri_nonorth_correction(
    const Params &par,
    const DeviceMesh &dmesh,
    const Mesh &mesh,
    const double *d_rAU,
    const double *d_gradx,
    const double *d_grady,
    const double *d_gradz,
    double *d_phiNonOrth,
    double *d_divNonOrth)
{
  const int pressureSubtriEnv =
      simple_env_or_param_int("ANABASIS_SIMPLE_PRESSURE_SUBTRI", -1);
  const int pressureSubtri =
      (pressureSubtriEnv >= 0) ? pressureSubtriEnv : par.simplePressureSubtri;

  if(!pressureSubtri) return;
  if(dmesh.nSubTris <= 0) return;
  ...
  if(!printed){
    ...
    if(r == 0){
      std::printf("SIMPLE pressure subtri nonorth correction: enabled=1 nSubTris=%d caseOption=%d envOverride=%d\n",
                  dmesh.nSubTris, par.simplePressureSubtri, pressureSubtriEnv);
    }
    printed = 1;
  }

  const int block = 256;
  const int grid = (dmesh.nSubTris + block - 1) / block;

  kernel_simple_pressure_subtri_nonorth_correction<<<grid, block>>>(
      dmesh.nSubTris, mesh.nInternalFaces,
      dmesh.d_subFace,
      dmesh.d_subx, dmesh.d_suby, dmesh.d_subz,
      dmesh.d_subsx, dmesh.d_subsy, dmesh.d_subsz,
      dmesh.d_owner, dmesh.d_neigh,
    ...
    double *phiNonOrth, double *divNonOrth)
{
  ...
  if(f < nInternalFaces){
    ...
    // In SIMPLE MMS fixed-velocity-boundary pressure-equation mode, boundary
    // pressure coefficients are intentionally omitted from the pressure matrix
    // and the boundary flux is hard-enforced from phiStar after correction.
    // Therefore the explicit nonorthogonal boundary contribution must also be
    // omitted from the RHS. Keeping it on skew/poly boundary faces injects an
    // inconsistent source term and can blow up the second SIMPLE iteration.
    if(skipBoundaryNonOrth){
      phiNonOrth[f] = 0.0;
      return;
    }
...
static void update_pressure_matrix_from_rAU(const Mesh &mesh, const DeviceMesh &dm, GPULinearSystem &ps, const DeviceBC &bcP, const double *d_rAU, double rho, double pCoeffScale, int refCell, bool usePressureAnchor, bool skipPressureBoundaryCoeffs, bool doSetup, double &tsetup){
  int block=256;
  int gridVals=(ps.pat.nnz + block - 1)/block;
  int gridFaces=(mesh.nInternalFaces + block - 1)/block;
  int nBoundaryFaces = mesh.nFaces - mesh.nInternalFaces;
...
CUDA_CALL(cudaFree(0));
  ...
  if(par.simpleMmsEnable && par.velSolver == 3){
    if(rank==0) std::fprintf(stderr, "simpleMmsEnable currently supports velSolver bicgstab/mcgs paths, not fused mcgs. Set velSolver bicgstab for MMS verification.\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  if(rank==0){
    ...
    std::printf("SIMPLE consistent phiStar option: %d\n", par.simpleConsistentPhiStar);
    std::printf("SIMPLE MMS jump-lift phiStar option: %d\n", par.simpleMmsUseJumpLiftPhiStar);
    std::printf("SIMPLE MMS source quad8: %d\n", par.simpleMmsSourceQuad8);
    std::printf("SIMPLE MMS      : enable=%d initExact=%d pressureDirichlet=%d U0=%.8g P0=%.8g fixedVelBoundaryPEqn=%d\n",
  ...
  const bool usePressureAnchor = pressureReferenceNeeded || pressureEqnFixedVelocityBoundaryFlux;
  ...
  if(rank==0){
    ...
    std::printf("SIMPLE MMS force analytical phiStar: %s\n",
        (par.simpleMmsEnable && par.simpleMmsForceExactPhiStar) ? "yes" : "no");
    std::printf("SIMPLE MMS force DISCRETE analytical phiStar: %s\n",
        (par.simpleMmsEnable && par.simpleMmsForceDiscreteExactPhiStar) ? "yes" : "no");
    std::printf("SIMPLE MMS force exact INTEGRATED U-flux phiStar: %s\n",
        (par.simpleMmsEnable && par.simpleMmsForceExactIntegratedUFlux) ? "yes" : "no");
  }
  ...
  if(par.initMode == 1){
    ...
    if(pressureEqnFixedVelocityBoundaryFlux){
      // MMS fixed-velocity boundary flux hard enforcement.
      const int blockBFlux = 256;
      const int nBF = mesh.nFaces - mesh.nInternalFaces;
      const int gridBFlux = (nBF + blockBFlux - 1) / blockBFlux;
      kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockBFlux>>>(
          mesh.nFaces, mesh.nInternalFaces, ss.d_phiStar, ss.d_phi);
      CUDA_CHECK_LAST();
    }
  ...
  if(par.simpleMmsEnable && par.simpleMmsInitExact){
    ...
    CUDA_CALL(cudaMemcpy(ss.d_u, u.data(), mesh.nCells*sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_v, v.data(), mesh.nCells*sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_w, w.data(), mesh.nCells*sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_p, p.data(), mesh.nCells*sizeof(double), cudaMemcpyHostToDevice));

    if(rank == 0){
      std::printf("SIMPLE MMS exact fields forced to device immediately before SIMPLE loop.\n");
      simple_mms_print_stage_norms("pre_loop_forced_exact", 0, mesh, u, v, w, p,
          par.rho, mu,
          par.simpleMmsVelocityScale,
          par.simpleMmsPressureScale);
    }
  ...
  {
    if(!par.simpleMmsEnable) return;
    if(par.simpleMmsDebugEvery <= 0) return;
    if(par.simpleMmsDebugMaxStep > 0 && step > par.simpleMmsDebugMaxStep) return;
    if(!(step == 1 || (step % par.simpleMmsDebugEvery) == 0)) return;

    copy_device_to_vec(dU, u);
    copy_device_to_vec(dV, v);
    copy_device_to_vec(dW, w);
    copy_device_to_vec(dP, p);

    if(rank == 0){
      simple_mms_print_stage_norms(stage, step, mesh, u, v, w, p,
          par.rho, mu,
          par.simpleMmsVelocityScale,
          par.simpleMmsPressureScale);

      if(std::strcmp(stage, "after_HbyA_reconstruct_or_predictor") == 0){
        // Optimized SIMPLE keeps rAU device-resident. The host rAU vector is
        // intentionally cleared by extract_rAU_from_momentum_matrix(), so copy
        // mom.d_rAU only for this debug diagnostic.
        std::vector<double> rAUDebug(mesh.nCells, 0.0);
        copy_device_to_vec(mom.d_rAU, rAUDebug);

        simple_mms_print_hbya_pressure_lift_fit(stage, step, mesh,
            u, v, w, rAUDebug,
            par.rho, mu,
            par.simpleMmsVelocityScale,
            par.simpleMmsPressureScale);
  ...
  auto simple_mms_trace_pressure_equation = [&](
      const char* label,
      int step,
      const double* dPField,
      const double* dFlux)
  {
    if(!par.simpleMmsEnable || !par.simpleMmsTracePressureEqn) return;
    if(par.simpleMmsDebugMaxStep > 0 && step > par.simpleMmsDebugMaxStep) return;
    if(!(step == 1 || (par.simpleMmsDebugEvery > 0 && (step % par.simpleMmsDebugEvery) == 0))) return;

    const int block = 256;
    const int gridC = (mesh.nCells + block - 1) / block;
    const int gridF = (mesh.nFaces + block - 1) / block;

    // 1) divergence of supplied flux
    continuity_residual_gpu(dmesh, dFlux, ss.d_divNonOrth);
    const double massMax = maxabs_device(ss.d_divNonOrth, mesh.nCells, ss.d_reduce, ss.reduceSize);
    kernel_divide_by_volume_inplace<<<gridC, block>>>(mesh.nCells, ss.d_divNonOrth, dmesh.d_vol);
    CUDA_CHECK_LAST();
    const double divVolMax = maxabs_device(ss.d_divNonOrth, mesh.nCells, ss.d_reduce, ss.reduceSize);

    std::vector<double> hDivTrace(mesh.nCells, 0.0);
    copy_device_to_vec(ss.d_divNonOrth, hDivTrace);

    std::vector<double> hPhiTrace(mesh.nFaces, 0.0);
    copy_device_to_vec(dFlux, hPhiTrace);

    static std::vector<double> s_lastPostHbyaRawFlux;
    static int s_lastPostHbyaRawStep = -1;

    if(std::strcmp(label, "POST_HBYA_phiStar_raw") == 0){
      s_lastPostHbyaRawFlux = hPhiTrace;
      s_lastPostHbyaRawStep = step;
    }

    // 2) pressure error of supplied pressure field
    std::vector<double> hp(mesh.nCells, 0.0);
    copy_device_to_vec(dPField, hp);

    if(rank == 0){
      std::printf("MMSPEQN_FLUX step=%04d label=%s maxAbsDivFlux=%.12e maxAbsDivOverVol=%.12e\n",
          step, label, massMax, divVolMax);
      simple_mms_trace_divergence_layers_host(label, step, mesh, hDivTrace);
      simple_mms_trace_facegroup_residual_host(label, step, mesh, hPhiTrace);

      if(std::strcmp(label, "POST_HBYA_phiStar_raw") == 0){
        const int blockDiagRaw = 256;
        const int gridFaceDiagRaw = (mesh.nFaces + blockDiagRaw - 1) / blockDiagRaw;

        // Exact integrated velocity flux C.
        kernel_zero_double<<<gridFaceDiagRaw, blockDiagRaw>>>(ss.d_phi, mesh.nFaces);
        CUDA_CHECK_LAST();
        if(dmesh.nSubTris > 0){
          const int gridExactSubtri_1 = (dmesh.nSubTris + blockDiagRaw - 1) / blockDiagRaw;
          kernel_simple_mms_accumulate_exact_subtri_u_flux<<<gridExactSubtri_1, blockDiagRaw>>>(
              dmesh.nSubTris,
              dmesh.d_subFace,
              dmesh.d_subx, dmesh.d_suby, dmesh.d_subz,
              dmesh.d_subsx, dmesh.d_subsy, dmesh.d_subsz,
          ...
          kernel_simple_mms_fill_exact_midpoint_u_face_flux<<<gridFaceDiagRaw, blockDiagRaw>>>(
              mesh.nFaces,
              dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
              dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
              dmesh.d_Af,
        ...
        std::vector<double> hExactIntRaw(mesh.nFaces, 0.0);
        copy_device_to_vec(ss.d_phi, hExactIntRaw);

        std::vector<double> hRawMinusExact(mesh.nFaces, 0.0);
        for(int ff=0; ff<mesh.nFaces; ++ff){
          hRawMinusExact[ff] = hPhiTrace[ff] - hExactIntRaw[ff];
        }

        auto hRawMinusExactDiv = simple_mms_host_div_over_vol_from_flux(mesh, hRawMinusExact);

        simple_mms_trace_divergence_layers_host("POST_HBYA_RAW_MINUS_EXACT_INTEGRATED_U", step, mesh, hRawMinusExactDiv);
        simple_mms_trace_facegroup_residual_host("POST_HBYA_RAW_MINUS_EXACT_INTEGRATED_U", step, mesh, hRawMinusExact);
        simple_mms_trace_k0k1_face_error_orient_host("POST_HBYA_RAW_MINUS_EXACT_INTEGRATED_U", step, mesh, hRawMinusExact);
      }

      if(std::strcmp(label, "POST_HBYA_phi_after_exact_p") == 0){
        const int blockDiag = 256;
        const int gridFaceDiag = (mesh.nFaces + blockDiag - 1) / blockDiag;

        // ss.d_phi is used here as temporary exact integrated U flux.
        kernel_zero_double<<<gridFaceDiag, blockDiag>>>(ss.d_phi, mesh.nFaces);
        CUDA_CHECK_LAST();
        if(dmesh.nSubTris > 0){
          const int gridExactSubtri_2 = (dmesh.nSubTris + blockDiag - 1) / blockDiag;
          kernel_simple_mms_accumulate_exact_subtri_u_flux<<<gridExactSubtri_2, blockDiag>>>(
              dmesh.nSubTris,
              dmesh.d_subFace,
              dmesh.d_subx, dmesh.d_suby, dmesh.d_subz,
              dmesh.d_subsx, dmesh.d_subsy, dmesh.d_subsz,
          ...
          kernel_simple_mms_fill_exact_midpoint_u_face_flux<<<gridFaceDiag, blockDiag>>>(
              mesh.nFaces,
              dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
              dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
              dmesh.d_Af,
        ...
        // ss.d_phiNonOrth = corrected_flux - exact_integrated_U_flux
        kernel_subtract_flux_vectors_mmsdiag<<<gridFaceDiag, blockDiag>>>(
            mesh.nFaces, dFlux, ss.d_phi, ss.d_phiNonOrth);
        CUDA_CHECK_LAST();

        continuity_residual_gpu(dmesh, ss.d_phi, ss.d_divCorr);

        std::vector<double> hExactUDiv(mesh.nCells, 0.0);
        copy_device_to_vec(ss.d_divCorr, hExactUDiv);
        for(int cc=0; cc<mesh.nCells; ++cc){
          hExactUDiv[cc] /= std::max(mesh.vol[cc], 1.0e-300);
        }

        continuity_residual_gpu(dmesh, ss.d_phiNonOrth, ss.d_divCorr);

        std::vector<double> hDiffDiv(mesh.nCells, 0.0);
        std::vector<double> hDiffFlux(mesh.nFaces, 0.0);
        copy_device_to_vec(ss.d_divCorr, hDiffDiv);
        for(int cc=0; cc<mesh.nCells; ++cc){
          hDiffDiv[cc] /= std::max(mesh.vol[cc], 1.0e-300);
        }
        copy_device_to_vec(ss.d_phiNonOrth, hDiffFlux);

        simple_mms_trace_divergence_layers_host("EXACT_INTEGRATED_U_FLUX", step, mesh, hExactUDiv);
        simple_mms_trace_divergence_layers_host("POST_HBYA_exact_p_MINUS_EXACT_INTEGRATED_U", step, mesh, hDiffDiv);
        simple_mms_trace_facegroup_residual_host("POST_HBYA_exact_p_MINUS_EXACT_INTEGRATED_U", step, mesh, hDiffFlux);
        simple_mms_trace_k0k1_face_error_orient_host("POST_HBYA_exact_p_MINUS_EXACT_INTEGRATED_U", step, mesh, hDiffFlux);

        // Exact midpoint flux B.
        kernel_simple_mms_fill_exact_midpoint_u_face_flux<<<gridFaceDiag, blockDiag>>>(
            mesh.nFaces,
            dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
            dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
        ...
        // HOSTSAFE comparison: preserve original A=dFlux from hPhiTrace.
        // C = exact integrated flux is in ss.d_phi.
        // B = exact midpoint flux is in ss.d_phiPhys.
        std::vector<double> hExactIntFluxHost(mesh.nFaces, 0.0);
        std::vector<double> hExactMidFluxHost(mesh.nFaces, 0.0);
        copy_device_to_vec(ss.d_phi, hExactIntFluxHost);
        copy_device_to_vec(ss.d_phiPhys, hExactMidFluxHost);

        std::vector<double> hAminusIntHost(mesh.nFaces, 0.0);
        std::vector<double> hAminusMidHost(mesh.nFaces, 0.0);
        std::vector<double> hMidMinusIntHost(mesh.nFaces, 0.0);

        for(int ff=0; ff<mesh.nFaces; ++ff){
          hAminusIntHost[ff] = hPhiTrace[ff] - hExactIntFluxHost[ff];
          hAminusMidHost[ff] = hPhiTrace[ff] - hExactMidFluxHost[ff];
          hMidMinusIntHost[ff] = hExactMidFluxHost[ff] - hExactIntFluxHost[ff];
        }

        auto hAminusIntDivHost = simple_mms_host_div_over_vol_from_flux(mesh, hAminusIntHost);
        auto hAminusMidDivHost = simple_mms_host_div_over_vol_from_flux(mesh, hAminusMidHost);
        auto hMidMinusIntDivHost = simple_mms_host_div_over_vol_from_flux(mesh, hMidMinusIntHost);

        simple_mms_trace_divergence_layers_host("HOSTSAFE_A_MINUS_INTEGRATED_U", step, mesh, hAminusIntDivHost);
        simple_mms_trace_facegroup_residual_host("HOSTSAFE_A_MINUS_INTEGRATED_U", step, mesh, hAminusIntHost);
        simple_mms_trace_flux_divergence_topo_host(
            "HOSTSAFE_A_MINUS_INTEGRATED_U", step, mesh, hAminusIntHost);
        simple_mms_trace_k0k1_face_error_orient_host("HOSTSAFE_A_MINUS_INTEGRATED_U", step, mesh, hAminusIntHost);

        if(s_lastPostHbyaRawStep == step && s_lastPostHbyaRawFlux.size() == hPhiTrace.size()){
          std::printf("MMSCORRFIT_TOPO_CALLSITE_MARKER step=%04d label=POST_HBYA_RAW_TO_EXACTP rawSize=%zu phiSize=%zu exactSize=%zu\\n",
              step,
              (size_t)s_lastPostHbyaRawFlux.size(),
              (size_t)hPhiTrace.size(),
              (size_t)hExactIntFluxHost.size());

          simple_mms_trace_k0k1_pressure_correction_fit_host(
              "POST_HBYA_RAW_TO_EXACTP", step, mesh,
              s_lastPostHbyaRawFlux, hPhiTrace, hExactIntFluxHost);

          simple_mms_trace_pressure_correction_fit_topo_groups_host(
              "POST_HBYA_RAW_TO_EXACTP", step, mesh,
              s_lastPostHbyaRawFlux, hPhiTrace, hExactIntFluxHost);
      ...
      simple_mms_trace_pressure_norms_host(label, step, mesh, hp,
          par.rho, mu,
          par.simpleMmsVelocityScale,
          par.simpleMmsPressureScale);
    }
  ...
  for(int step=1; step<=maxSteps; ++step){
    ...
    simple_mms_debug_stage("iter_start_old", step, ss.d_uOld, ss.d_vOld, ss.d_wOld, ss.d_pOld);
    ...
    if(par.velSolver == 3){
      ...
      solve_scalar_component_with_nonorth(ss.d_uOld, ss.d_uOld, ss.d_vOld, ss.d_wOld, ss.d_gradx, dbcU, ss.d_u, lastItsU, lastRelU, corrUsedU, 0, true,  true,  prof[PH_UGRAD], prof[PH_UASM], prof[PH_USOLVE]);
      solve_scalar_component_with_nonorth(ss.d_vOld, ss.d_uOld, ss.d_vOld, ss.d_wOld, ss.d_grady, dbcV, ss.d_v, lastItsV, lastRelV, corrUsedV, 1, false, false, prof[PH_VGRAD], prof[PH_VASM], prof[PH_VSOLVE]);
      solve_scalar_component_with_nonorth(ss.d_wOld, ss.d_uOld, ss.d_vOld, ss.d_wOld, ss.d_gradz, dbcW, ss.d_w, lastItsW, lastRelW, corrUsedW, 2, false, false, prof[PH_WGRAD], prof[PH_WASM], prof[PH_WSOLVE]);
    }

    if(par.simpleMmsEnable && par.simpleMmsForceExactPhiStar){
      const int blockMmsPhi = 256;
      const int gridMmsPhi = (mesh.nFaces + blockMmsPhi - 1) / blockMmsPhi;
      kernel_simple_mms_force_exact_phistar<<<gridMmsPhi, blockMmsPhi>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
      ...
      if(rank == 0 && (step == 1 || (par.simpleMmsDebugEvery > 0 && step % par.simpleMmsDebugEvery == 0))){
        std::printf("SIMPLE MMS forcing analytical phiStar at step %d\n", step);
      }
    }

    if(par.simpleMmsEnable && par.simpleMmsForceDiscreteExactPhiStar){
      const int blockMmsPhiD = 256;
      const int gridMmsPhiD = (mesh.nFaces + blockMmsPhiD - 1) / blockMmsPhiD;
      const int gridMmsCellD = (mesh.nCells + blockMmsPhiD - 1) / blockMmsPhiD;

      // ss.d_phi is used temporarily as phiUExact.
      kernel_simple_mms_fill_exact_u_face_flux<<<gridMmsPhiD, blockMmsPhiD>>>(
          mesh.nFaces,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
          dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
          dmesh.d_Af,
      ...
      // Fill exact pressure at cells.
      kernel_fill_simple_mms_pressure_exact_device<<<gridMmsCellD, blockMmsPhiD>>>(
          mesh.nCells,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          par.rho, mu,
          par.simpleMmsVelocityScale,
      ...
      // Apply the actual pressure-flux kernel to phiUExact using p_exact.
      // Output goes to ss.d_phiNonOrth.
      kernel_correct_face_fluxes_simple_nonorth<<<gridMmsPhiD, blockMmsPhiD>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
          ...
          dmesh.d_Af, mom.d_rAU, par.rho,
          dbcP.d_type, dbcP.d_faceValue,
          ss.d_phi, ss.d_pCorrDelta, ss.d_phiStar,
          0.0, par.pCoeffScale, par.pFluxMode,
          pressureEqnFixedVelocityBoundaryFlux ? 1 : 0,
          ss.d_phiNonOrth);
      CUDA_CHECK_LAST();
      ...
      if(pressureEqnFixedVelocityBoundaryFlux){
        const int nBF = mesh.nFaces - mesh.nInternalFaces;
        const int gridBFlux = (nBF + blockMmsPhiD - 1) / blockMmsPhiD;
        kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockMmsPhiD>>>(
            mesh.nFaces, mesh.nInternalFaces, ss.d_phiPhys, ss.d_phiNonOrth);
        CUDA_CHECK_LAST();
      }

      // Build phiStar = 2*phiUExact - corrected(phiUExact,p_exact).
      kernel_simple_mms_build_discrete_exact_phistar<<<gridMmsPhiD, blockMmsPhiD>>>(
          mesh.nFaces, ss.d_phiPhys, ss.d_phiNonOrth, ss.d_phiStar);
      CUDA_CHECK_LAST();

      if(rank == 0 && (step == 1 || (par.simpleMmsDebugEvery > 0 && step % par.simpleMmsDebugEvery == 0))){
        std::printf("SIMPLE MMS forcing DISCRETE analytical phiStar at step %d\n", step);
      }
    }

    if(par.simpleMmsEnable && par.simpleMmsForceExactIntegratedUFlux){
      const int blockMmsInt = 256;
      const int gridMmsFaceInt = (mesh.nFaces + blockMmsInt - 1) / blockMmsInt;
      const int gridMmsCellInt = (mesh.nCells + blockMmsInt - 1) / blockMmsInt;

      // ss.d_phi is used temporarily as exact integrated physical U flux.
      kernel_zero_double<<<gridMmsFaceInt, blockMmsInt>>>(ss.d_phi, mesh.nFaces);
        CUDA_CHECK_LAST();
        if(dmesh.nSubTris > 0){
          const int gridExactSubtri_3 = (dmesh.nSubTris + blockMmsInt - 1) / blockMmsInt;
          kernel_simple_mms_accumulate_exact_subtri_u_flux<<<gridExactSubtri_3, blockMmsInt>>>(
              dmesh.nSubTris,
              dmesh.d_subFace,
              dmesh.d_subx, dmesh.d_suby, dmesh.d_subz,
              dmesh.d_subsx, dmesh.d_subsy, dmesh.d_subsz,
          ...
          kernel_simple_mms_fill_exact_midpoint_u_face_flux<<<gridMmsFaceInt, blockMmsInt>>>(
              mesh.nFaces,
              dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
              dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
              dmesh.d_Af,
      ...
      // Exact pressure at cell centres.
      kernel_fill_simple_mms_pressure_exact_device<<<gridMmsCellInt, blockMmsInt>>>(
          mesh.nCells,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          par.rho, mu,
          par.simpleMmsVelocityScale,
      ...
      // Apply actual pressure-flux operator to phiUExactIntegrated using p_exact.
      kernel_correct_face_fluxes_simple_nonorth<<<gridMmsFaceInt, blockMmsInt>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
      ...
      if(pressureEqnFixedVelocityBoundaryFlux){
        const int nBF = mesh.nFaces - mesh.nInternalFaces;
        const int gridBFlux = (nBF + blockMmsInt - 1) / blockMmsInt;
        kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockMmsInt>>>(
            mesh.nFaces, mesh.nInternalFaces, ss.d_phiPhys, ss.d_phiNonOrth);
        CUDA_CHECK_LAST();
      }

      // Build phiStar = 2*phiUExactIntegrated - corrected(phiUExactIntegrated,p_exact).
      kernel_simple_mms_build_discrete_exact_phistar<<<gridMmsFaceInt, blockMmsInt>>>(
          mesh.nFaces, ss.d_phiPhys, ss.d_phiNonOrth, ss.d_phiStar);
      CUDA_CHECK_LAST();

      if(rank == 0 && (step == 1 || (par.simpleMmsDebugEvery > 0 && step % par.simpleMmsDebugEvery == 0))){
        std::printf("SIMPLE MMS forcing exact INTEGRATED U-flux discrete phiStar at step %d\n", step);
      }
    }

    if(par.simpleMmsEnable && par.simpleMmsTracePressureEqn){
      const int blockTrace = 256;
      const int gridTraceC = (mesh.nCells + blockTrace - 1) / blockTrace;
      const int gridTraceF = (mesh.nFaces + blockTrace - 1) / blockTrace;

      // exact pressure into ss.d_pCorrDelta
      kernel_fill_simple_mms_pressure_exact_device<<<gridTraceC, blockTrace>>>(
          mesh.nCells,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          par.rho, mu,
          par.simpleMmsVelocityScale,
      ...
      // trace raw phiStar
      simple_mms_trace_pressure_equation("phiStar_raw", step, ss.d_pCorrDelta, ss.d_phiStar);

      // corrected flux with exact pressure, output temp ss.d_phiNonOrth
      kernel_correct_face_fluxes_simple_nonorth<<<gridTraceF, blockTrace>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
          ...
          dmesh.d_Af, mom.d_rAU, par.rho,
          dbcP.d_type, dbcP.d_faceValue,
          ss.d_phiStar, ss.d_pCorrDelta, ss.d_phiNonOrth,
          0.0, par.pCoeffScale, par.pFluxMode,
          pressureEqnFixedVelocityBoundaryFlux ? 1 : 0,
          ss.d_phiNonOrth);
      CUDA_CHECK_LAST();
      ...
      if(pressureEqnFixedVelocityBoundaryFlux){
        const int nBF = mesh.nFaces - mesh.nInternalFaces;
        const int gridBFlux = (nBF + blockTrace - 1) / blockTrace;
        kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockTrace>>>(
            mesh.nFaces, mesh.nInternalFaces, ss.d_phiStar, ss.d_phiNonOrth);
        CUDA_CHECK_LAST();
      }

      simple_mms_trace_pressure_equation("phi_after_exact_p", step, ss.d_pCorrDelta, ss.d_phiNonOrth);
    }

    PhaseMark pm_psetup = profile_begin();
    simple_mms_debug_stage("after_momentum_predictor_Ustar", step, ss.d_u, ss.d_v, ss.d_w, ss.d_p);
    if(par.simpleConsistentPhiStar){
      const int blockSavePhiPhys = 256;
      const int gridSavePhiPhys = (mesh.nFaces + blockSavePhiPhys - 1) / blockSavePhiPhys;

      // ss.d_u/v/w are Ustar here: momentum predictor output before HbyA reconstruction.
      kernel_simple_build_physical_velocity_flux<<<gridSavePhiPhys, blockSavePhiPhys>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz, dmesh.d_Af,
          ss.d_u, ss.d_v, ss.d_w,
          dbcU.d_type, dbcV.d_type, dbcW.d_type,
          dbcU.d_faceValue, dbcV.d_faceValue, dbcW.d_faceValue,
          par.rho,
          ss.d_phiPhys);
      CUDA_CHECK_LAST();

      if(rank == 0 && step == 1){
        std::printf("SIMPLE consistent phiStar: saved Ustar physical flux immediately after momentum predictor into d_phiPhys.\n");
      }
    }
    ...
    update_pressure_matrix_from_rAU(mesh, dmesh, pressureSys, dbcP, mom.d_rAU, par.rho, par.pCoeffScale,
                                    refCell, usePressureAnchor, pressureEqnFixedVelocityBoundaryFlux,
    ...
    if(false && par.simpleConsistentPhiStar){
      const int blockConsPhiSaveU = 256;
      const int gridConsPhiSaveU = (mesh.nFaces + blockConsPhiSaveU - 1) / blockConsPhiSaveU;

      // At this point ss.d_u/v/w are still the momentum-predictor Ustar fields.
      // Save their physical flux before adding rAU*grad(pOld) to form HbyA.
      kernel_simple_build_physical_velocity_flux<<<gridConsPhiSaveU, blockConsPhiSaveU>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz, dmesh.d_Af,
          ss.d_u, ss.d_v, ss.d_w,
      ...
      if(rank == 0 && step == 1){
        std::printf("SIMPLE consistent phiStar: saved physical predictor flux BEFORE HbyA lift into d_phiPhys.\n");
      }
    }

    if(false && par.simpleConsistentPhiStar){
      const int blockConsPhiSaveReal = 256;
      const int gridConsPhiSaveReal = (mesh.nFaces + blockConsPhiSaveReal - 1) / blockConsPhiSaveReal;

      // Save true momentum-predictor Ustar flux before adding rAU*grad(pOld).
      kernel_simple_build_physical_velocity_flux<<<gridConsPhiSaveReal, blockConsPhiSaveReal>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz, dmesh.d_Af,
          ss.d_u, ss.d_v, ss.d_w,
      ...
      if(rank == 0 && step == 1){
        std::printf("SIMPLE consistent phiStar: saved physical predictor flux REALLY BEFORE HbyA lift into d_phiPhys.\n");
      }
    }
    ...
    if(false && par.simpleConsistentPhiStar){
      const int blockConsPhiPre = 256;
      const int gridConsPhiPre = (mesh.nFaces + blockConsPhiPre - 1) / blockConsPhiPre;

      // At this point ss.d_u/v/w are the momentum-predictor fields.
      // Save their physical flux in ss.d_phi; HbyA may overwrite ss.d_u/v/w.
      kernel_simple_build_physical_velocity_flux<<<gridConsPhiPre, blockConsPhiPre>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz, dmesh.d_Af,
          ss.d_u, ss.d_v, ss.d_w,
      ...
      if(rank == 0 && step == 1){
        std::printf("SIMPLE consistent phiStar: saved physical predictor flux before HbyA into d_phiPhys.\n");
      }
    }

    PhaseMark pm_phi = profile_begin();
    simple_mms_debug_stage("after_HbyA_reconstruct_or_predictor", step, ss.d_u, ss.d_v, ss.d_w, ss.d_p);

    if(false && par.simpleConsistentPhiStar){
      const int blockConsPhiAfterH = 256;
      const int gridConsPhiAfterH = (mesh.nFaces + blockConsPhiAfterH - 1) / blockConsPhiAfterH;

      // ss.d_phiPhys contains physical predictor flux saved before HbyA.
      // Rebuild phiStar immediately after the actual HbyA/Rhie-Chow predictor
      // has finished, but before the pressure equation RHS is assembled.
      kernel_correct_face_fluxes_simple_nonorth<<<gridConsPhiAfterH, blockConsPhiAfterH>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
          ...
          dmesh.d_Af, mom.d_rAU, par.rho,
          dbcP.d_type, dbcP.d_faceValue,
          ss.d_phiPhys, ss.d_pOld, ss.d_phiStar,
          par.pNonOrthScale, par.pCoeffScale, par.pFluxMode,
          pressureEqnFixedVelocityBoundaryFlux ? 1 : 0,
          ss.d_phiNonOrth);
      CUDA_CHECK_LAST();
      ...
      if(pressureEqnFixedVelocityBoundaryFlux){
        const int nBF = mesh.nFaces - mesh.nInternalFaces;
        const int gridBFlux = (nBF + blockConsPhiAfterH - 1) / blockConsPhiAfterH;
        kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockConsPhiAfterH>>>(
            mesh.nFaces, mesh.nInternalFaces, ss.d_phiPhys, ss.d_phiNonOrth);
        CUDA_CHECK_LAST();
      }

      kernel_simple_mms_build_discrete_exact_phistar<<<gridConsPhiAfterH, blockConsPhiAfterH>>>(
          mesh.nFaces, ss.d_phiPhys, ss.d_phiNonOrth, ss.d_phiStar);
      CUDA_CHECK_LAST();

      if(rank == 0 && step == 1){
        std::printf("SIMPLE consistent phiStar: rebuilt immediately AFTER HbyA diagnostic.\n");
      }
    }
    ...
    if(par.simpleConsistentPhiStar){
      const int blockConsPhi = 256;
      const int gridConsPhi = (mesh.nFaces + blockConsPhi - 1) / blockConsPhi;

      // Rebuild phiStar after the normal Rhie-Chow predictor flux has been built,
      // but before pressure RHS/continuity is assembled.
      // ss.d_phiPhys contains physical predictor flux saved before HbyA lift.
      // Apply the exact same pressure-flux operator used later by the pressure
      // equation, but using old pressure pOld. Output temp: ss.d_phiNonOrth.
      kernel_correct_face_fluxes_simple_nonorth<<<gridConsPhi, blockConsPhi>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
      ...
      if(pressureEqnFixedVelocityBoundaryFlux){
        const int nBF = mesh.nFaces - mesh.nInternalFaces;
        const int gridBFlux = (nBF + blockConsPhi - 1) / blockConsPhi;
        kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockConsPhi>>>(
            mesh.nFaces, mesh.nInternalFaces, ss.d_phiPhys, ss.d_phiNonOrth);
        CUDA_CHECK_LAST();
      }

      // Build consistent predictor:
      //   phiStar = phiU - C(pOld) = 2*phiU - corrected(phiU,pOld)
      kernel_simple_mms_build_discrete_exact_phistar<<<gridConsPhi, blockConsPhi>>>(
          mesh.nFaces, ss.d_phiPhys, ss.d_phiNonOrth, ss.d_phiStar);
      CUDA_CHECK_LAST();

      // Pressure RHS is assembled from ss.d_phi, not ss.d_phiStar.
      // Keep both arrays consistent after replacing the predictor flux.
      CUDA_CALL(cudaMemcpy(ss.d_phi, ss.d_phiStar, mesh.nFaces*sizeof(double), cudaMemcpyDeviceToDevice));

      if(rank == 0 && step == 1){
        std::printf("SIMPLE consistent phiStar: rebuilt phiStar AFTER normal predictor phi and BEFORE pressure RHS.\n");
      }
    }


    // POST-HBYA MMS force/trace block.
    // Important: this is after the actual SIMPLE HbyA/phiStar construction.
    // Earlier diagnostic blocks before pm_psetup are too early and get overwritten
    // by kernel_build_rhiechow_predicted_flux_stokes_3d().
    if(par.simpleMmsEnable){
      const int blockPostMms = 256;
      const int gridPostFace = (mesh.nFaces + blockPostMms - 1) / blockPostMms;
      const int gridPostCell = (mesh.nCells + blockPostMms - 1) / blockPostMms;

      if(par.simpleMmsForceExactPhiStar){
        kernel_simple_mms_force_exact_phistar<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces, mesh.nInternalFaces,
            dmesh.d_owner, dmesh.d_neigh,
            dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
            dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
        ...
        if(rank == 0 && (step == 1 || (par.simpleMmsDebugEvery > 0 && step % par.simpleMmsDebugEvery == 0))){
          std::printf("SIMPLE MMS POST-HBYA forcing analytical phiStar at step %d\n", step);
        }
      }

      if(par.simpleMmsForceDiscreteExactPhiStar){
        // ss.d_phi is temporary phiUExact from face-centre analytical U.
        kernel_simple_mms_fill_exact_u_face_flux<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces,
            dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
            dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
            dmesh.d_Af,
        ...
        kernel_fill_simple_mms_pressure_exact_device<<<gridPostCell, blockPostMms>>>(
            mesh.nCells,
            dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
            par.rho, mu,
            par.simpleMmsVelocityScale,
        ...
        kernel_correct_face_fluxes_simple_nonorth<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces, mesh.nInternalFaces,
            dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
            dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
            dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
        ...
        if(pressureEqnFixedVelocityBoundaryFlux){
          const int nBF = mesh.nFaces - mesh.nInternalFaces;
          const int gridBFlux = (nBF + blockPostMms - 1) / blockPostMms;
          kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockPostMms>>>(
              mesh.nFaces, mesh.nInternalFaces, ss.d_phiPhys, ss.d_phiNonOrth);
          CUDA_CHECK_LAST();
        }

        kernel_simple_mms_build_discrete_exact_phistar<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces, ss.d_phiPhys, ss.d_phiNonOrth, ss.d_phiStar);
        CUDA_CHECK_LAST();
        ...
        if(rank == 0 && (step == 1 || (par.simpleMmsDebugEvery > 0 && step % par.simpleMmsDebugEvery == 0))){
          std::printf("SIMPLE MMS POST-HBYA forcing DISCRETE analytical phiStar at step %d\n", step);
        }
      }

      if(par.simpleMmsForceExactIntegratedUFlux){
        // SIMPLE_MMS_FORCE_COMPATIBLE_PHI_STAR_POSTHBYA
        //
        // Correct control for MMS pressure-operator testing:
        //
        //   exactU     = exact integrated velocity flux
        //   corrected  = exactU - C(exactPressure)
        //   phiStar    = 2*exactU - corrected = exactU + C(exactPressure)
        //
        // Then the later pressure correction gives:
        //
        //   phiStar - C(exactPressure) = exactU
        //
        // The old branch used only the orthogonal correction and sometimes used
        // ss.d_phiPhys instead of the exact integrated flux ss.d_phi.

        // ss.d_phi = exact integrated physical U flux.
        kernel_simple_mms_fill_exact_integrated_u_face_flux<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces, mesh.nCells,
            dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
            dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
            par.rho,
            par.simpleMmsVelocityScale,
        ...
        // ss.d_pCorrDelta = exact pressure at cell centres.
        kernel_fill_simple_mms_pressure_exact_device<<<gridPostCell, blockPostMms>>>(
            mesh.nCells,
            dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
            par.rho, mu,
        ...
        // Build exact-p LSQ gradient and exact-p nonorthogonal pressure flux.
        compute_lsq_gradient_gpu(gop, dmesh, dbcP, ss.d_pCorrDelta,
                                 ss.d_gradx, ss.d_grady, ss.d_gradz);

        kernel_zero_double<<<gridPostCell, blockPostMms>>>(ss.d_divNonOrth, mesh.nCells);
        CUDA_CHECK_LAST();

        kernel_pressure_nonorth_flux_and_divergence<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces, mesh.nInternalFaces,
            dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
            dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
            dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
        ...
        CUDA_CHECK_LAST();
        apply_simple_pressure_subtri_nonorth_correction(
            par, dmesh, mesh, mom.d_rAU,
            ss.d_gradx, ss.d_grady, ss.d_gradz,
            ss.d_phiNonOrth, ss.d_divNonOrth);

        // ss.d_phiPhys = corrected exact flux = exactU - C(exactPressure).
        kernel_correct_face_fluxes_simple_nonorth<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces, mesh.nInternalFaces,
            dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
            dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
            ...
            dmesh.d_Af, mom.d_rAU, par.rho,
            dbcP.d_type, dbcP.d_faceValue,
            ss.d_phi, ss.d_pCorrDelta, ss.d_phiNonOrth,
            par.pNonOrthScale, par.pCoeffScale, par.pFluxMode,
            pressureEqnFixedVelocityBoundaryFlux ? 1 : 0,
            ss.d_phiPhys);
        CUDA_CHECK_LAST();
        ...
        if(pressureEqnFixedVelocityBoundaryFlux){
          const int nBF = mesh.nFaces - mesh.nInternalFaces;
          const int gridBFlux = (nBF + blockPostMms - 1) / blockPostMms;
          // Enforce corrected boundary flux from exactU, not from stale phiPhys.
          kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockPostMms>>>(
              mesh.nFaces, mesh.nInternalFaces, ss.d_phi, ss.d_phiPhys);
          CUDA_CHECK_LAST();
        }

        // phiStar = 2*exactU - correctedExactU = exactU + C(exactPressure).
        kernel_simple_mms_build_discrete_exact_phistar<<<gridPostFace, blockPostMms>>>(
            mesh.nFaces, ss.d_phi, ss.d_phiPhys, ss.d_phiStar);
        CUDA_CHECK_LAST();

        CUDA_CALL(cudaMemcpy(ss.d_phi, ss.d_phiStar,
                             mesh.nFaces*sizeof(double),
                             cudaMemcpyDeviceToDevice));

        if(rank == 0 && (step == 1 || (par.simpleMmsDebugEvery > 0 && step % par.simpleMmsDebugEvery == 0))){
          std::printf("SIMPLE_MMS_FORCE_COMPATIBLE_PHI_STAR_POSTHBYA step=%04d enabled=1 fullNonOrth=%.6g fixedBoundaryFlux=%d\\n",
              step, par.pNonOrthScale, pressureEqnFixedVelocityBoundaryFlux ? 1 : 0);
        }
      }

      if(par.simpleMmsTracePressureEqn){
        kernel_fill_simple_mms_pressure_exact_device<<<gridPostCell, blockPostMms>>>(
            mesh.nCells,
            dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
            par.rho, mu,
        ...
        simple_mms_trace_pressure_equation("POST_HBYA_phiStar_raw", step, ss.d_pCorrDelta, ss.d_phiStar);

        // PATCH_POST_HBYA_EXACTP_FULL_NONORTH_TRACE:
        // The old diagnostic corrected phiStar with exact p but used nonOrthScale=0.
        // That only tests the orthogonal jump part.  For polyMesh MMS with
        // pNonOrthScale=1, compute the exact-p nonorthogonal flux and include it,
        // matching the production pressure-correction operator.
        compute_lsq_gradient_gpu(gop, dmesh, dbcP, ss.d_pCorrDelta,
                                 ss.d_gradx, ss.d_grady, ss.d_gradz);
        kernel_zero_double<<<gridPostCell, blockPostMms>>>(ss.d_divNonOrth, mesh.nCells);
        CUDA_CHECK_LAST();
        ...
        simple_mms_trace_pressure_equation("POST_HBYA_phi_after_exact_p", step, ss.d_pCorrDelta, ss.d_phiNonOrth);
      }
    }
    ...
    for(int pcorr=1; pcorr<=nPressureSolves; ++pcorr){
      ...
      CUDA_CHECK_LAST();
        apply_simple_pressure_subtri_nonorth_correction(
            ...
            ss.d_gradx, ss.d_grady, ss.d_gradz,
            ss.d_phiNonOrth, ss.d_divNonOrth);
      if(par.simpleMmsEnable && rank == 0 && (step <= 10 || (par.printEvery > 0 && (step % par.printEvery) == 0))){
        const double maxDivBaseDbg = maxabs_device(ss.d_divStar, mesh.nCells, ss.d_reduce, ss.reduceSize);
        const double maxDivNonOrthDbg = maxabs_device(ss.d_divNonOrth, mesh.nCells, ss.d_reduce, ss.reduceSize);
        std::printf("SIMPLE_MMS_PLOOP_DIV step=%04d pcorr=%d maxDivBase=%.12e maxDivNonOrth=%.12e scaledNonOrth=%.12e pNonOrthScale=%.6g fixedBoundaryFlux=%d\n",
            step, pcorr, maxDivBaseDbg, maxDivNonOrthDbg,
            par.pNonOrthScale * maxDivNonOrthDbg, par.pNonOrthScale,
            pressureEqnFixedVelocityBoundaryFlux ? 1 : 0);
      }
    ...
    if(par.simpleMmsEnable && par.simpleMmsTracePressureEqn){
      simple_mms_trace_pressure_equation("phi_after_solved_p", step, ss.d_pCorr, ss.d_phi);
    }
    simple_mms_debug_stage("after_pressure_solve_pCorr_field", step, ss.d_u, ss.d_v, ss.d_w, ss.d_pCorr);
    ...
    simple_mms_debug_stage("after_pressure_update_before_velcorr", step, ss.d_u, ss.d_v, ss.d_w, ss.d_p);
    ...
    simple_mms_debug_stage("after_velocity_correction", step, ss.d_u, ss.d_v, ss.d_w, ss.d_p);
  ...
  double simpleMmsExactPFluxMassMax = 0.0;
  double simpleMmsExactPDivVolMax = 0.0;
  double simpleMmsPhiStarMassMax = 0.0;
  double simpleMmsPhiStarDivVolMax = 0.0;

  if(par.simpleMmsEnable && par.simpleMmsAuditPressureEqn){
    const int block = 256;

    // Fill exact manufactured pressure at cell centres into d_pCorrDelta.
    kernel_fill_simple_mms_pressure_exact_device<<<(mesh.nCells + block - 1)/block, block>>>(
        mesh.nCells,
        dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
        par.rho, mu,
        par.simpleMmsVelocityScale,
    ...
    // Audit the raw HbyA/predictor flux imbalance.
    if(false && par.simpleConsistentPhiStar){
      const int blockConsPhiRhs = 256;
      const int gridConsPhiRhs = (mesh.nFaces + blockConsPhiRhs - 1) / blockConsPhiRhs;

      // ss.d_phi contains physical predictor flux saved before HbyA.
      // Build pressure-operator-consistent phiStar BEFORE the pressure RHS
      // continuity residual is assembled.
      kernel_correct_face_fluxes_simple_nonorth<<<gridConsPhiRhs, blockConsPhiRhs>>>(
          mesh.nFaces, mesh.nInternalFaces,
          dmesh.d_owner, dmesh.d_neigh, dmesh.d_bPatch,
          dmesh.d_ccx, dmesh.d_ccy, dmesh.d_ccz,
          dmesh.d_xfx, dmesh.d_xfy, dmesh.d_xfz,
      ...
      if(pressureEqnFixedVelocityBoundaryFlux){
        const int nBF = mesh.nFaces - mesh.nInternalFaces;
        const int gridBFlux = (nBF + blockConsPhiRhs - 1) / blockConsPhiRhs;
        kernel_mms_enforce_fixed_velocity_boundary_flux<<<gridBFlux, blockConsPhiRhs>>>(
            mesh.nFaces, mesh.nInternalFaces, ss.d_phiPhys, ss.d_phiNonOrth);
        CUDA_CHECK_LAST();
      }

      kernel_simple_mms_build_discrete_exact_phistar<<<gridConsPhiRhs, blockConsPhiRhs>>>(
          mesh.nFaces, ss.d_phiPhys, ss.d_phiNonOrth, ss.d_phiStar);
      CUDA_CHECK_LAST();

      if(rank == 0){
        std::printf("SIMPLE consistent phiStar: rebuilt phiStar BEFORE pressure RHS.\n");
      }
    }


    continuity_residual_gpu(dmesh, ss.d_phiStar, ss.d_divNonOrth);
    simpleMmsPhiStarMassMax = maxabs_device(ss.d_divNonOrth, mesh.nCells, ss.d_reduce, ss.reduceSize);
    kernel_divide_by_volume_inplace<<<(mesh.nCells + block - 1)/block, block>>>(
        mesh.nCells, ss.d_divNonOrth, dmesh.d_vol);
    CUDA_CHECK_LAST();
    simpleMmsPhiStarDivVolMax = maxabs_device(ss.d_divNonOrth, mesh.nCells, ss.d_reduce, ss.reduceSize);

    // Audit corrected flux using exact manufactured pressure in the current
    // pressure equation. For blockMesh pNonOrthScale is zero, so we explicitly
    // pass zero and use ss.d_phiNonOrth as a temporary flux buffer.
    kernel_zero_double<<<(mesh.nFaces + block - 1)/block, block>>>(ss.d_phiNonOrth, mesh.nFaces);
    CUDA_CHECK_LAST();
    ...
    continuity_residual_gpu(dmesh, ss.d_phiNonOrth, ss.d_divNonOrth);
    simpleMmsExactPFluxMassMax = maxabs_device(ss.d_divNonOrth, mesh.nCells, ss.d_reduce, ss.reduceSize);
    kernel_divide_by_volume_inplace<<<(mesh.nCells + block - 1)/block, block>>>(
        mesh.nCells, ss.d_divNonOrth, dmesh.d_vol);
    CUDA_CHECK_LAST();
    simpleMmsExactPDivVolMax = maxabs_device(ss.d_divNonOrth, mesh.nCells, ss.d_reduce, ss.reduceSize);

    if(rank == 0){
      // Host-geometry pressure-flux fit diagnostic.
      // Only the two flux arrays are copied from device. Face centres and area
      // vectors are taken from host mesh.xf / mesh.Sf to avoid DeviceMesh
      // layout assumptions.
      std::vector<double> h_phiStar(mesh.nFaces, 0.0);
      std::vector<double> h_phiExactP(mesh.nFaces, 0.0);

      CUDA_CALL(cudaMemcpy(h_phiStar.data(), ss.d_phiStar,
                           mesh.nFaces*sizeof(double), cudaMemcpyDeviceToHost));
      CUDA_CALL(cudaMemcpy(h_phiExactP.data(), ss.d_phiNonOrth,
                           mesh.nFaces*sizeof(double), cudaMemcpyDeviceToHost));
      ...
      auto add = [&](Acc& a, int f){
        const double Sx = mesh.Sf[f][0];
        const double Sy = mesh.Sf[f][1];
        const double Sz = mesh.Sf[f][2];

        const auto qf = simple_mms_exact_value(
            mesh.xf[f][0], mesh.xf[f][1], mesh.xf[f][2],
            par.rho, mu,
            par.simpleMmsVelocityScale,
            par.simpleMmsPressureScale);

        const double phiUExact = par.rho * (qf.u*Sx + qf.v*Sy + qf.w*Sz);

        // If HbyA is correct:
        //   phiStar = phi(U_exact) + phi(rAU grad p_exact)
        // so exact pressure should contribute:
        //   actual correction = phiExactP - phiStar ≈ -[phiStar - phi(U_exact)]
        const double lift = h_phiStar[f] - phiUExact;
        const double targetCorr = -lift;
        const double actualCorr = h_phiExactP[f] - h_phiStar[f];
        const double err = actualCorr - targetCorr;

        a.n += 1;
        a.dot += actualCorr * targetCorr;
        a.tt  += targetCorr * targetCorr;
        a.cc  += actualCorr * actualCorr;
        a.ee  += err * err;
        a.maxTarget = std::max(a.maxTarget, std::fabs(targetCorr));
        a.maxCorr   = std::max(a.maxCorr,   std::fabs(actualCorr));
        a.maxErr    = std::max(a.maxErr,    std::fabs(err));
      };

      Acc all, internal, boundary;
      for(int f=0; f<mesh.nFaces; ++f){
        add(all, f);
        if(f < mesh.nInternalFaces) add(internal, f);
        else add(boundary, f);
      }

      auto print_acc = [&](const char* name, const Acc& a){
        const double alpha = a.dot / std::max(a.tt, 1.0e-300);
        const double relErr = std::sqrt(std::max(a.ee, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));
        const double corrOverTarget = std::sqrt(std::max(a.cc, 0.0)) / std::sqrt(std::max(a.tt, 1.0e-300));

        std::printf(
            "MMSPFLUXFIT scope=%s n=%d alpha=%.12e relErr=%.12e corrNormOverTarget=%.12e "
            "maxTarget=%.12e maxCorr=%.12e maxErr=%.12e\n",
            name, a.n, alpha, relErr, corrOverTarget,
            a.maxTarget, a.maxCorr, a.maxErr);
      };

      print_acc("all", all);
      print_acc("internal", internal);
      print_acc("boundary", boundary);
    }

    if(rank == 0){
      std::printf("------------------------------------------------------------\n");
      std::printf("SIMPLE MMS pressure-equation audit at final HbyA/rAU\n");
      std::printf("  max|div(phiStar)| flux-sum        = %.12e\n", simpleMmsPhiStarMassMax);
      std::printf("  max|div(phiStar)| / V             = %.12e\n", simpleMmsPhiStarDivVolMax);
      std::printf("  max|div(phiExactP)| flux-sum      = %.12e\n", simpleMmsExactPFluxMassMax);
      std::printf("  max|div(phiExactP)| / V           = %.12e\n", simpleMmsExactPDivVolMax);
      std::printf("  ratio exactP/phiStar flux-sum     = %.12e\n",
          simpleMmsExactPFluxMassMax / fmax(simpleMmsPhiStarMassMax, 1.0e-300));
      std::printf("  ratio exactP/phiStar div/V        = %.12e\n",
          simpleMmsExactPDivVolMax / fmax(simpleMmsPhiStarDivVolMax, 1.0e-300));
    }
  }
  ...
  double simpleMmsUAbsL2Core1 = 0.0, simpleMmsURelL2Core1 = 0.0;
  double simpleMmsPAbsL2Core1 = 0.0, simpleMmsPRelL2Core1 = 0.0, simpleMmsPMeanOffsetCore1 = 0.0;
  double simpleMmsUAbsL2Core2 = 0.0, simpleMmsURelL2Core2 = 0.0;
  double simpleMmsPAbsL2Core2 = 0.0, simpleMmsPRelL2Core2 = 0.0, simpleMmsPMeanOffsetCore2 = 0.0;
  int simpleMmsNCore1 = 0, simpleMmsNCore2 = 0;
  ...
  if(par.simpleMmsEnable){
    ...
    const int Nguess = std::max(1, (int)std::llround(std::cbrt((double)mesh.nCells)));
    const double hguess = 1.0 / (double)Nguess;

    simple_mms_accumulate_error_for_margin(mesh, u, v, w, p, par.rho, mu,
        par.simpleMmsVelocityScale, par.simpleMmsPressureScale,
        1.01*hguess,
        simpleMmsUAbsL2Core1, simpleMmsURelL2Core1,
        simpleMmsPAbsL2Core1, simpleMmsPRelL2Core1,
        simpleMmsPMeanOffsetCore1,
        simpleMmsNCore1);

    simple_mms_accumulate_error_for_margin(mesh, u, v, w, p, par.rho, mu,
        par.simpleMmsVelocityScale, par.simpleMmsPressureScale,
        2.01*hguess,
        simpleMmsUAbsL2Core2, simpleMmsURelL2Core2,
        simpleMmsPAbsL2Core2, simpleMmsPRelL2Core2,
        simpleMmsPMeanOffsetCore2,
        simpleMmsNCore2);
  }

  if(par.simpleMmsEnable && rank == 0){
    simple_mms_print_stage_norms("final", stepConverged, mesh, u, v, w, p,
        par.rho, mu,
        par.simpleMmsVelocityScale,
        par.simpleMmsPressureScale);

    const int NguessShape = std::max(1, (int)std::llround(std::cbrt((double)mesh.nCells)));
    const double hShape = 1.0 / (double)NguessShape;
    simple_mms_pressure_shape_diagnostic(mesh, p, par.rho, mu,
        par.simpleMmsVelocityScale, par.simpleMmsPressureScale,
        -1.0, "all");
    simple_mms_pressure_shape_diagnostic(mesh, p, par.rho, mu,
        par.simpleMmsVelocityScale, par.simpleMmsPressureScale,
    ...
    simple_mms_pressure_layer_mode_diagnostic(mesh, p, par.rho, mu,
        par.simpleMmsVelocityScale, par.simpleMmsPressureScale);
  }
  ...
  if(rank==0){
    ...
    if(par.simpleMmsEnable){
      std::printf("------------------------------------------------------------\n");
      std::printf("SIMPLE MMS U abs L2  = %.12e\n", simpleMmsUAbsL2);
      std::printf("SIMPLE MMS U rel L2  = %.12e\n", simpleMmsURelL2);
      std::printf("SIMPLE MMS p abs L2  = %.12e  (mean-free)\n", simpleMmsPAbsL2);
      std::printf("SIMPLE MMS p rel L2  = %.12e  (mean-free)\n", simpleMmsPRelL2);
      std::printf("SIMPLE MMS p mean offset = %.12e\n", simpleMmsPMeanOffset);
      std::printf("SIMPLE MMS core1 cells = %d\n", simpleMmsNCore1);
      std::printf("SIMPLE MMS U rel L2 core1 = %.12e\n", simpleMmsURelL2Core1);
      std::printf("SIMPLE MMS p rel L2 core1 = %.12e  (mean-free, excludes one boundary layer)\n", simpleMmsPRelL2Core1);
      std::printf("SIMPLE MMS core2 cells = %d\n", simpleMmsNCore2);
      std::printf("SIMPLE MMS U rel L2 core2 = %.12e\n", simpleMmsURelL2Core2);
      std::printf("SIMPLE MMS p rel L2 core2 = %.12e  (mean-free, excludes two boundary layers)\n", simpleMmsPRelL2Core2);
    }
    if(cylForce.valid){
      std::printf("------------------------------------------------------------\n");
  ...
  if(par.simpleMmsEnable){
    ...
    if(par.simpleMmsAuditPressureEqn){
      sout << "simpleMmsPhiStarMassMax " << simpleMmsPhiStarMassMax << "\n";
      sout << "simpleMmsPhiStarDivVolMax " << simpleMmsPhiStarDivVolMax << "\n";
      sout << "simpleMmsExactPFluxMassMax " << simpleMmsExactPFluxMassMax << "\n";
      sout << "simpleMmsExactPDivVolMax " << simpleMmsExactPDivVolMax << "\n";
    }
    sout << "simpleMmsNCore1 " << simpleMmsNCore1 << "\n";
    sout << "simpleMmsURelL2Core1 " << simpleMmsURelL2Core1 << "\n";
    sout << "simpleMmsPRelL2Core1MeanFree " << simpleMmsPRelL2Core1 << "\n";
    sout << "simpleMmsNCore2 " << simpleMmsNCore2 << "\n";
    sout << "simpleMmsURelL2Core2 " << simpleMmsURelL2Core2 << "\n";
    sout << "simpleMmsPRelL2Core2MeanFree " << simpleMmsPRelL2Core2 << "\n";
  }
