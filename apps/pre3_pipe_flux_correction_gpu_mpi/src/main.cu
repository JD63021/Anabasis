    ...
    HYPRE_Complex *rhs)
{
  ...
  const double D = ss / dDotS;
  // PRE3E2 pressure-correction skeleton:
...
// -----------------------------------------------------------------------------
// PRE3E2 velocity bridge kernels.
...
// -----------------------------------------------------------------------------
// PRE3E2 pipe flux correction report.
...
// -----------------------------------------------------------------------------
// PRE3E2 pipe pressure-correction kernels.
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    if(rank == 0) {
      ...
      std::printf("PRE3E2 pipe flux correction report setup: ranks=%d globalRows=%lld globalNnz=%lld\n",
    ...
    // Pre-solve vector halo for predictor flux on processor faces.
    ...
    if(!pFace.empty()) {
      ...
      if(procPatches.size() != 1) {
        throw std::runtime_error("PRE3E2 currently expects exactly one processor patch");
      }
      ...
      if(recvN != sendN) {
        throw std::runtime_error("PRE3E2 predictor velocity halo count mismatch");
      }
    ...
    k_boundary_dirichlet_geom_poisson<<<((int)bFace.size() + block - 1)/block, block>>>(
      (int)bFace.size(),
    ...
    // -----------------------------------------------------------------------
    // PRE3E2: pCorr/pressure halo exchange bridge.
    ...
    if(!pFace.empty()) {
      ...
      if(recvN != sendN) {
        throw std::runtime_error("PRE3E2 pCorr halo count mismatch");
      }
      ...
      // PRE3E2: vector U/V/W halo exchange, host staged first.
      ...
      if(rank == 0) {
        std::printf("PRE3E2 vector halo / velocity correction bridge: globalMaxVecJump=%.12e globalMaxVelCorr=%.12e globalSumVelCorr=%.12e\n",
      ...
      if(rank == 0) {
        std::printf("PRE3E2 pCorr halo/correction bridge: nProcFacesLocal=%zu globalMaxHaloJump=%.12e globalMaxFaceCorr=%.12e globalSumAbsFaceCorr=%.12e\n",
      ...
      // PRE3E2 pressure-correction residual: apply A*x-rhs on GPU.
      ...
      if(rank == 0) {
        ...
        std::printf("PRE3E2 GPU pressure-correction residual: resInf=%.12e resL2=%.12e rhsInf=%.12e relInf=%.12e\n",
                    globalResInf,
                    globalResL2,
                    globalRhsInf,
                    globalResInf / std::max(globalRhsInf, 1e-300));

        std::printf("PRE3E2 mass/flux correction summary: massBeforeInf=%.12e massAfterInf=%.12e reduction=%.12e improvement=%.6e\n",
                    massBefore,
                    massAfter,
                    reduction,
                    improvement);
    ...
    if(rank == 0) {
      std::printf("PRE3E2 pipe flux correction report solve RESULT: solveErr=%d its=%d finalRel=%.12e xL2=%.12e xInf=%.12e wall=%.6e s\n",
                  (int)solveErr, (int)its, double(rel), globalL2, globalInf, maxWall);
      if(std::isfinite(globalL2) && std::isfinite(globalInf)) {
        std::printf("PRE3E2 RESULT: PASS_RAN\n");
      } else {
        std::printf("PRE3E2 RESULT: FAIL_NAN_INF\n");
      }
    }
