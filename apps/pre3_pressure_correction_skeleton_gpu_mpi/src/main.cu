...
// -----------------------------------------------------------------------------
// PRE3D2 velocity bridge kernels.
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    if(rank == 0) {
      ...
      std::printf("PRE3D2 pressure correction skeleton setup: ranks=%d globalRows=%lld globalNnz=%lld\n",
    ...
    // -----------------------------------------------------------------------
    // PRE3D2: pCorr/pressure halo exchange bridge.
    ...
    if(!pFace.empty()) {
      ...
      if(procPatches.size() != 1) {
        throw std::runtime_error("PRE3D2 currently expects exactly one processor patch");
      }
      ...
      if(recvN != sendN) {
        throw std::runtime_error("PRE3D2 pCorr halo count mismatch");
      }
      ...
      // PRE3D2: vector U/V/W halo exchange, host staged first.
      ...
      if(rank == 0) {
        std::printf("PRE3D2 vector halo / velocity correction bridge: globalMaxVecJump=%.12e globalMaxVelCorr=%.12e globalSumVelCorr=%.12e\n",
    ...
    if(rank == 0) {
      std::printf("PRE3D2 pressure correction skeleton pressure-solve RESULT: solveErr=%d its=%d finalRel=%.12e L2=%.12e Linf=%.12e wall=%.6e s\n",
                  (int)solveErr, (int)its, double(rel), globalL2, globalInf, maxWall);
      if(std::isfinite(globalL2) && std::isfinite(globalInf)) {
        std::printf("PRE3D2 pressure correction residual skeleton RESULT: PASS_RAN\n");
      } else {
        std::printf("PRE3D2 pressure correction residual skeleton RESULT: FAIL_NAN_INF\n");
      }
    }
