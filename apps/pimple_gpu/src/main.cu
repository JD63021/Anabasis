...
struct Params {
  ...
  // PIMPLE time controls.  nsteps is interpreted as number of physical
  // time steps in this app.  If dt <= 0, dt is estimated from CFL*hmin/Umean.
  // timeScheme: 0=Euler/BDF1, 1=BDF2/backward.  BDF2 bootstraps step 1
  // with Euler and uses (3U^{n+1}-4U^n+U^{n-1})/(2dt) from step 2 onward.
  double dt=-1.0, startTime=0.0;
  int timeScheme=0;
  int nOuterCorrectors=1;
  int nCorrectors=1;
  int momentumPredictor=1;
  int transientMomentum=1;
  ...
  // OpenFOAM-style final-iteration controls for BDF1 PIMPLE.
  // Intermediate outer/corrector passes may be relaxed/inexact; the final
  // outer+pressure-corrector pass can be unrelaxed and tighter.  Negative
  // final absolute tolerances mean "reuse the non-final value".
  double uRelaxFinal=1.0, pRelaxFinal=1.0;
  double velTolFinal=-1.0, velRelTolFinal=0.0;
  double pTolFinal=-1.0, pRelTolFinal=0.0;
  int velMaxitFinal=0, pMaxitFinal=0;
  ...
  // Optional transient force time-series logging.
  // If forceEvery > 0, the code copies p/U to host only at those sampled
  // time steps and appends cylinder vector-force coefficients to CSV.
  int forceEvery = 0;
...
static std::vector<std::string> expand_case_config_args(int argc, char** argv){
  ...
  std::map<std::string,std::string> keyToFlag = {
    ...
    {"uRelax", "-u-relax"},
    {"pRelax", "-p-relax"},
    {"uRelaxFinal", "-u-relax-final"},
    {"pRelaxFinal", "-p-relax-final"},
    {"velTolFinal", "-vel-tol-final"},
    {"velRelTolFinal", "-vel-reltol-final"},
    {"pTolFinal", "-p-tol-final"},
    {"pRelTolFinal", "-p-reltol-final"},
    {"velMaxitFinal", "-vel-maxit-final"},
    {"pMaxitFinal", "-p-maxit-final"},
    {"corrTol", "-corrTol"},
    {"tolMass", "-tolMass"},
    {"tolVel", "-tolVel"},
...
static void parse_args(int argc, char** argv, Params &par){
  for(int i=1;i<argc;++i){
    ...
    if(!std::strcmp(argv[i],"-polyMeshDir")){need(argv[i]); par.polyMeshDir=argv[++i];}
    ...
    else if(!std::strcmp(argv[i],"-dt") || !std::strcmp(argv[i],"-deltaT")){need(argv[i]); par.dt=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-time-scheme")){
      need(argv[i]);
      std::string v=argv[++i];
      for(char &c:v) c=(char)std::tolower((unsigned char)c);
      if(v=="euler" || v=="bdf1" || v=="first" || v=="firstorder" || v=="first-order") par.timeScheme=0;
      else if(v=="bdf2" || v=="backward" || v=="second" || v=="secondorder" || v=="second-order") par.timeScheme=1;
      else { std::fprintf(stderr,"Unknown -time-scheme '%s'. Use Euler/BDF1 or BDF2/backward.\n", v.c_str()); MPI_Abort(MPI_COMM_WORLD,1); }
    }
    else if(!std::strcmp(argv[i],"-start-time")){need(argv[i]); par.startTime=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-nOuterCorrectors")){need(argv[i]); par.nOuterCorrectors=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-nCorrectors")){need(argv[i]); par.nCorrectors=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-momentumPredictor")){need(argv[i]); par.momentumPredictor=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-transientMomentum")){need(argv[i]); par.transientMomentum=std::atoi(argv[++i]);}
    ...
    else if(!std::strcmp(argv[i],"-u-relax-final")){need(argv[i]); par.uRelaxFinal=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-p-relax-final")){need(argv[i]); par.pRelaxFinal=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-vel-tol-final")){need(argv[i]); par.velTolFinal=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-vel-reltol-final")){need(argv[i]); par.velRelTolFinal=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-p-tol-final")){need(argv[i]); par.pTolFinal=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-p-reltol-final")){need(argv[i]); par.pRelTolFinal=std::atof(argv[++i]);}
    else if(!std::strcmp(argv[i],"-vel-maxit-final")){need(argv[i]); par.velMaxitFinal=std::atoi(argv[++i]);}
    else if(!std::strcmp(argv[i],"-p-maxit-final")){need(argv[i]); par.pMaxitFinal=std::atoi(argv[++i]);}
      ...
      if(v=="compact" || v=="local" || v=="0") par.lsqStencilMode=0;
        ...
        if(MPI_COMM_WORLD != MPI_COMM_NULL) {
          ...
          if(rank==0) std::fprintf(stderr,"WARNING: pimple_gpu v1.1d currently accepts -lsq-stencil extended but uses compact LSQ coefficients.\n");
        }
...
struct GPUSimpleScratch {
  ...
  double *d_uTimeOld=nullptr, *d_vTimeOld=nullptr, *d_wTimeOld=nullptr, *d_pTimeOld=nullptr;
  double *d_uTimeOldOld=nullptr, *d_vTimeOldOld=nullptr, *d_wTimeOldOld=nullptr;
  double *d_HbyAx=nullptr, *d_HbyAy=nullptr, *d_HbyAz=nullptr;
    ...
    DeviceTimeSineVelocityBC& tbc)
{
  ...
  for(const auto& spec : velocityPatchSpecs){
    ...
    // Generate peak-amplitude values once by evaluating the runtime BC at
    // t = timeScale/2, for which sin(pi*t/timeScale)=1.  Runtime time stepping
    // then scales these peak values on the GPU only; no per-step host->device
    // boundary-value copy is needed.
...
static void update_device_time_sine_velocity_bc(
    const DeviceTimeSineVelocityBC& tbc,
    double time,
    DeviceBC& dbcU,
    DeviceBC& dbcV,
    DeviceBC& dbcW)
...
static void init_simple_scratch(GPUSimpleScratch &ss, const Mesh &mesh){
  ...
  device_alloc(ss.d_uTimeOld, mesh.nCells); device_alloc(ss.d_vTimeOld, mesh.nCells); device_alloc(ss.d_wTimeOld, mesh.nCells);
  device_alloc(ss.d_uTimeOldOld, mesh.nCells); device_alloc(ss.d_vTimeOldOld, mesh.nCells); device_alloc(ss.d_wTimeOldOld, mesh.nCells);
  device_alloc(ss.d_HbyAx, mesh.nCells); device_alloc(ss.d_HbyAy, mesh.nCells); device_alloc(ss.d_HbyAz, mesh.nCells);
  device_alloc(ss.d_p, mesh.nCells); device_alloc(ss.d_pOld, mesh.nCells); device_alloc(ss.d_pTimeOld, mesh.nCells); device_alloc(ss.d_pCorr, mesh.nCells);
...
static void destroy_simple_scratch(GPUSimpleScratch &ss){
  ...
  device_free(ss.d_uTimeOld); device_free(ss.d_vTimeOld); device_free(ss.d_wTimeOld); device_free(ss.d_pTimeOld);
  device_free(ss.d_uTimeOldOld); device_free(ss.d_vTimeOldOld); device_free(ss.d_wTimeOldOld);
  device_free(ss.d_HbyAx); device_free(ss.d_HbyAy); device_free(ss.d_HbyAz);
...
__global__ static void kernel_add_bdf1_time_to_momentum(
    int nCells, const int *diagPos, const double *vol, const double *qTimeOld,
    double rho, double dt, HYPRE_Complex *vals, HYPRE_Complex *rhs)
{
  int c = blockIdx.x * blockDim.x + threadIdx.x;
  if(c < nCells){
    const double aTime = rho * vol[c] / dt;
    vals[diagPos[c]] += (HYPRE_Complex)aTime;
    rhs[c] += (HYPRE_Complex)(aTime * qTimeOld[c]);
  }
}

__global__ static void kernel_add_bdf2_time_to_momentum(
    int nCells, const int *diagPos, const double *vol,
    const double *qTimeOld, const double *qTimeOldOld,
    double rho, double dt, HYPRE_Complex *vals, HYPRE_Complex *rhs)
{
  int c = blockIdx.x * blockDim.x + threadIdx.x;
  if(c < nCells){
    const double massOverDt = rho * vol[c] / dt;
    vals[diagPos[c]] += (HYPRE_Complex)(1.5 * massOverDt);
    rhs[c] += (HYPRE_Complex)(massOverDt * (2.0 * qTimeOld[c] - 0.5 * qTimeOldOld[c]));
  }
}
...
__global__ static void kernel_add_bdf1_time_rhs_to_momentum(
    int nCells, const double *vol, const double *qTimeOld,
    double rho, double dt, HYPRE_Complex *rhs)
{
  int c = blockIdx.x * blockDim.x + threadIdx.x;
  if(c < nCells){
    const double aTime = rho * vol[c] / dt;
...
__global__ static void kernel_add_bdf2_time_rhs_to_momentum(
    int nCells, const double *vol, const double *qTimeOld, const double *qTimeOldOld,
    double rho, double dt, HYPRE_Complex *rhs)
{
  int c = blockIdx.x * blockDim.x + threadIdx.x;
  if(c < nCells){
...
static void assemble_momentum_on_gpu_device_grad(
    const DeviceMesh &dm, const Mesh &mesh, GPUMomentumAssembler &mom,
    double rho, double mu, double transientDt, int timeSchemeActive,
    const double *d_qOld,
    const double *d_qTimeOld,
    const double *d_qTimeOldOld,
    ...
    double corrPsi, int momentumConvectionScheme)
{
  (void)d_qOld; // used by relaxation outside this assembly path
  ...
  kernel_momentum_base_steady<<<gridCells, block>>>(
      ...
      Avals,
      mom.lin.d_rhs);
  if(transientDt > 0.0 && d_qTimeOld){
    if(timeSchemeActive == 1 && d_qTimeOldOld){
      kernel_add_bdf2_time_to_momentum<<<gridCells, block>>>(
          mesh.nCells, mom.lin.pat.d_diagPos, dm.d_vol, d_qTimeOld, d_qTimeOldOld,
          rho, transientDt, Avals, mom.lin.d_rhs);
    } else {
      kernel_add_bdf1_time_to_momentum<<<gridCells, block>>>(
          mesh.nCells, mom.lin.pat.d_diagPos, dm.d_vol, d_qTimeOld,
          rho, transientDt, Avals, mom.lin.d_rhs);
    }
  }
    ...
    double uRelax)
{
  ...
  if(transientDt > 0.0 && d_qTimeOld){
    if(timeSchemeActive == 1 && d_qTimeOldOld){
      kernel_add_bdf2_time_rhs_to_momentum<<<gridCells, block>>>(
          mesh.nCells, dm.d_vol, d_qTimeOld, d_qTimeOldOld, rho, transientDt, mom.lin.d_rhs);
    } else {
      kernel_add_bdf1_time_rhs_to_momentum<<<gridCells, block>>>(
          mesh.nCells, dm.d_vol, d_qTimeOld, rho, transientDt, mom.lin.d_rhs);
    }
  }
...
struct ForceTimeStats {
  bool active = false;
  int samples = 0;
  double cdMin =  1.0e300, cdMax = -1.0e300;
  double clyMin = 1.0e300, clyMax = -1.0e300;
  double clzMin = 1.0e300, clzMax = -1.0e300;
};

static void update_force_time_stats(ForceTimeStats& s, const CylinderForceVectorReport& r)
{
  if(!r.valid) return;
  s.active = true;
  s.samples += 1;
  s.cdMin  = std::min(s.cdMin,  r.C[0]);
  s.cdMax  = std::max(s.cdMax,  r.C[0]);
  s.clyMin = std::min(s.clyMin, r.C[1]);
  s.clyMax = std::max(s.clyMax, r.C[1]);
  s.clzMin = std::min(s.clzMin, r.C[2]);
  s.clzMax = std::max(s.clzMax, r.C[2]);
}
...
static void write_force_timeseries_header(const std::string& path, bool append)
{
  ...
  std::ofstream out;
  if(append) out.open(path.c_str(), std::ios::out | std::ios::app);
  else       out.open(path.c_str(), std::ios::out | std::ios::trunc);
  if(!out) return;
  if(!append){
    out << "step,time,CD_vector,CL_y_vector,CL_z_vector,"
        << "Fx,Fy,Fz,Fp_x,Fp_y,Fp_z,Fv_x,Fv_y,Fv_z,"
...
static void append_force_timeseries_row(
    ...
    double massRes, double duRel, double dvRel, double dwRel, double dpRel,
    const CylinderForceVectorReport& r)
{
...
CUDA_CALL(cudaFree(0));
  ...
  if(par.dt <= 0.0) par.dt = dt0;
  if(par.dt <= 0.0 || !std::isfinite(par.dt)){
    if(rank==0) std::fprintf(stderr, "Invalid PIMPLE dt=%.12e. Set dt or CFL/Umean so dt is positive.\n", par.dt);
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  if(!par.momentumPredictor){
    if(rank==0) std::fprintf(stderr, "pimple_gpu v1.1d currently requires momentumPredictor 1.\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  // PIMPLE uses the OpenFOAM-like absolute pressure path by default.
  // It is still overridable, but the known robust combination is ofAbsolute + oflike.
  if(par.pSolveMode == 1) par.pMode = 1;

  if(rank==0){
    std::printf("============================================================\n");
    std::printf("Anabasis pimple_gpu v1.1d.2: OpenFOAM polyMesh PIMPLE solver\n");
    ...
    std::printf("time scheme    : %s transient momentum %s%s\n", par.timeScheme == 1 ? "BDF2/backward" : "BDF1/Euler", par.transientMomentum ? "ON" : "OFF", (par.timeScheme == 1 ? " (step 1 Euler bootstrap; ddtCorr OFF)" : ""));
    std::printf("startTime      : %.8g\n", par.startTime);
    std::printf("dt             : %.8g%s\n", par.dt, (std::fabs(par.dt - dt0) < 1e-14*std::max(1.0, std::fabs(dt0))) ? " (from CFL estimate/default)" : "");
    std::printf("nTimeSteps     : %d\n", par.nsteps);
    std::printf("nOuterCorrectors: %d\n", par.nOuterCorrectors);
    std::printf("nCorrectors    : %d\n", par.nCorrectors);
    std::printf("momentumPredictor: %d\n", par.momentumPredictor);
    ...
    std::printf("velFinalTol / velFinalRelTol : %.3e / %.3e\n",
                (par.velTolFinal >= 0.0 ? par.velTolFinal : par.velTol), par.velRelTolFinal);
    std::printf("pTol   / pRelTol   : %.3e / %.3e\n", par.pTol, par.pRelTol);
    std::printf("pFinalTol / pFinalRelTol   : %.3e / %.3e\n",
                (par.pTolFinal >= 0.0 ? par.pTolFinal : par.pTol), par.pRelTolFinal);
    std::printf("pAmgRebuildEvery   : %d\n", par.pAmgRebuildEvery);
    if(par.forceEnable){
      std::printf("forcePatch     : %s\n", par.forcePatchName.c_str());
      std::printf("forceEvery     : %d\n", par.forceEvery);
      std::printf("forceLog       : %s\n", par.forceLogPath.empty() ? "<outPrefix>_force_timeseries.csv" : par.forceLogPath.c_str());
    }
    std::printf("------------------------------------------------------------\n");
  }
  ...
  if(par.bcConfigPath.empty()){
    if(rank==0){
      std::fprintf(stderr,
          "pimple_gpu requires explicit runtime BCs.\n"
  ...
  // Force coefficient convention used by the existing cylinder vector postprocess.
  // For the benchmark inlet profile, Umean is the profile maximum Um and
  // cross-section average speed is 4/9 Um.
  const double forceBenchmarkH = 0.41;
  const double forceUbar = (4.0/9.0) * par.Umean;

  ForceTimeStats forceStats;
  std::string forceLogPath = par.forceLogPath.empty()
      ? (par.outPrefix + "_force_timeseries.csv")
      : par.forceLogPath;
  if(rank == 0 && par.forceEnable && par.forceEvery > 0){
    forceStats.active = true;
    write_force_timeseries_header(forceLogPath, par.forceLogAppend != 0);
    std::printf("Force time-series logging: every=%d path=%s mode=%s\n",
                par.forceEvery, forceLogPath.c_str(),
                par.forceLogAppend ? "append" : "truncate");
  }

  int activeTimeScheme = 0; // 0=Euler/BDF1, 1=BDF2 for current physical step; captured by assembly lambdas
    ...
    for(int it=1; it<=nVelSolves; ++it){
      ...
      if(doMatrixSetup){
        assemble_momentum_on_gpu_device_grad(
            ...
            d_qOut, d_qTimeOld, d_qTimeOldOld, d_uConv, d_vConv, d_wConv,
        ...
        assemble_momentum_rhs_only_on_gpu_device_grad(
            ...
            mom.d_gradQx, mom.d_gradQy, mom.d_gradQz, d_gradPcomp,
            dbcQ, dbcU, dbcV, dbcW,
            par.momNonOrthScale, uRelaxThis);
      }
      ...
      PhaseStats &asmStats)
  {
    ...
    if(rebuildMomentumMatrix){
      assemble_momentum_on_gpu_device_grad(
          dmesh, mesh, mom, par.rho, mu, par.transientMomentum ? par.dt : -1.0, activeTimeScheme,
          d_qInitial, d_qTimeOld, d_qTimeOldOld, d_uConv, d_vConv, d_wConv,
          mom.d_gradQx, mom.d_gradQy, mom.d_gradQz, d_gradPcomp,
          dbcQ, dbcU, dbcV, dbcW,
          par.momNonOrthScale, par.momentumConvectionScheme);
  ...
  // Initialise U^{n-1} storage for BDF2.  Step 1 uses Euler, so these
  // values are first consumed at step 2 after U^n has been copied into d_*TimeOld.
  CUDA_CALL(cudaMemcpy(ss.d_uTimeOldOld, ss.d_u, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
  CUDA_CALL(cudaMemcpy(ss.d_vTimeOldOld, ss.d_v, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
  CUDA_CALL(cudaMemcpy(ss.d_wTimeOldOld, ss.d_w, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
  ...
  for(int step=1; step<=maxSteps; ++step){
    const double currentTime = par.startTime + step * par.dt;
    // BDF2/backward needs two old time levels.  Step 1 bootstraps with Euler/BDF1.
    activeTimeScheme = (par.transientMomentum && par.timeScheme == 1 && step > 1) ? 1 : 0;
    const double timeStepStart = MPI_Wtime();

    // Apply time-dependent inlet values at t_{n+1}.  For parabolic_box_sine_inlet,
    // only the inlet boundary faces are updated, and the update is a tiny GPU
    // kernel; the full BC face arrays are not copied from host every step.
    update_device_time_sine_velocity_bc(timeSineBC, currentTime, dbcU, dbcV, dbcW);

    CUDA_CALL(cudaMemcpy(ss.d_uTimeOld, ss.d_u, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_vTimeOld, ss.d_v, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_wTimeOld, ss.d_w, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_pTimeOld, ss.d_p, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    int outerUsed = 0;
    const int nOuterPimple = std::max(par.nOuterCorrectors, 1);
    const int nPisoCorrectors = std::max(std::max(par.nCorrectors, 1), std::max(par.nPressureCorr, 0) + 1);
    const int nNonOrthPressure = std::max(par.nNonOrthCorr, 0);
    for(int pimpleOuter=1; pimpleOuter<=nOuterPimple; ++pimpleOuter){
    outerUsed = pimpleOuter;
    const bool finalOuter = (pimpleOuter == nOuterPimple);
    const double uRelaxThis = finalOuter ? par.uRelaxFinal : par.uRelax;
    Params velSolvePar = par;
    if(finalOuter){
      velSolvePar.velRelTol = par.velRelTolFinal;
      if(par.velTolFinal >= 0.0) velSolvePar.velTol = par.velTolFinal;
      if(par.velMaxitFinal > 0) velSolvePar.velMaxit = par.velMaxitFinal;
    }
    ...
    if(par.velSolver == 3){
      ...
      assemble_scalar_component_rhs_for_fused(
          ss.d_uOld, ss.d_uTimeOld, ss.d_uTimeOldOld, ss.d_uOld, ss.d_vOld, ss.d_wOld,
          ss.d_gradx, dbcU, g_fused_mcgs.d_rhsU,
          true, true, uRelaxThis, prof[PH_UGRAD], prof[PH_UASM]);
      assemble_scalar_component_rhs_for_fused(
          ss.d_vOld, ss.d_vTimeOld, ss.d_vTimeOldOld, ss.d_uOld, ss.d_vOld, ss.d_wOld,
          ss.d_grady, dbcV, g_fused_mcgs.d_rhsV,
          false, false, uRelaxThis, prof[PH_VGRAD], prof[PH_VASM]);
      assemble_scalar_component_rhs_for_fused(
          ss.d_wOld, ss.d_wTimeOld, ss.d_wTimeOldOld, ss.d_uOld, ss.d_vOld, ss.d_wOld,
          ss.d_gradz, dbcW, g_fused_mcgs.d_rhsW,
          false, false, uRelaxThis, prof[PH_WGRAD], prof[PH_WASM]);
      ...
      solve_scalar_component_with_nonorth(ss.d_uOld, ss.d_uTimeOld, ss.d_uTimeOldOld, ss.d_uOld, ss.d_vOld, ss.d_wOld, ss.d_gradx, dbcU, ss.d_u, lastItsU, lastRelU, corrUsedU, true,  true,  velSolvePar, uRelaxThis, prof[PH_UGRAD], prof[PH_UASM], prof[PH_USOLVE]);
      solve_scalar_component_with_nonorth(ss.d_vOld, ss.d_vTimeOld, ss.d_vTimeOldOld, ss.d_uOld, ss.d_vOld, ss.d_wOld, ss.d_grady, dbcV, ss.d_v, lastItsV, lastRelV, corrUsedV, false, false, velSolvePar, uRelaxThis, prof[PH_VGRAD], prof[PH_VASM], prof[PH_VSOLVE]);
      solve_scalar_component_with_nonorth(ss.d_wOld, ss.d_wTimeOld, ss.d_wTimeOldOld, ss.d_uOld, ss.d_vOld, ss.d_wOld, ss.d_gradz, dbcW, ss.d_w, lastItsW, lastRelW, corrUsedW, false, false, velSolvePar, uRelaxThis, prof[PH_WGRAD], prof[PH_WASM], prof[PH_WSOLVE]);
    }

    PhaseMark pm_psetup = profile_begin();
    double ps0 = pressureSetup;
    const int rebuildEvery = std::max(par.pAmgRebuildEvery, 1);
    const int globalOuter = (step - 1) * std::max(par.nOuterCorrectors,1) + pimpleOuter;
    const bool doPressureSetup = (!pressureSys.is_setup) || (globalOuter == 1) || (((globalOuter - 1) % rebuildEvery) == 0);
    ...
    // Preserve HbyA separately.  Multiple PISO pressure correctors must always
    // reconstruct U = HbyA - rAU*grad(p) from the same momentum predictor, not
    // from an already pressure-corrected velocity field.
    CUDA_CALL(cudaMemcpy(ss.d_HbyAx, ss.d_u, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_HbyAy, ss.d_v, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_HbyAz, ss.d_w, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    PhaseMark pm_phi = profile_begin();
    {
      ...
      kernel_build_rhiechow_predicted_flux_stokes_3d<<<(mesh.nFaces + block - 1)/block, block>>>(
          ...
          dmesh.d_nfx, dmesh.d_nfy, dmesh.d_nfz,
          dmesh.d_Af,
          ss.d_HbyAx, ss.d_HbyAy, ss.d_HbyAz,
          ss.d_p,
          ss.d_gradx, ss.d_grady, ss.d_gradz,
          mom.d_rAU,
          dbcU.d_type, dbcU.d_faceValue,
    ...
    const int nPressureSolves = nNonOrthPressure + 1;
    int pressureSolveCounter = 0;
    int pcgTotalIts = 0;

    auto update_pressure_anchor = [&](){
      if(usePressureAnchor){
        double pref = 0.0;
        CUDA_CALL(cudaMemcpy(&pref, ss.d_p + refCell, sizeof(double), cudaMemcpyDeviceToHost));
        kernel_subtract_scalar_inplace<<<(mesh.nCells + 255)/256, 256>>>(mesh.nCells, ss.d_p, pref);
        CUDA_CHECK_LAST();
      }
    };

    auto relax_pressure_to_candidate = [&](double pRelaxThis){
      const int block = 256;
      if(par.pMode == 1){
        kernel_update_pressure_absolute_relax<<<(mesh.nCells + block - 1)/block, block>>>(mesh.nCells, ss.d_p, ss.d_pCorr, pRelaxThis);
      } else {
        kernel_update_pressure_relax<<<(mesh.nCells + block - 1)/block, block>>>(mesh.nCells, ss.d_p, ss.d_pCorr, pRelaxThis);
      }
      CUDA_CHECK_LAST();
      update_pressure_anchor();
    };

    auto correct_velocity_from_current_pressure = [&](){
      ...
      kernel_correct_velocity_from_pcorr_grad<<<(mesh.nCells + block - 1)/block, block>>>(
          mesh.nCells, ss.d_HbyAx, ss.d_HbyAy, ss.d_HbyAz, mom.d_rAU,
          ss.d_gradx, ss.d_grady, ss.d_gradz,
          ss.d_u, ss.d_v, ss.d_w);
      CUDA_CHECK_LAST();
    ...
    auto pressure_solve_settings = [&](bool finalPressureSolve, double &relTol, double &absTol, int &maxIt){
      if(finalPressureSolve){
        relTol = par.pRelTolFinal;
        absTol = (par.pTolFinal >= 0.0 ? par.pTolFinal : par.pTol);
        maxIt = (par.pMaxitFinal > 0 ? par.pMaxitFinal : par.pMaxit);
      } else {
        relTol = par.pRelTol;
        absTol = par.pTol;
        maxIt = par.pMaxit;
    ...
    // PIMPLE pressure hierarchy:
    //   nCorrectors     = PISO pressure-corrector passes
    //   nNonOrthCorr    = inner pressure non-orthogonal sweeps for the first pass
    //   final outer+final corrector uses pFinal-style tolerances/relaxation.
    for(int pisoCorr=1; pisoCorr<=nPisoCorrectors; ++pisoCorr){
      const bool finalPisoCorr = (pisoCorr == nPisoCorrectors);
      const bool finalPressureCorrection = finalOuter && finalPisoCorr;
      const double pRelaxThis = finalPressureCorrection ? par.pRelaxFinal : par.pRelax;
      ...
      if(pisoCorr == 1){
        ...
        for(int nonOrth=0; nonOrth<=nNonOrthPressure; ++nonOrth){
          const bool finalNonOrth = (nonOrth == nNonOrthPressure);
          ...
          const bool finalPressureSolve = finalOuter && finalPisoCorr && finalNonOrth;
          double curPRelTol=par.pRelTol, curPTol=par.pTol; int curPMaxit=par.pMaxit;
          pressure_solve_settings(finalPressureSolve, curPRelTol, curPTol, curPMaxit);
          PhaseMark pm_psolve = profile_begin();
          solve_pressure_gpu_device_rhs_device_x0(pressureSys, ss.d_pCorr, ss.d_pCorr, curPRelTol, curPTol, curPMaxit, lastItsP, lastRelP, pressureSolve);
        ...
        // Extra PISO corrector: solve an incremental pressure equation from the
        // remaining face-flux divergence, add it to current absolute pressure,
        // update phi, then correct U from preserved HbyA.
        const int block = 256;
        ...
        ++pressureSolveCounter;
        const bool finalPressureSolve = finalOuter && finalPisoCorr;
        double curPRelTol=par.pRelTol, curPTol=par.pTol; int curPMaxit=par.pMaxit;
        pressure_solve_settings(finalPressureSolve, curPRelTol, curPTol, curPMaxit);
        PhaseMark pm_psolve = profile_begin();
        solve_pressure_gpu_device_rhs_device_x0(pressureSys, ss.d_pCorrDelta, ss.d_pCorrDelta, curPRelTol, curPTol, curPMaxit, lastItsP, lastRelP, pressureSolve);
        profile_record(prof[PH_PSOLVE_LOOP], pm_psolve);
        pcgTotalIts += (int)lastItsP;
      ...
      relax_pressure_to_candidate(pRelaxThis);
      correct_velocity_from_current_pressure();
    }
    ...
    if(rank==0 && (step==1 || (par.printEvery>0 && step%par.printEvery==0))){
      std::printf("timeStep %4d t=%.6e outer %2d/%2d : massRes = %.3e, duRel = %.3e, dvRel = %.3e, dwRel = %.3e, dpRel = %.3e, velIts=[%d %d %d], pcgLast=%d, pcgTot=%d, outerWall = %.3e s, totalWall = %.3e s\n",
                  step, currentTime, pimpleOuter, std::max(par.nOuterCorrectors,1), massRes, duRel, dvRel, dwRel, dpRel,
    ...
    if(!std::isfinite(massRes) || massRes > 1e50){
      if(rank==0) std::fprintf(stderr, "PIMPLE solve diverged.\n");
      break;
    }
    if(massRes < par.tolMass && std::max({duRel,dvRel,dwRel}) < par.tolVel){
      stepConverged = step;
      if(rank==0) std::printf("PIMPLE outer converged at timeStep %d outer %d : t = %.6e massRes = %.3e\n", step, pimpleOuter, currentTime, massRes);
      break;
    }
    stepConverged = step;
    } // end PIMPLE outer corrector loop
    if(rank==0 && par.monitor && (step==1 || (par.printEvery>0 && step%par.printEvery==0))){
      std::printf("completed timeStep %4d t=%.6e outerUsed=%d timeStepWall=%.3e s\n",
                  step, currentTime, outerUsed, MPI_Wtime() - timeStepStart);
    }

    if(par.forceEnable && par.forceEvery > 0 && cylinderPatch >= 0 &&
       (step == 1 || (step % par.forceEvery) == 0)){
      // This is intentionally sampled, not every inner iteration.  The expensive
      // solve path stays GPU-resident; only sampled p/U fields are copied to CPU
      // for the existing, validated force integration routine.
      copy_device_to_vec(ss.d_p, p);
      copy_device_to_vec(ss.d_u, u);
      copy_device_to_vec(ss.d_v, v);
      copy_device_to_vec(ss.d_w, w);

      CylinderForceVectorReport forceSample = compute_cylinder_forces_vector_wall_shear(
          mesh, cylinderPatch, u, v, w, p,
          par.rho, mu, par.pipeDiameter, forceBenchmarkH, forceUbar);

      if(rank == 0 && forceSample.valid){
        update_force_time_stats(forceStats, forceSample);
        append_force_timeseries_row(forceLogPath, step, currentTime,
            massRes, duRel, dvRel, dwRel, dpRel, forceSample);
        std::printf("forceTime step %6d t=%.8e : CD_vector=%.12e CL_y_vector=%.12e CL_z_vector=%.12e samples=%d\n",
                    step, currentTime, forceSample.C[0], forceSample.C[1], forceSample.C[2], forceStats.samples);
      }
    }

    // Advance BDF2 history after the accepted physical time step:
    //   U^{n-1} <- U^n.  At the top of the next loop U^n <- current U^{n+1}.
    CUDA_CALL(cudaMemcpy(ss.d_uTimeOldOld, ss.d_uTimeOld, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_vTimeOldOld, ss.d_vTimeOld, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
    CUDA_CALL(cudaMemcpy(ss.d_wTimeOldOld, ss.d_wTimeOld, mesh.nCells*sizeof(double), cudaMemcpyDeviceToDevice));
  }
  ...
  if(cylForce.valid){
    ...
    if(forceStats.active && forceStats.samples > 0){
      std::printf("------------------------------------------------------------\n");
      std::printf("Cylinder force time-series extrema, vector wall-shear traction\n");
      std::printf("force samples   = %d\n", forceStats.samples);
      std::printf("force log file  = %s\n", forceLogPath.c_str());
      std::printf("CD_vector min/max   = %.12e / %.12e\n", forceStats.cdMin, forceStats.cdMax);
      std::printf("CL_y_vector min/max = %.12e / %.12e\n", forceStats.clyMin, forceStats.clyMax);
      std::printf("CL_z_vector min/max = %.12e / %.12e\n", forceStats.clzMin, forceStats.clzMax);
    }
  ...
  if(forceStats.active && forceStats.samples > 0){
    sout << "forceTimeseriesSamples " << forceStats.samples << "\n";
    sout << "forceTimeseriesLog " << forceLogPath << "\n";
    sout << "CDVectorMin " << forceStats.cdMin << "\n";
    sout << "CDVectorMax " << forceStats.cdMax << "\n";
    sout << "CLyVectorMin " << forceStats.clyMin << "\n";
    sout << "CLyVectorMax " << forceStats.clyMax << "\n";
    sout << "CLzVectorMin " << forceStats.clzMin << "\n";
    sout << "CLzVectorMax " << forceStats.clzMax << "\n";
  }
  if(!scalarField.empty()){
