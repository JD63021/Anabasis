...
#define CUDA_CALL(stmt) do {   cudaError_t _err = (stmt);   if (_err != cudaSuccess) {     std::fprintf(stderr, "CUDA ERROR at %s:%d: %s\n", __FILE__, __LINE__, cudaGetErrorString(_err));     MPI_Abort(MPI_COMM_WORLD, -1);   } } while (0)

#define HYPRE_CALL(stmt) do {   HYPRE_Int _ierr = (stmt);   if (_ierr) {     int _rank = 0; MPI_Comm_rank(MPI_COMM_WORLD, &_rank);     std::fprintf(stderr, "[%d] HYPRE ERROR %s:%d code=%d\n", _rank, __FILE__, __LINE__, (int)_ierr);     MPI_Abort(MPI_COMM_WORLD, (int)_ierr);   } } while (0)
...
inline void init_hypre_gpu_runtime(int device) {
...
inline void finalize_hypre_gpu_runtime() {
